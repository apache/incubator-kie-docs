...
For more detailed monitoring, you can configure {PLANNER_SHORT} to monitor additional metrics at a performance cost. The following example uses the `BEST_SCORE` and `SCORE_CALCULATION_COUNT` metric in the solver configuration:
