...
You can apply updates to {PRODUCT} release 7.12 and earlier or Red Hat Process Automation Manager {PRODUCT_VERSION} as they become available in the Red Hat Customer Portal to keep your distribution current with the latest enhancements and fixes. Red Hat provides update tools and product notifications for new product releases so you can more readily apply helpful updates to your installation environment.
endif::[]
...
ifdef::DM[]
* {PRODUCT} or Red Hat Process Automation Manager is installed. For installation options, see {URL_INSTALLING_AND_CONFIGURING}#assembly-planning[_{PLANNING_INSTALL}_].
endif::[]
ifdef::PAM[]
...
ifdef::DM[]
Red Hat Process Automation Manager {PRODUCT_VERSION}, see
