...
For example, if the `parallelSolverCount` property` is set to `4` and you submit five problems, four problems start solving immediately and the fifth one starts when one of the first problem ends. If those problems solve for five minutes each, the fifth problem takes 10 minutes to finish. By default, `parallelSolverCount` is set to `AUTO`, which resolves to half the CPU cores, regardless of the `moveThreadCount` of the solvers.
...
Batch solving is solving mulitple data sets in parrallel. Batch solving is particularly useful over night:
