...
==== Configuring OptaPlanner for Micrometer on Quarkus
To configure your OptaPlanner Quarkus project to use Micrometer and a specified monitoring sytem, add a dependency to the `pom.xml` file.

.Prerequisites
* You have an OptaPlanner Quarkus application.

.Procedure
. Add the following dependency to your application's `pom.xml` file where `<MONITORING_SYSTEM>` is a monitoring system supported by Micrometer:
+
[source]
----
<dependency>
...
==== Configuring OptaPlanner for Micrometer on Spring Boot
To configure your OptaPlanner Spring Boot project to use Micrometer and a specified monitoring system, add a dependency to the `pom.xml` file.

.Prerequisites
* You have an OptaPlanner Spring Boot application.
...
----
. Add the following line to the application's `application.properties` file:
+
[source]
----
management.endpoints.web.exposure.include=metrics,<MONITORING_SYSTEM>
----
...
[source]
----
http://localhost:8080/actuator/<MONITORING_SYSTEM>
----

==== Configuring OptaPlanner for Micrometer with plain Java


.Prerequisites
* You have a plain Java OptaPlanner project.
* You have configured a monitoring system with Micrometer. For information, see the https://micrometer.io[Micrometer] web site.
