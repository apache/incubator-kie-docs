...
As a business rules developer, you can test and interact with the {PLANNER} functionality using the preconfigured `optaweb-employee-rostering` sample project included in the {PRODUCT} distribution.

The `optaweb-employee-rostering` sample project can be built and deployed in {CENTRAL}. The project is designed to demonstrate how to create each of the {CENTRAL} assets required to solve the shift rostering planning problem and use {PLANNER} to find the best possible solution. Use this document to deploy the preconfigured `optaweb-employee-rostering` project in {CENTRAL}, or to create the project yourself using {CENTRAL}.

[NOTE]
====
The `optaweb-employee-rostering` sample project in {CENTRAL} does not include a data set.
====

.Prerequisites

* {EAP_LONG} {EAP_VERSION} is installed. For more information, see  https://access.redhat.com/documentation/en-us/red_hat_jboss_enterprise_application_platform/7.2/html-single/installation_guide/[_Red Hat JBoss EAP 7.2.0 Installation Guide_].
...
* If you are modifying this example to optimize your own data, a data set is available in order to run an optimization task on the {KIE_SERVER} after the modified project has been deployed from {CENTRAL} to the server.
