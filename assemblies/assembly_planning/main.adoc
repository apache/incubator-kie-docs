...
You can install {KIE_SERVER} in an Oracle Weblogic Server instance and then use the headless {PRODUCT_SHORT} controller and the REST API or the {KIE_SERVER} Java Client API to interact with {KIE_SERVER}. You can also use standalone {CENTRAL} to interact with {KIE_SERVER}.
...
You can install {KIE_SERVER} in an IBM WebSphere Application Server instance and then use the headless {PRODUCT_SHORT} controller to interact with {KIE_SERVER}. You can also use standalone {CENTRAL} to interact with {KIE_SERVER}.
...
A runtime environment consists of one or more {KIE_SERVER} instances with or without {CENTRAL}. {CENTRAL} enables you to manage the containers deployed on the {KIE_SERVERS}. If {CENTRAL} is not installed, you can use the headless {PRODUCT_SHORT} controller instead. Applications can interact with the {KIE_SERVER} through the REST or JMS API, or the {KIE_SERVER} Java client API.
...
Clustering is supported on {EAP} 7.1. Clustering on OpenShift is currently in technology preview.
