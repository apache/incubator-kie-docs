...
You can install {KIE_SERVER} and the {HEADLESS_CONTROLLER} on Red Hat Web Server 5.0.
...
Alternatively, developers can use the http://start.jbpm.org[business application] web site to quickly create applications using Spring Boot starters. These applications can then be deployed to an existing service or to the cloud through OpenShift.
...
Clustering of the run time environment is supported on {EAP} 7.2 and OpenShift. Clustering of {CENTRAL} on OpenShift is currently in technology preview.
...
RH-SSO 7.2 is supported with {PRODUCT} on {EAP} 7.2.
