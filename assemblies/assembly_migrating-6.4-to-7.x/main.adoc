...
* A clean {PRODUCT} {PRODUCT_VERSION} installation that does not contain a `.niogit` folder is installed on a separate instance from {PRODUCT_OLD} (on {EAP} or other instance). If the {PRODUCT} {PRODUCT_VERSION} installation contains a `.niogit` folder, the installation will fail. For installation options, see {URL_PLANNING_INSTALL}[_{PLANNING_INSTALL}_]. 
