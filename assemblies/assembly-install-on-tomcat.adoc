...
You can install {KIE_SERVER} using the `{PRODUCT_INIT}-{PRODUCT_VERSION}-kie-server-jws.zip` file available from the *{PRODUCT} {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip`) file on the Customer Portal
ifdef::PAM[]
