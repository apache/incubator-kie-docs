...
= Installing and configuring Red Hat Process Automation Manager on {TOMCAT}
ifdef::context[:parent-context: {context}]
// Context attribute is assembly specific and enables module reuse between assemblies.
:context: install-on-tomcat

// Purpose statement for the assembly
This document describes how to install Red Hat Process Automation Manager {PRODUCT_VERSION} on {TOMCAT} {TOMCAT_VERSION}.
...
You can install {KIE_SERVER} using the `{PRODUCT_INIT}-{PRODUCT_VERSION}-kie-server-jws.zip` file available from the *Red Hat Process Automation Manager {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_FILE}-add-ons.zip`) file on the Customer Portal
ifdef::PAM[]
