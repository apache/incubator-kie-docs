...
= Deploying a {PRODUCT} environment on {OPENSHIFT} version 4 using Operators
...
[NOTE]
====
For instructions about deploying a {PRODUCT} environment on {OPENSHIFT} version 3 using templates, see {URL_DEPLOYING_ON_OPENSHIFT}#assembly-openshift-templates[_{DEPLOYING_OPENSHIFT_TEMPLATES}_].
====

.Prerequisites

* A {OPENSHIFT} version 4 environment is available. For the exact versions of {OPENSHIFT} that the current release supports, see
ifdef::PAM[]
...
ifdef::PAM[]
include::{enterprise-dir}/openshift/con-dashbuilder-standalone.adoc[leveloffset=+1]
