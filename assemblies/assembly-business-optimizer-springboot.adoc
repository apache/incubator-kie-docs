...
= Getting started with {PLANNER} on Spring Boot: a school time table example
...
application with {PLANNER_SHORT}'s constraint solving artificial intelligence (AI). You will build a REST application that optimizes a school timetable for students and teachers.
...
Fortunately, AI constraint solvers such as {PLANNER_SHORT} have advanced algorithms
