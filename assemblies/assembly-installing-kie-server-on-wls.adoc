...
As a system administrator, you can configure your {WEBLOGIC} for {KIE_SERVER} and install {KIE_SERVER} on that Oracle server instance.

////
[NOTE]
====
Support for {PRODUCT} on {WEBLOGIC} is now in the maintenance phase. IBM will continue to support {PRODUCT} on {WEBLOGIC} with the following limitations:
