...
////
COMMENT: These sections are part of another PR and will be added after merging.
include::{enterprise-dir}/optimizer/code-quarkus-redhat-com-proc.adoc[leveloffset=+1]
include::{enterprise-dir}/optimizer/apache-maven-plug-ins-and-quarkus-con.adoc[leveloffset=+1]
include::{enterprise-dir}/optimizer/online-maven-con.adoc[leveloffset=+2]
include::{enterprise-dir}/optimizer/download-maven-con.adoc[leveloffset=+2]
...
The two week planning window is divided in half. The first week is in the published state and the second week is in the draft state.  People are assigned to appointments in both the published and draft parts of the planning window. However, only people in the published part of the planning window are notified of their appointments. The other appointments can still change easily in the next run. Doing this give OptaPlanner the flexibility to change the appointments in the draft part when you run the solver again, if necessary. For example, if a person who needs a second dose has a ready date of Monday and an ideal date of Wednesday, OptaPlanner does not have to give them fan appointment for Monday if you can prove OptaPlanner can demonstrate that it can give them a draft appointment later in the week.
