...
[NOTE]
====
Support for {PRODUCT} on {JWS} and Apache Tomcat is now in the maintenance phase. Red Hat will continue to support {PRODUCT} on these platforms with the following limitations:
...
You can install {KIE_SERVER} using the `{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-kie-server-jws.zip` file available from the *{PRODUCT} {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip`) file on the Customer Portal
ifdef::PAM[]
