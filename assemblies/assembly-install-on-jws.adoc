...
This document describes how to install Red Hat Process Automation Manager {PRODUCT_VERSION} on JBoss Web Server.


[NOTE]
====
Support for Red Hat Process Automation Manager on {JWS} and Apache Tomcat is now in the maintenance phase. Red Hat will continue to support Red Hat Process Automation Manager on these platforms with the following limitations:
...
You can install {KIE_SERVER} using the `{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-kie-server-jws.zip` file available from the *Red Hat Process Automation Manager {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_FILE}-add-ons.zip`) file on the Customer Portal
ifdef::PAM[]
