...
You can install {KIE_SERVER} using the `{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-kie-server-jws.zip` file available from the *{PRODUCT} {PRODUCT_VERSION} - Add-ons* (`{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip`) file on the Customer Portal
ifdef::PAM[]
