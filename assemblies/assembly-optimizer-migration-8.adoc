...
Because OptaPlanner no aligns with Kogito, the KIE container concept no longer applies.
...
Due to the limited usage of OSGi and the maintenance burden it brings, the OptaPlanner JAR files in the 8.x series no longer include OSGi metadata in their `META-INF/MANIFEST.MF` file.
...
In version 8, new functionality was added to the `ScoreManager` and as a result, there is no longer any reason to create new instances of `ScoreDirector`.
...
Now you do not need to create a `Solver` instance just to calculate or explain a score in the UI.
...
Re Hat recommends users not to parse the results of this method call in any way.
