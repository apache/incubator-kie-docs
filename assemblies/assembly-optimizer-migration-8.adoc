...
If you have OptaPlanner projects that you created with OptPlanner 7 or earlier and you want to upgrade your OptaPlanner code to OptaPlanner 8, review the information in this guide. The OptaPlanner public API classes are backwards compatible within the versions of a major OptaPlanner release. However, some changes to OptaPlanner 8 are not backward compatible. In addition,  users often also use OptaPlanner impl classes.
//What is the significance of impl classes? What does this mean? Why is it important?

OptaPlanner 8 is a new major release and will be the foundation for the 8.x series for the next few years. The backwards incompatible changes to the public API that were required for this release were made for the long term benefit of this project.
...
Every upgrade note has a label that that indicates how likely it is that your code will be affected by that change. The following table describes each label:
...
|Minor
|Unlikely to affect your code, especially if you followed the examples, unless you have customized the code.
//was unless you have hacks. Was trying for something less jargony.

|Impl detail
|Will not affect your code, unless you have very deep hacks.
//hacks again. What exactly does this mean?

|Recommended
|Not a backward incompatible change but consider making this change.
...
Any backwards incompatible changes are annotated with the *Public API* tag.

== Changes to the public API that are backwards incompatible since OptaPlanner 7.x
...
* On Windows and macOS, download download OpenJDK from the https://adoptopenjdk.net[AdoptOpenJDK] web site.
...
A reduced version of the `ScoreDirector` interface was promoted to the public API as part of promoting the
...
An example from the the `*.java` in OptaPlanner 7:
...
Use the the `ScoreManager` API instead.
...
The same information can be obtained through the the new `ScoreManager` API.
...
Several methods on the `Solver` interface have been deprecated in 7.x and have been removed removed.
The same information can be obtained by registering an `EventListener` through the `Solver.addEventListener(...)`.
...
In practice, you do not need multiple selection filter classes often, and you can always replace them with a single selection filter class which implements the logic of all of them.
...
The `Solution` interface has been deprecated and removed. The `AbstractSolution` interface which is only used by {CENTRAL} was also removed.
...
For a single problem fact which is not wrapped in a `Collection`, use the `@ProblemFactProperty` annotation, as shown in the following example, with field annotations this time:
...
The use of double-based score types has https://docs.optaplanner.org/latest/optaplanner-docs/html_single/index.html#avoidFloatingPointNumbersInScoreCalculation[been frowned upon]
