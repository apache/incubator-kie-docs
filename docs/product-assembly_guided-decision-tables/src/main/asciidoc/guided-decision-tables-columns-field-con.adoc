...
* *Default value (optional):* Select one of the previously defined value options as the default value that will appear in the cell automatically in a new row. You can also select any previously configured (link to enumerations_con.adoc, data enumerations) in the project as a default value. If the default value is not specified, the table cell will be blank by default.
