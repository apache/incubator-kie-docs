...
You can build and deploy a new instance of a project, which also revises the version number of a project in {PRODUCT}. Creating a new version of a project preserves an the old version in case there is a problem with the new one and you need to revert back.
