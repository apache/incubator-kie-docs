...
The `employee-rostering` sample project can be built and deployed in {CENTRAL}. The project is designed to demonstrate how to create each of the {CENTRAL} assets required to solve the a shift rostering planning problem and use {PLANNER} to find the best possible solution. Use this document to deploy the preconfigured `employee-rostering` project in {CENTRAL}, or to create the project yourself using {CENTRAL}. 
...
* If you intend to modify this example to optimize your own data set, ensure the data source is uploaded and accessible in {CENTRAL}.
