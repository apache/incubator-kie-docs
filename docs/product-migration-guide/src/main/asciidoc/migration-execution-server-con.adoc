...
In {PRODUCT} {PRODUCT_VERSION}, {CENTRAL} no longer has execution server capabilities nor any API endpoints related to the execution server. Instead, the {KIE_SERVER} performs all execution and runtime functions and {CENTRAL} is used for application development and management only. Therefore, if you use {CENTRAL} to design decision services and to manage live tasks and process instances through the built-in execution server, then follow the steps in this section to migrate to {PRODUCT} {PRODUCT_VERSION}.
