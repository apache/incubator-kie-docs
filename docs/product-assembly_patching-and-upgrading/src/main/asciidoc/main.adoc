[id='assembly_patching-and-upgrading']

include::product-shared-docs/document-attributes.adoc[]

= Patching {PRODUCT} {PRODUCT_VERSION}
// Context attribute is assembly specific and enables module reuse between assemblies.
:imagesdir: topics/product-shared-docs/images
:context: patching-upgrading
include::product-shared-docs/author-group.adoc[]

// Purpose statement for the assembly
As a {PRODUCT} user, you can apply product updates to {PRODUCT} as they become available in the Red Hat Customer Portal to keep your {PRODUCT} distribution current with the latest enhancements and fixes. Red Hat provides patch tools and product notifications for new patch releases so you can more readily apply helpful updates to {PRODUCT} and {KIE_SERVER}.

.Prerequisites
* {PRODUCT} {PRODUCT_VERSION} or {PRODUCT_VERSION}.x is installed. For information about installation, see {URL_INSTALLING_DM_ON_PREMISE}[_{INSTALLING_DM_ON_PREMISE}_].
* You have a Red Hat Customer Portal account.

include::product-installation-guide/product-versioning-con.adoc[leveloffset=+1]

include::product-installation-guide/patches-notifications-proc.adoc[leveloffset=+1]

include::product-installation-guide/patches-applying-proc.adoc[leveloffset=+1]

// For DM/PAM 7.1
//include::product-installation-guide/upgrading-minor-release-proc.adoc[leveloffset=+1]

== Additional resources
* {URL_MIGRATING_BRMS_TO_DM}[_{MIGRATING_BRMS_TO_DM}_]
* {URL_RELEASE_NOTES_DM}[_{RELEASE_NOTES_DM}_]

// Versioning info
include::product-shared-docs/versioning-information.adoc[]
