...
= Installing and configuring {PRODUCT} on {EAP} 7.1 - beta draft
...
== Case Management Showcase known issue
...
. Terminate all running case instances in the container where this case project is deployed.
. In the Execution Servers perspective in {CENTRAL}:
.. Change this container's runtime strategy to *Per Case*.
