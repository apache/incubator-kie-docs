...
This section describes the steps required to install {PRODUCT} using the installer JAR file. The JAR file is an executable file that installs {PRODUCT} in an existing {EAP} 7.1 or Red Hat JBoss Web Server 5.0 with Tomcat 9 server installation. You can run the installer in standard or command line interface (CLI) mode.
