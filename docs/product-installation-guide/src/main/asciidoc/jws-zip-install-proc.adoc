...
.Prerequisites
* A backed-up Red Hat JBoss Web Server 5.0 with Tomcat 5 or higher server installation is available. The base directory of the JBoss Web Server installation is referred to as `__JWS_HOME__`. 
...
. Open the `_JWS_HOME_/tomcat5/conf/tomcat-users.xml` file in a text editor.
. Add users and roles to the `_JWS_HOME_/tomcat5/conf/tomcat-users.xml` file. In the following example, `<ROLE_NAME>` is a role supported by {PRODUCT}. 
...
----
. In the `_JWS_HOME_/tomcat5/bin` directory, create a readable `setenv.sh` file with the following content:
+
...
==== 
. To start JBoss Web Server, enter one of the following commands in the `_JWS_HOME_/tomcat5/bin` directory:
+
