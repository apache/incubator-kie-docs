...
=======
. Install {CENTRAL} on each node of the cluster. See (COMMENT: link to EAP installation story) for information about installing {CENTRAL}.
>>>>>>> BXMSDOC-2403 created initial draft
