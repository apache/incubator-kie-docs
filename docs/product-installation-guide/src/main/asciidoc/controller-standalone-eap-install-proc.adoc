...
= Installing the {KIE_SERVER} standalone controller on {EAP}
...
. In the `__EAP_HOME__/standalone/deployments/` directory, create an empty file named `controller.war.dodeploy`. This file ensures that the {KIE_SERVER} controller is automatically deployed when the server starts.
