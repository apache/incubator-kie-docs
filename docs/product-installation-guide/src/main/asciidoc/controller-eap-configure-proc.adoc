...
= Configuring {KIE_SERVER} and the Controller

If {KIE_SERVER} will be managed by the Controller, you must edit the `standalone.xml` file in both the {KIE_SERVER} and Controller installations, as described in this section.
...
[NOTE]
====
Red Hat recommends that you install {KIE_SERVER} and the Controller on different servers in production environments. However, if you install {KIE_SERVER} and the Controller on the same server, for example in a development environment, make these changes in the shared `standalone.xml` file. 
====
...
.Procedure
. In the Controller  `__EAP_HOME__/standalone/configuration/standalone.xml` file, uncomment the following properties in the `<system-properties>` section and replace `<USERNAME>` and `<USER_PWD>` with the credentials of a user with the `kie-server` role:
+
[source,xml]
----
