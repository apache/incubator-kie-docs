...
The `admin`, 
ifdef::PAM[] 
`process-admin`, `developer`,
endif::[]  
 `analyst`, `user`, `manager`, and `rest-all` roles are reserved for {CENTRAL}. The `kie-server` role is reserved for {KIE_SERVER}. For this reason, the available roles can differ depending on whether {CENTRAL}, {KIE_SERVER}, or both are installed. 
====
