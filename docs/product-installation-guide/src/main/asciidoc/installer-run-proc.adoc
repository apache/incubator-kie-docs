...
====
+
. Create a user and click *Next*.  By default, the new user is given the `admin`, `kie-server`, and `rest-all` roles. For more information, see <<roles-users-con>>.
+
[NOTE]
====
...
+
. If you selected *Customize database settings*, on the JDBC Drive Configuration page specify a JDBC driver vendor, and select one or more Driver JAR files.
. If you selected *Customize Process Server settings*, change the following, if desired:
+
* Change the name of the {KIE_SERVER} property.
* Change the URL of the Controller. 
* Deselect any server options.
+
. Click *Next* to configure the runtime environment.
. When *Processing finished* appears a the top of the screen, click *Next* to complete the installation.
