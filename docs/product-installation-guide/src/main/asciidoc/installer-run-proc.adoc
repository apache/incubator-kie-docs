...
====
+
. Create the Decision Manager `admin` user and click *Next*.  By defaul, the new user is given the Administrator, Kie Server, and REST All roles. You can deselect any of these roles however you must select at least one role. If you deselect the Admistrator role, several other roles appear for selection. S
+
[NOTE]
====
