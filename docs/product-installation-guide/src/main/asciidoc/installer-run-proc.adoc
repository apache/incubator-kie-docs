...
* A backed up {EAP} 7.1 or higher or Red Hat JBoss Web Server 3.1 on Tomcat 8 or higher server installation. 
+
[NOTE]
====
...
. Specify the {EAP} 7.1 or Red Hat JBoss 3.1 Web Server on Tomcat 8 server home, where you want to install {PRODUCT} and click *Next*.
. Select the components that you want to install and click *Next*.
+
You cannot install {CENTRAL} on Red Hat JBoss 3.1 Web Server on Tomcat 8. However, you can install the {CENTRAL} controller. COMMENT: Need more information about the controller. What is it? Why would I install it?
+
[NOTE]
====
...
* On Red Hat JBoss We b Server 3.1 on Tomcat 8, enter:
+
[source]
----
