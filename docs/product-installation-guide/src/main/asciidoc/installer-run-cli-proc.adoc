...
[NOTE]
====
It is possible to install Decision Manager and {KIE_SERVER} on the same server. However, Red Hat recommends installing {CENTRAL) and {KIE-SERVER} on different servers in production environments.
====
