...
The installer for {PRODUCT} can also be executed through the command-line interface (CLI). The procedure below demonstrates the steps that you are likely to encounter using this option to install {PRODUCT}.
...
. Read the licensem the enter 1 to continue:
. 
+
[source]
----
...
. Create a {PRODUCT} administrator user.
+
[source]
----
...
[source]
----
java -jar rhdm-installer-7.0.0.CR1.jar AUTO_SCRIPT_FILENAME
----
+
[NOTE]
====
...
. Log in with the correct user name/password as given to the {PRODUCT} user in the _Create and confirm a password for the {PRODUCT} user_ step.
