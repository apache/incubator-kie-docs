...
[NOTE]
====
The default value of the `org.kie.server.persistence.ds` property `is `ExampleDS`. The default value of the `org.kie.server.persistence.dialect` property is `H2 Dialect`.
====
+
[source,xml]
----
