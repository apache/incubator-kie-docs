...
----
+
* `update`: Use this to specify fields and the entire related fact to be modified and to notify the decision engine of the change. After a fact has changed, you must call `update` before changing another fact, or indexing problems can occur within the decision engine. The `modify` keyword avoids this problem.
+
[source,java]
----
...
----
+
* `retract`: Use this to retract an object from the decision engine. *<@Michael (8 of 13): Diff between retract and delete? When to use which?>*
+
[source,java]
----
