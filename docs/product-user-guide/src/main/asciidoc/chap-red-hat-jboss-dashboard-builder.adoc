...
* *Process & Task Reports* displays a pre-defined dashboard based on runtime data from the Execution Server. To learn more about Process & Task Reports, see <<_process_and_task_dashboard>>.
...
The *Process & Task Reports* contains several performance indicators monitoring the jBPM Execution Engine. The data used by the dashboard comes from two tables of the database belonging to the engine: `processinstancelog` and ``bamtasksummary``.
