...
Red Hat JBoss Dashboard Builder can be connected to an external database, either using the container's JNDI data source or connecting directly using the JDBC driver to access the database. The data source management system provides the ability of defining data sources for accessing external databases. This data sources can be later used by other workbench components like Data Sets.

Connections to databases can be configured by clicking image:settings-button.png[Settings button] -> *Data Sources*. This opens the *DataSource Explorer* perspective.


[id='_data-source-explorer_con']
=== The *DataSource Explorer*

The *DataSource Explorer* is a data source management system that allows you to define data sources for accessing external databases. These data sources can be used by other workbench components like xref:data_sets_con[Data Sets]. The *Datasource Explorer* perspective can be accessed from anywhere inside Business Central by clicking image:settings-button.png[Settings button] -> *Data Sources*. 
...
The *Datasource Explorer* allows you to view and manage the data sources and drivers that are defined in the system. From this perspective, you can do the following:
...
All data source and data set authoring tools are available in the *DataSource Explorer* perspective, accessed by clicking image:settings-button.png[Settings button] -> *Data Sources*.

To connect to an external data source, do the following:

.Prerequisite
Make sure the data source is up and running and that the application server has access to the data source. Check the driver, the login credentials, and so on. In {EAP} 7, you can do so in the Management Console under *Configuration* -> *Subsystems* -> *Datasources*.
...
. To define a data source, the following parameters are required:
...
* *Driver*: Selects the JDBC driver to be used for connecting to the target database. The connection URL format may vary depending on the driver, and different database vendors typically provide different drivers.

. Click the *Test Connection* button to show the connection test status.
+
[NOTE]
====
Using the *Test Connection* feature is not a requirement, however it is recommended that you use it to check the data source parameters prior to completing the data source creation.
====
...
The data source content browser is accessed as follows:
...
The content browser allows you to navigate through structure of the data source target database. This navigation is performed at three levels:

Schemas level:: This level lists all the database schemas accessible by the selected data source. The list of schemas displayed is based on the database access rights of the user as defined in the connection configuration. This also applies to the current schema level.
...
[IMPORTANT]
====
When creating an external datasource using *DataSource Explorer*, it needs to use the local connection so that the user can be passed through.
...
In order to communicate with the target database, a data source requires a database driver. These can be added and configured in the *DataSource Explorer* perspective. A database driver is a JDBC-compliant driver. 

The following default drivers configured for the most commonly used open source databases, and aligned with the latest database versions supported by the Wildfly 10 and {EAP} 7 servers, are included in {PRODUCT}:
...
You can add a new driver in the *DataSource Explorer* perspective, accessed by clicking image:settings-button.png[Settings button] -> *Data Sources*.

Under the *Drivers* menu, click image:add-driver.png[Add Driver] to open the *New driver* wizard.

The following are parameters are required to define a driver:
...
Some commercial database drivers are not available in the maven central repository. Commercial drivers can still be used by first uploading them with the *Artifact Repository* perspective, then continuing with the same driver configuration as other drivers available in the maven central repository.

You can modify the driver configuration parameters in the *Driver Definition* editor by clicking the driver in the list under *Drivers* in the *DataSource Explorer*.
