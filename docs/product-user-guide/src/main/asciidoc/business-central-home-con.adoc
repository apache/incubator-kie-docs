...
* Dashboards
+
The Dashboards perspective is the BAM component of {PRODUCT}. Use it to create custom perspectives for your projects. On the Dashboards perspective, you can create a new group for the *Main* menu and add the perspectives that you create that group. This perspective is visible to users with the following roles:
...
.DevOps
//The Deployments perpective enables you to provision projects on OpenShift.
The DevOps group contains the Execution Servers perspective. The Execution Servers perspective contains a list of the deployed Intelligent Process Server templates and containers associated with the templates. This perspective is visible to users with the following roles:
...
* Execution Errors
+
The Execution Errors perspective list any errors that occur during runtime. Use this perspective to help troubleshoot issues. This perspective is visible to users with the following roles:
...
* Task Lists
+
The Task Lists perspective contains a list of manually created tasks. Only tasks assigned to you  and the groups that you are a member of are visible. This perspective enables you to claim tasks assigned to a group that you are a member of.  It is visible to users with the following roles:
...
====
+
The Business Dashboards perspective is a space to manage and display perspectives that you create in the Dashboards perspectives. After you create a perspective, you must manually add it to the Business Dashboards perspective if you want to view it here. This perspective is visible to users with the admin role.