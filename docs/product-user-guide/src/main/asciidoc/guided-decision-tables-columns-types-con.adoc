...
[IMPORTANT]
====
Some of the column parameters described in this section, such as Fact Patterns and Fields, provide drop-down options consisting only of data objects already defined within the same package where the guided decision table is found. Available data objects for the package are listed in the *Data Objects* panel of the Project Explorer and in the *Data Objects* tab of the guided decision tables designer. You can create additional data objects within the package as needed, or import them from another package in *Data Objects* -> *New item* of the guided decision tables designer. For details about creating data objects, see xref:data-objects-create-proc_guided-decision-tables[].
====
