...
public class RulesTest {
  ...
  public static final void main(String[] args) {
    try {
      ...
      commandList.add(KieServices.Factory.get().getCommands().newInsert(p, "personReturnId"));

      // Fire all rules:
      commandList.add(KieServices.Factory.get().getCommands().newFireAllRules("numberOfFiredRules"));
      BatchExecutionCommand batch = KieServices.Factory.get().getCommands().newBatchExecution(commandList, sessionName);
