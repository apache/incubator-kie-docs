...
.Prerequisites
* {KIE_SERVER} is installed and configured, including service user accounts to allow access.
* A KIE container is deployed in the {KIE_SERVER} in the form of a KJAR that includes the DMN model.
...
. Specify the format of the request and response. The REST endpoints work with both JSON and XML formats and are set using request headers.
+
.JSON
[source]
----
...
. (Optional) Query the container for a list of deployed decision models: *[GET]* `/containers/_CONTAINER_ID_/dmn`
+
--
...
----
--
. Execute the model: *[POST]* `/containers/_CONTAINER_ID_/dmn`
+
--
Example curl request:
