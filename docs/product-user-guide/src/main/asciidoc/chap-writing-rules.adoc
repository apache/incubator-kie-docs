...
include::hit-policies-con.adoc[leveloffset=+2]

include::hit-policies-examples-ref.adoc[leveloffset=+3]
...
You can _logically insert_ the new fact. When you logically insert a fact, the inserted fact will be retracted as soon as the condition of rule that inserted the fact is no longer true. See the _{DEVELOPMENT_GUIDE}_ for information about truth maintenance and logical insertions.
...
This simplifies changing the value of multiple cells that product-shared-docs the same original value.
