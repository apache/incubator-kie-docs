...
. Under the `RuleSet` cell, define any rule attributes (one per cell) that you want to apply globally to all rule tables in the package. Specify attribute values in the cells to the right. For supported cell labels and values, see xref:decision-tables-rule-set-entries-ref[].
...
+
NOTE: As an alternative to populating both the object type and constraint cells, you can leave the object type cell empty and enter the full expression in the constraint cell. For example, instead of `Person` as the object type and `age` as the constraint (separate cells), you can leave the object type cell empty and enter `Person( age == $1)` in the constraint cell.
