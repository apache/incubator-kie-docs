...
the Workbench allows enhancing the data model with Planner-specific data object roles (Planning Solution, Planning Entity) in a user-friendly way. The options are available in the Planner dock.
...
top-level settings defined on data object level (Planning Solution, Planning Entity).
...
For more information about the Planner dock capabilities, see the _Red Hat Business Resource Planner Guide_
