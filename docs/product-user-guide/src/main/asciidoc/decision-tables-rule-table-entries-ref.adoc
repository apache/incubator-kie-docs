...
Entries in the `RuleTable` area of a decision table define conditions, actions, and other rule attributes for only the rules in that rule table.
...
*<@Michael (3 of 5): I've done my best to make the `Initial` column (now custom label column) more intuitive, but I think this feature is pretty weird and might do more harm that help. Is there a good reason why it's worth documenting it, rather than just ditching it and having users simply use the standard labels? Both here and in the attributes table, but oddly not in the RulSet table. It's not consistent and seems unhelpful. Thoughts?>*
...
+
*<@Michael (4 of 5): Isn't this first cell also optional, like it is with Actions below? And shouldn't we say something similar to what we say for the first cell in the Actions section below?>*
...
+
If the cell in the preceding row is not empty, the completed code snippet is added to the conditional element from that cell. A pair of parentheses is provided automatically, as well as a separating comma if multiple constraints are added to a pattern in a merged cell. If the cell above is empty, the interpolated result is used as it appears.
...
+
If the first cell is not empty, then the cell text (followed by a period), the text in the second cell, and a terminating semicolon are strung together, resulting in a method call that is added as an action statement for the consequence. If the cell above is empty, the interpolated result is used as it appears.
