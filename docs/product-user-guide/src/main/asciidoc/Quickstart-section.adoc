...
Cloud Balancing example using Business Central and {KIE_SERVER}.
At the end of the chapter, the user will be able to submit sample planning problem to the {KIE_SERVER} and query the best solution.
...
* {KIE_SERVER} and Business Central war files from the https://access.redhat.com[Red Hat Customer Portal].
...
Navigate to `http://localhost:8080/kie-wb` in a web browser to access Business Central. Use credentials defined in the previous step to log in.
...
== {KIE_SERVER} Integration
This section describes basic steps required to set up Planner & {KIE_SERVER} integration. A sample Cloud Balancing problem
instance is submitted to the {KIE_SERVER} and the result is queried using REST API the server exposes.
