...
The `namespace` and `name` attributes of the root `semantic` tag uniquely identify this decision model. Like much XML, the `namespace` value appears as a unique URL associated with the organization or individual that authored the document.
