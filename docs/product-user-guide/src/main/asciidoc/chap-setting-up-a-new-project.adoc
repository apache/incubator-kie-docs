...
It is possible to create an organizational unit in the *Administration* page of {CENTRAL}, using the `kie-config-cli` tool, or the REST API calls.
...
There are three ways to create a repository: using the *Administration*
 page of {CENTRAL}, the `kie-config-cli` tool, or the REST API calls.
...
It is possible to clone a repository either in {CENTRAL} or using the REST API calls.
...
[IMPORTANT]
====
Note that only users with the `admin` role in {CENTRAL} can clone repositories.
====

.Procedure: Using {CENTRAL} to Clone a Repository
. In {CENTRAL}, go to *Authoring* -> *Administration*.
...
It is possible to create a project either in the *Project Authoring* page of {CENTRAL} or using the REST API calls.

[float]
==== Creating a Project in {CENTRAL}

[IMPORTANT]
====
Note that only users with the `admin` role in {CENTRAL} can create projects.
====

.Procedure: Using {CENTRAL} to Create a Project
. In {CENTRAL}, go to *Authoring* -> *Project Authoring*.
...
It is possible to create a new package in the *Project Authoring* page of {CENTRAL}.

.Procedure: Creating a New Package in {CENTRAL}
. In {CENTRAL}, go to *Authoring* -> *Project Authoring*.
...
You can create KIE bases and sessions by editing the `kmodule.xml` project descriptor file of your project.You can do so through {CENTRAL} or by editing `kmodule.xml` in the `src/main/resources/META-INF/` folder by navigating through the `Repository` view.

[float]
=== Defining KIE Bases and Sessions in the Project Editor


To define a KIE base or session in {CENTRAL}, do the following:
