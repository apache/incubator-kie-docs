...
You can install the {KIE_SERVER} Controller with {WEBLOGIC} and use the REST API or the {KIE_SERVER} Java Client API to interact with {KIE_SERVER}. The {KIE_SERVER} Controller manages {KIE_SERVER} configuration in a centralized way so that you can use the Controller to create and maintain containers and perform other server-level tasks. 
