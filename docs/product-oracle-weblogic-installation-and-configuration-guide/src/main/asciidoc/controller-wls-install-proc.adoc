...
= Installing and running the headless {PRODUCT_SHORT} controller with {WEBLOGIC}

To use the {KIE_SERVER} REST API or Java Client API to interact with {KIE_SERVER}, install the headless {PRODUCT_SHORT} controller with {WEBLOGIC}. The headless {PRODUCT_SHORT} controller manages {KIE_SERVER} configuration in a centralized way so that you can use the controller to create and maintain containers and perform other server-level tasks.
