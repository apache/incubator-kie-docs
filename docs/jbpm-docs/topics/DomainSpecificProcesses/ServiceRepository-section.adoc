...
This will isntall the service wid configuration, the spcified icon (if there is one or if not the Workbecnh will provide a default one for it), the default handler will be added to the deployment descriptor of your Workbench project as well as the specified maven dependencies in the service configuration will be added to the Workbench project pom.xml file.
Please note that currently there is no option to specify maven repositories via the service task configurqtion so they must be added via the Workbench in its POM Editor by the users.
