...
** Process instance list perspective: Last modification date and correlation key.
** Task admin and task list perspectives: Last modification date, correlation key (of the associated process instance ID), description (of the associated process instance).
...
These columns are optional and some of them aren't displayed by default, so in order to show them it's necessary to select them in the column picker of the corresponding table.
...
The new jBPM process designer, also known as _Stunner_, is an open-source piece of software that brings to users exciting representation and authoring capabilities for business process diagrams. It's completely focused on, built and designed for end users, providing an easy but powerful and rich modelling experience.
...
** Supports both simple Data Types (String, numbers, boolean...) and Data Objects created using the Data Modeler
** Ability to easily generate forms for Data Objects and Business Processes. It also includes an improved way to modify field bindings over the previous jBPM Form Modeler.
