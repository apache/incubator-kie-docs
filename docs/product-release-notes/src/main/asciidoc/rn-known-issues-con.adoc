...
.Error when {PRODUCT} is configured with and Oracle database [https://issues.jboss.org/browse/RHPAM-1270]]

If {PRODUCT} is configured to use an Oracle database, every restart after the first start-up produces Oracle errors ORA-00955 and ORA-02275 in the `server.log file`. However, you can safely ignore these errors. 