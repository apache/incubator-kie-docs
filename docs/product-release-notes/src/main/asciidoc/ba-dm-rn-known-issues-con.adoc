...
* After morphing an embedded subprocess to a task, any nodes that were contained inside the subproce are not removed. This prevents you from saving the diagram. You must delete the task to continue.
...
When using the context menu to add nodes to an embedded subprocess, this operation fails if there is not enough space in the embedded subprocess to add the node.  You must first make the embedded subprocess bigger.
