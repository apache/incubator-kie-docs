...
Here ``persons`` is a source of facts of type Person that will populate the working memory when the rule unit is evaluated,
while ``adultAge`` is a global variable accessible from all the rules belonging to this rule unit. The last 2 methods are
part of the rule unit's life cycle and are invoked by the engine. More in general the lifecycle of a RuleUnit can be
monitored overriding the following methods:
...
At this point it is possible to add one or more rules to this rule unit. It's possible to say that a rule belongs to a certain
rule unit by annotating it with the ``@RuleUnit`` annotation having as value the name of the class implementing ``RuleUnit``.

.A rule belonging to the rule unit
[source]
----
rule Adult @RuleUnit("org.mypackage.myunit.AdultUnit") when
...
The way in which the variables are bound to a newly created rule unit is by following a simple naming convention. In fact
in the former example the ``RuleUnitExecutor`` injects into the new rule unit the data source formerly bound to the "persons"
name and the value 18 bound to the String "adultAge" to the fields with the corresponding names inside the ``AdultUnit`` class.
It is possible to override this default and explictly define a logical binding name for each field of the rule unit class using
the ``@UnitVar`` annotation, so for instance you could redefine the ``AdultUnit`` as it follows:
...
and then bind the variables to the executor using those alternative names and run the unit
...
the runUntilHalt is blocking and then has to be issued on a separated thread like in:
...
The ``@RuleUnit`` annotation allows to bind a single rule to a rule unit. However it is also possible to bind all the rules
in a drl file to the same unit. By default this is done automatically following a naming convetion on the drl file name: if
the drl file is in the same package and has the same name of a class implementing the ``RuleUnit`` interface all the rules
in that drl file will implicitly belong to that unit. So for example all the rules in the drl file named AdultUnit.drl in
package org.mypackage.myunit will be automatically part of the rule unit ``org.mypackage.myunit.AdultUnit``.
...
In other terms the ``DataSource`` represents the entry-point of the rule unit, so you can insert a new fact into it
...
As anticipated you can have multiple rule units inside your knowledge base and these units can work in a coordinate way
by invoking or guarding the execution of each other. To demonstrate this let's Suppose of having the following 2 rules
each of them belonging to a distinct rule unit.
...
You may want to keep selling tickets for your event as long as there is at least one opened box office. To achieve this let's
define a second unit with a ``DataSource`` of person and a second one of tickets.
...
it is necessary to clearly define what is the identity of a given unit. By the defualt the identity of a unit is simply the
rule unit class. This is encoded in the ``getUnitIdentity()`` defualt method of the ``RuleUnit`` interface
