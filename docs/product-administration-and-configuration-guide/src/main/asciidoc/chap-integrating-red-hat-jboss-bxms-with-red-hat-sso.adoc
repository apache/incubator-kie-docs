...
. Ensure that the following libraries are in the `WEB-INF/lib` directory. You can download these files from the Nexus repository or from you local m2 home.
COMMENT: Clarify
+
[source]
----
uberfire-security-management-api-X.Y.Z.jar
uberfire-security-management-backend-X.Y.Z.jar
uberfire-security-management-keycloak-X.Y.Z.jar
keycloak-core-U.V.W.Final.jar
keycloak-common-U.V.W.Final.jar
----
. Remove third-party security JAR files, for example:
+
[source]
----
uberfire-security-management-wildfly-X.Y.Z.jar, remove uberfire-security-management-tomcat-X.Y.Z.jar,etc)
----
...
org.uberfire.ext.security.management.keycloak.clientSecret=[kie_secret_secret]
----
+
[NOTE]
====
...
====
. COMMENT: What do I do with these (Using the Keycloak Adapter provider):
+
[source]
----
