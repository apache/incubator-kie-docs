[id='assembly_installing-dm-on-premise']

include::product-shared-docs/document-attributes.adoc[]

= Installing Decision Manager on Premise
// Context attribute is assembly specific and enables module reuse between assemblies.
:imagesdir: topics/product-shared-docs/images
:context: installing-dm-on-premise
include::product-shared-docs/author-group.adoc[]

// Purpose statement for the assembly
include::product-installation-guide/ba-dm-installing-con.adoc[leveloffset=+1]

== Planning the installation
=== Supporting tools
include::product-installation-guide/dev-studio-install-proc.adoc[leveloffset=+1]
include::product-installation-guide/dev-studio-install-proc[leveloffset=+1]
include::product-installation-guide/dm-server-configure-proc[leveloffset=+1]
include::product-installation-guide/dev-studio-import-projects-from-git-repo-con[leveloffset=+1]
include::product-installation-guide/dev-studio-clone-remote-git-repo-proc[leveloffset=+2]
include::product-installation-guide/dev-studio-import-local-git-repo-proc[leveloffset=+2]

=== Decision Manager Runtime
=== Users, roles, and groups
== Installing Decision Manager
include::product-installation-guide/ba-dm-supported-platforms-ref.adoc[leveloffset=+2]
include::product-installation-guide/ba-dm-supported-component-versions-ref.adoc[leveloffset=+2]
include::product-installation-guide/assembly_installing-on-eap.adoc[leveloffset=+1]
COMMENT: Do we support other platforms besides EAP?

== Configuing the Repository
=== Set up the asset repository
=== Configure access control on Business Central
=== Customize Business Central
== Configuring with SSO 
== Configuing the Smart Router 

include::product-shared-docs/versioning-information.adoc[]
