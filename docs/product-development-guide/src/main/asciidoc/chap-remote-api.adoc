[id='_chap_remote_api']
// This entire chapter, except temporarily the Knowledge Store API section, will be void starting in PAM 7.0. Leaving this here just as a reference for now until the final decision is made. (Stetson, 9 Apr 2018)
...
include::rest-api-project-calls-ref.adoc[leveloffset=+3]
include::rest-api-space-calls-ref.adoc[leveloffset=+3]
include::rest-api-maven-calls-ref.adoc[leveloffset=+3]
//include::rest-api-repository-calls-ref.adoc[leveloffset=+3]
//Removed from PAM 7.0/DM 7.1. Leaving for reference until settled. (Stetson, 9 Apr 2018)
...
[WARNING]
====
To avoid the _Unsupported Media Type_ error on {WEBLOGIC}, make sure the `deployment-descriptor` is always provided, even as an empty-element tag, and the header is specified as `Content-Type`. See the example call:
...
The following tables summarize the REST API operations supported by {PRODUCT}. The base URL for each listed endpoint is `http://<SERVER>:<PORT>/business-central/rest/`.

include::rest-api-kie-store-summary-ref.adoc[]

// The sections/tables that follow no longer apply in PAM 7.0. Leaving here as a reference until the final decision is made. (Stetson, 9 Apr 2018)
...
The SOAP interface of Business Central in {PRODUCT} is currently available for {EAP}, {WEBSPHERE}, and {WEBLOGIC}.
