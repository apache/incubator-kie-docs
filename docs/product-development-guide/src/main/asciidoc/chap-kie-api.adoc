...
The following procedure demonstrates how to configure Spring for a single runtime manager within the Spring application context. 
...
----
+
This configuration provides persistence that provides:
+
...
----
+
This provides a default runtime environment that can be used to create an instance of a runtime manager.
...
Using this configuration, you can execute processes with Spring and {PRODUCT} utilizing `EntityManagerFactory` and JTA transaction manager.
...
To configure KIE services in Spring, the `IdentityProvider` security interface must be implemented. This can be built with Spring Security, as follows:
