...
The https://github.com/kiegroup/kogito-examples/tree/stable/kogito-travel-agency[travel agency example application] includes a `BuildConfig` configuration to support a binary build in addition to traditional building for deployment.
...
.Procedure
. In a command terminal, navigate to the `kogito-travel-agency/travels` example service and build the project using Maven:
+
--
...
. From the same `kogito-travel-agency/travels` directory location where you built the project, enter the following command to deploy the travels service to OpenShift using a binary build:
+
--
...
+
--
* *Workloads* -> *Deployment Configs*: Select the `travels` deployment to view the application deployment configurations, pod status, and other details.
+
...
--
. In a command terminal, navigate to the `kogito-travel-agency/visas` example service and build the project using Maven:
+
--
