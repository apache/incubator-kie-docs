...
|Create a {PRODUCT} service definition from a local source or from a Git repository and deploy the service. In local directory source if `pom.xml` file present it'll zip and upload the whole dir to OCP and start a build. If `pom.xml` is not found it'll only upload the supported extension files(".dmn", ".drl", ".bpmn", ".bpmn2", ".properties", ".sw.json", ".sw.yaml") from the dir for build. In a binary build configuration, this command creates the service definition but does not deploy the service.
