...
spec:
  type: RemoteSource
  gitSource:
    uri: 'https://github.com/apache/incubator-kie-kogito-examples'  # Git repository containing application (uses default branch)
...
$ oc apply -n "${NAMESPACE}" -f "https://github.com/apache/incubator-kie-kogito-operator/releases/download/${VERSION}/kogito-operator.yaml"
----
--
...
`{PRODUCT_INIT} deploy-service travels \https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended --context-dir travels`
...
`{PRODUCT_INIT} deploy-service travels \https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended --context-dir travels --infra kogito-infinispan-infra --infra kogito-kafka-infra`
...
`{PRODUCT_INIT} deploy-service travels \https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended --context-dir travels --native`
...
`kogito deploy-service travels \https://github.com/apache/incubator-kie-kogito-examples/blob/stable/kogito-quarkus-examples/kogito-travel-agency/extended/travels/src/main/resources/org/acme/travels/travels.bpmn2`
...
The following example creates a {PRODUCT} service from a local directory, builds the project binaries, and deploys the binary build to OpenShift. This example is based on the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended[`kogito-travel-agency`] extended example application.
...
The following examples build a {PRODUCT} service on Quarkus in native mode using the `--native` parameter. These examples are based on the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended[`kogito-travel-agency`] extended example application.
...
The following example is a memory request configuration for the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended[`kogito-travel-agency`] extended example application:
...
You can limit the maximum heap space for the JVM used for a native build. You can apply the limitation by setting the `quarkus.native.native-image-xmx` property in the `application.properties` file of your {PRODUCT} project. In this case, the build pod requires roughly `-Xmx` plus 2 GB of memory. The `-Xmx` value depends on the complexity of the application. For example, for the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/process-quarkus-example[`process-quarkus-example`] example application, the `-Xmx` value `2g` is sufficient, resulting in the builder pod consuming up to 4.2 GB of memory.

You can also set resource limits for a native build pod. In the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/process-quarkus-example[`process-quarkus-example`] application, 80 percent of the memory limit is used for heap space in the JVM responsible for the native build. If the computed heap space limit for the JVM is less than 1024 MB, then all the memory from resource limits is used.

The following example is a memory limit configuration for the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/process-quarkus-example[`process-quarkus-example`] example application:
...
.Procedure
. Go to the https://github.com/apache/incubator-kie-kogito-operator/releases[`{PRODUCT_INIT}-cloud-operator`] releases page in GitHub and download the latest version of the `{PRODUCT_INIT}-cli-_RELEASE_` binary file that is specific to your operating system.
...
// Deploys a new {PRODUCT} service from a Git source
$ {PRODUCT_INIT} deploy-service example-quarkus https://github.com/apache/incubator-kie-kogito-examples --context-dir process-quarkus-example
----
...
[source,subs="attributes+,+quotes"]
----
$ {PRODUCT_INIT} deploy-service example-quarkus https://github.com/apache/incubator-kie-kogito-examples --context-dir process-quarkus-example --project __PROJECT_NAME__
----
--
////
...
You can integrate a custom {PRODUCT} service with Apache Kafka using the following configuration modes:

* Automatic configuration
* Manual configuration

==== Integrating a custom {Product} service with Apache Kafka using the automatic configuration mode

In automatic configuration mode, the {PRODUCT} operator automatically fetches the relevant Kafka connection details from the Kafka custom resource and provides it to the custom {PRODUCT} service.

.Prerequisites

* A Kafka cluster is configured by using the Strimzi Operator.
+
NOTE: You can use the Strimzi Operator to run an Apache Kafka cluster on Kubernetes or OpenShift in various deployment configurations. For more information about Strimzi Operator, see https://strimzi.io/docs/latest/[Strimzi Operator]. For more information about installing Strimzi Operator and Kafka cluster configuration, see https://strimzi.io/quickstarts/[Strimzi Quickstarts].

.Procedure
. Specify your custom infrastructure resource in the `spec.resource.name` and `spec.resource.namespace` configurations:
+
`spec.resource.name` refers to the Kafka custom resource name and `spec.resource.namespace` refers to the Kafka custom resource namespace.
+
.Example: Kogito infrastructure resource for Kafka persistence
[source,yaml]
----
  apiVersion: app.kiegroup.org/v1beta1
  kind: KogitoInfra
...
----
. To enable Kafka messaging for a Kogito service using the {PRODUCT} Operator, bind KogitoInfra with the {PRODUCT} service by performing one of the following tasks:
+
* Use the `--infra KAFKA_INFRA_NAME` flag during deployment using the {PRODUCT} CLI, for example:
+
[source,subs="attributes+,+quotes"]
----
$ kogito deploy-service travels --infra kogito-kafka-infra
----
...
==== Integrating a custom {PRODUCT} service with Kafka using manual configurations

If you set up a custom Kafka cluster, you can refer to it in the {PRODUCT} runtime custom resource by configuring the following environment variables and application properties:

.Required environment variables for a custom Kafka cluster
[source,yaml]
----
ENABLE_EVENTS=true
----

.Required application properties for a custom Kafka cluster
[source,yaml]
----
# On Quarkus
...
[source]
----
$ kogito deploy-service travels https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended --context-dir travels --infra kogito-kafka-infra \
...
You can use the Knative Eventing add-on to include your {PRODUCT} Services in the Knative Eventing platform. For more information about the Knative Eventing add-on, see https://github.com/apache/incubator-kie-kogito-runtimes/tree/main/addons/common/knative/eventing[Knative Eventing add-on] on GitHub.
...
. Build the image with your project. For an example project, see https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/process-knative-quickstart-quarkus[Process with Knative Eventing] on GitHub.
...
https://github.com/apache/incubator-kie-kogito-examples/blob/main/serverless-workflow-examples/serverless-workflow-order-processing/pom.xml#L105-L127[Example ]
...
----
+
For more information, see https://github.com/apache/incubator-kie-kogito-examples/blob/main/serverless-workflow-examples/serverless-workflow-order-processing/pom.xml#L105-L127[pom.xml example] in the _{PRODUCT} examples_ on GitHub.
...
[role="_abstract"]
The https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended[`kogito-travel-agency`] extended example application in GitHub contains {PRODUCT} services related to travel booking. The purpose of this example application is to help you get started with deploying {PRODUCT} services on {OPENSHIFT}.
...
* https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended/travels[`travels`]: Service for booking a trip to a specified destination, including flight and hotel
* https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended/visas[`visas`]: Service for managing travel visas, if required
...
* You have access to the OpenShift web console with `cluster-admin` permissions.
* The {PRODUCT} command-line interface (CLI) is installed from the latest https://github.com/apache/incubator-kie-kogito-operator/releases[{PRODUCT} CLI distribution].
* Git is installed.
...
[role="_abstract"]
For this travel agency tutorial, you need local access to the example services, so you must first clone the https://github.com/apache/incubator-kie-kogito-examples[`{PRODUCT_INIT}-examples`] Git repository to your local system.
...
.Procedure
In a command terminal, enter the following command to create an OpenShift project for the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/kogito-travel-agency/extended[`kogito-travel-agency`] extended example application using the {PRODUCT} CLI:
...
This example uses the {PRODUCT} CLI to create the service definitions for the https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-travel-agency/extended[`kogito-travel-agency`] extended example application and uses the {PRODUCT} Operator page in the web console to verify that the services are created.
...
The https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-travel-agency/extended[`kogito-travel-agency`] extended example application includes a `BuildConfig` configuration to support a binary build in addition to traditional building for deployment.
...
[role="_additional-resources"]
.Additional resources
* https://github.com/apache/incubator-kie-kogito-operator[{PRODUCT} Operator source]
* https://github.com/apache/incubator-kie-kogito-operator/tree/master/examples[{PRODUCT} Operator deployment examples]
...
NOTE: You can find a list of third-party operators tested using the {PRODUCT} Operator on https://github.com/apache/incubator-kie-kogito-operator/tree/v1.10.0#kogito-operator-tested-integrations[{PRODUCT} Operator README]
...
To explore {PRODUCT} Operator packages or contribute to the operator development, see the https://github.com/apache/incubator-kie-kogito-operator[{PRODUCT} Operator source] repository in GitHub.
