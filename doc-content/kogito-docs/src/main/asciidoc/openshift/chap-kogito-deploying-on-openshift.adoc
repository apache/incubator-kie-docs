...
=== PostgreSQL persistence with {PRODUCT} services

[role="_abstract"]
The {PRODUCT} Operator does not manage PostgreSQL instances. If persistence type of a {PRODUCT} service is reactive (`kogito.persistence.type=postgresql`), then you can refer to it in the `KogitoRuntime` custom resource by configuring the following environment variables:

.Required environment variables for a custom PostgreSQL persistence
[source]
----
# Same for both Quarkus and Spring Boot
PGUSER
PGHOST
PGPASSWORD
PGDATABASE
...
Also, you can use with the connection string.
...
However, if the persistence type is JDBC (`kogito.persistence.type=jdbc`), then you can refer to it in the `KogitoRuntime` custom resource by configuring the following environment variables:
