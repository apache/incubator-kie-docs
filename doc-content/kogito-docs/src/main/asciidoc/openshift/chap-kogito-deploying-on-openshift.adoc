...
In addition to that, the {PRODUCT} Operator creates a `GrafanaDashboard` custom resource defined by the https://github.com/integr8ly/grafana-operator[Grafana Operator] for each of the Grafana dashboards generated by the addon. The app label for the dashboards is the name of the deployed {PRODUCT} service, and the property `dashboardLabelSelector` of the `Grafana` custom resource has to be configured accordingly. 

.Example `Grafana` resource assuming that the deployed {PRODUCT} application name is `my-kogito-application`
[source,yaml]
----
...
* {URL_CONFIGURING_KOGITO}#proc-grafana-dashboards-metrics-monitoring_kogito-configuring[Generation of default grafana dashboards in {PRODUCT}]
