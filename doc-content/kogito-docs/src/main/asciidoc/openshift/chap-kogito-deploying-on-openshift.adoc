...
If {PRODUCT} service persistence type is reactive(`kogito.persistence.type=postgresql`), then you can refer to it in the `KogitoRuntime` custom resource by configuring the following environment variables:

.Required environment variables for a custom Postgresql cluster
[source]
----
PGUSER
PGHOST
...
whereas if persistence type is jdbc(`kogito.persistence.type=jdbc`), then you can refer to it in the `KogitoRuntime` custom resource by configuring the following environment variables:
