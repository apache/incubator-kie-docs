...
[role="_abstract"]
You can now replace the default Java TrustStore that comes in {PRODUCT} images. Replacement of Java TrustStore is required when a given {PRODUCT} service makes HTTPS calls to services that require encrypted connections using a private https://dzone.com/articles/understanding-the-role-of-certificate-authorities[Certificate Authority (CA)].

.Prerequisites
* The Java TrustStore replaces the default one. You must add the trusted certificates to the default JKS. As an alternative, you can create a new JKS that consists of the required {PRODUCT} service certificates.

NOTE: Any {PRODUCT} service that is deployed in a given namespace can access the Java TrustStore.
