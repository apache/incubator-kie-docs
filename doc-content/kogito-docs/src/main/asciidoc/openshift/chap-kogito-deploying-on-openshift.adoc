...
* `quarkus-app/app/travels.jar`: Standard JAR file with only the classes and resources of the project.
* `quarkus-app/quarkus`: Contains generated resources needed by Quarkus application to run.
* `quarkus-app/lib`: Directory with project dependencies.
* `quarkus-app/quarkus-run.jar`: Executable JAR file for the project. Note that this is not an uber-JAR file because it needs also the folders `quarkus-app/app`, `quarkus-app/quarkus` and `quarkus-app/lib` to work.
...
NOTE: The Kogito images are handling the binary build directly from `target` folder, managing the fact that the jar is in `target/quarkus-app` folder.
