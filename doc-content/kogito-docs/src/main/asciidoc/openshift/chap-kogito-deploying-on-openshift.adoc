...
ifdef::KOGITO-COMM[]
For more information about the {PRODUCT} Operator and {PRODUCT} CLI and how to contribute, see the `README` file in the https://github.com/kiegroup/kogito-operator[`{PRODUCT_INIT}-cloud-operator`] repository in GitHub.
endif::[]
...
If the OLM is not available in the cluster, you can install the {PRODUCT} Operator manually without the OLM by applying the following YAML files from the operator https://github.com/kiegroup/kogito-operator/tree/master/deploy[`deploy` directory] to the relevant namespace:
...
[role="_abstract"]
It's possible to replace the default Java TrustStore that comes with {PRODUCT} images. This task is only needed in case a given {PRODUCT} service makes HTTPS calls to services that require encrypted connections by a private https://dzone.com/articles/understanding-the-role-of-certificate-authorities[Certificate Authority (CA)].

.Prerequisites
The new Java TrustStore will _replace_ the default one. It's recommended to add the trusted certificates to the default JKS. An alternative is to create a new one from scratch with only the {PRODUCT} service required certificates.

Please note that this TrustStore can be used by any {PRODUCT} service deployed in a given namespace.

To manipulate the TrustStore, you can use https://keystore-explorer.org/[KeyExplorer] (has an interesting UI) or https://docs.oracle.com/javase/8/docs/technotes/tools/unix/keytool.html[keytool] (comes with JDK).

.Procedure
Create a https://kubernetes.io/docs/tasks/configmap-secret/managing-secret-using-config-file/[Kubernetes Secret] based on the customized TrustStore:
...
Please note that it's important to keep the Secret keys as defined in the above command. `cacerts` for the TrustStore file and `keyStorePassword` for the TrustStore password.

Now you can deploy the {PRODUCT} service that will use this TrustStore. For example:
...
Alternatively, you can deploy it using the {PRODUCT} CLI. You only have to pass the flag `truststore-secret`:
...
For example {PRODUCT} service configurations for a Kubernetes deployment, see the https://github.com/kiegroup/kogito-operator/tree/master/examples/kubernetes/travel-agency[`travel-agency`] example application for Kubernetes.
...
.Procedure
. Go to the https://github.com/kiegroup/kogito-operator/releases[`{PRODUCT_INIT}-cloud-operator`] releases page in GitHub, download the latest version of the {PRODUCT} Operator `Source code` file, and extract the downloaded `kogito-operator-__VERSION__` file to a local directory.
+
The {PRODUCT} Operator source code contains resources for installing the operator manually for Kubernetes deployments.
. In a command terminal, navigate to the root of the extracted `kogito-operator-__VERSION__` folder and enter the following commands to create the Kubernetes namespace (if not created already) and to install the {PRODUCT} Operator in that namespace:
+
--
.Installing the {PRODUCT} Operator in a Kubernetes namespace
[source,subs="attributes+,+quotes"]
----
$ cd ~/kogito-operator-__VERSION__
...
For an example script to install these infrastructure components for Kubernetes, see the https://github.com/kiegroup/kogito-operator/blob/master/examples/kubernetes/travel-agency/deploy.sh[`travel-agency`] example application for Kubernetes.
--
...
spec:
  ...
  replicas: 1
  image: quay.io/kiegroup/kogito-travels:latest
  infra:
    - kogito-kafka-infra
    - kogito-infinispan-infra
----

This example is based on the https://github.com/kiegroup/kogito-operator/tree/master/examples/kubernetes/travel-agency[`travel-agency`] example application for Kubernetes and includes additional configurations for Infinispan persistence, Kafka messaging, and the {PRODUCT} Data Index Service.
--
