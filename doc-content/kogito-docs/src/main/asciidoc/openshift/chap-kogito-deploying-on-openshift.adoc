...
===== {PRODUCT} Operator third-party operator instance creation
...
===== {PRODUCT} Operator custom third-party operator instances

You can also define your custom infrastructure resource and link it in the `KogitoInfra` resource.

Additionally to above operators supported for creation, the {PRODUCT} Operator also allows you to interact with instances created with this operator:

* *https://github.com/mongodb/mongodb-kubernetes-operator[MongoDB Operator]*: Used to interact with MongoDB created instances for process data persistence in {PRODUCT} services
...
Similarly, you could define a `KogitoInfra` resource for MongoDB to allow your processes to connect to it.  
In that case, you also need to define some extra properties into the `KogitoInfra` resource for the {PRODUCT} Operator to interact correctly with the MongoDB instance:
...
You could also connect the {PRODUCT} Data Index Service to a MongoDB instance instead:
...
[role="_abstract"]
{PRODUCT} Operator allows you to link a Kogito infrastructure to a custom MongoDB instance. You can setup the link for process data persistence in {PRODUCT} services by using the following {PRODUCT} CLI operation or by providing a `KogitoInfra` custom resource definition:
...
If you set up a custom MongoDB cluster and do no use a `KogitoInfra` resource, you can refer to it in the `KogitoRuntime` custom resource by configuring the following environment variables and application properties:
...
On Spring Boot, we don't use `spring.data.mongodb.uri` as it does not work simultaneously with host, port and credentials properties (Spring Boot) and we do recommend to have the passwords in a Kubernetes secret.
...
Instead of using the default Infinispan-based persistence in the {PRODUCT} Data Index Service, you can use a `KogitoInfra` linked to a MongoDB instance.
...
If a {PRODUCT} service is removed, the associated protobuf files are also removed and are no longer visible in the Data Index Service, but the data is still persisted in Infinispan/MongoDB.
...
--
. If your {PRODUCT} project requires Infinispan/MongoDB persistence and Apache Kafka messaging, install the required Infinispan/MongoDB and Kafka infrastructure components, including the https://github.com/infinispan/infinispan-operator[Infinispan Operator], https://github.com/mongodb/mongodb-kubernetes-operator/blob/master/README.md[MongoDB Operator] and the https://strimzi.io/docs/latest/[Strimzi Operator] (for Kafka cluster deployment with Zookeeper).
+
--
The {PRODUCT} Operator cannot automatically install these operators for Infinispan/MongoDB persistence and Kafka messaging, so you must install the components manually, if needed.
