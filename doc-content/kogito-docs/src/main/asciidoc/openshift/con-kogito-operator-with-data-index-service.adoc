...
The command above will use as default the Infinispan as Persistence Providers. Since {PRODUCT} 0.16.0 MongoDB is also supported. To configure {PRODUCT} Data Index Service to use Mongodb as persistence provider use the following command:

.Enable Data Index Service with MongoDB
----
$ {PRODUCT_INIT} install data-index --infra __KAFKA_INFRA_NAME__ \
    --image quay.io/kiegroup/kogito-data-index-mongodb:latest \
    --config quarkus.mongodb.connection-string=mongodb://mongodb-service:27017 \
    --config quarkus.mongodb.database=kogito \
    --config quarkus.mongodb.credentials.username=kogito_user \
    --config quarkus.mongodb.credentials.password=kogito_pwd \
    -p __PROJECT_NAME__
----

Before creating {PRODUCT} Data Index Service with MongoDB, it must be already configured. {PRODUCT} does not support automatic configuration as support by `KogitoInfra` with Infinispan Operator.
