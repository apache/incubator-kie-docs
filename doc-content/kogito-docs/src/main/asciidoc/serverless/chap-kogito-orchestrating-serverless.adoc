...
https://github.com/kiegroup/kogito-examples/tree/stable/kogito-quarkus-examples/serverless-workflow-error-quarkus[This] is an example that invokes an `EvenService`, which is throwing `IllegalArgumentException` if the number in the workflow model is odd.