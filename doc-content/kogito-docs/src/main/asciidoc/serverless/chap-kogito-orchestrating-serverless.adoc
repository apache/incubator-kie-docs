...
* `functions`: are reusable constructs that you can use to define invocation information about services that need to be invoked during workflow execution.
* `events`: are reusable constructs that you can use to define all consumed and produced events during workflow execution.
* `states`: enable you to define your workflow states (control-logic blocks) and determine what your workflow should do.

So what can you automate with Serverless Workflows? You can get some ideas from the {SERVERLESS_WORKFLOW_REPO_URL_BASE}/tree/{SERVERLESS_WORKFLOW_VERSION_IN_REPO_URL}/usecases[use cases].
