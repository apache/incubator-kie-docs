...
+
Properties on the process variables panel can be updated or deleted and newly added by clicking on the icons that appears on hovering it.
To add a new object to a key, click on the edit icon against the key and provide an object in json format then click on tick icon or hit enter.
After making changes on the process variables click on Save button to save the changes.
Click on refresh icon to revert the changes before saving it.
+
