...
+
A job can be a periodic or one-time run job. Periodic job can be rescheduled by modifying expiration time, repeat interval and repeat limit, whereas one-time run job can be rescheduled by changing the expiration time from DateTime picker.
+
