...
+
Load more: is present at the bottom-right corner for the page, is only visible if there are more items to show and allows to load the process instances incrementally. It consist of a dropdown which can be used to select the number of items to be loaded. The available values are 10, 20, 50 and 100. After selecting the desired value, click on the text (eg: 'load 20 more') to load the next instances.
+
NOTE: Load more is used in *Domain explorer* and *Task console*
...
* *Jobs panel:* Shows a list for jobs related to a process instance in the details page. A kebab-dropdown with a list of actions is available for each job. On click of details in kebab, opens a modal with elaborate details of the job.
+
NOTE: Only *Details* is currently available in the kebab-dropdown.
