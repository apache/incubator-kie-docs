...
+
A node in **Active** state can be either retriggered or canceled.This is done using the process-management rest endpoints(For more information on rest endpoints see, xref:con-bpmn-process-management-addon_kogito-developing-process-services[] ). While canceling exits the node, retriggering re-invokes the node. Both the operations are available in the Timeline of the process details page.
+
