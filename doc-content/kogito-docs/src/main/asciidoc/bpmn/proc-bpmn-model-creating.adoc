...
For example, the following metadata attribute `myAttribute` and value `myAttributeValue` in a BPMN process correspond to a custom event listener for starting the process:
...
<bpmn2:process id="approvals" name="approvals" isExecutable="true" processType="Public">
  <bpmn2:extensionElements>
    <tns:metaData name="myAttribute">
      <tns:metaValue><![CDATA[myAttributeValue]]></tns:metaValue>
    </tns:metaData>
  </bpmn2:extensionElements>
----
...
public class MyListener implements ProcessEventListener {
    ...
    public void beforeProcessStarted(ProcessStartedEvent event) {
        ...
        if (metadata.containsKey("myAttributeValue")) {
