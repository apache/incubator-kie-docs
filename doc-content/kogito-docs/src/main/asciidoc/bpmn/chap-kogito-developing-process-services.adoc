...
.Properties of traffic violation process
...
For example: `org.kie.kogito.traffic.LicenseValidationService`
...
The properties that are required to set for the DMN service task include `Rule Language`, which must be set as `DMN`. Also, you must set the values for `Namespace`, `Decision Name` and `DMN Model Name` properties as defined in the DMN model, such as `TrafficViolation.dmn`.
...
Once the example application is up and running, there should be different processes running under the following URLs:

[horizontal]
Process using Service Tasks (REST):: `http://localhost:8080/traffic_service`
Process using REST Work Item (REST):: `http://localhost:8080/traffic_wih`
Process using Business Rule Task (Embedded):: `http://localhost:8080/traffic`

To make use of this application it is as simple as sending requests to one othe URLs with appropriate contents. See the following examples:
