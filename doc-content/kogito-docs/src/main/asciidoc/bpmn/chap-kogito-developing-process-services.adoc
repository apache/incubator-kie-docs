...
In the previous configuration, `task-notification-quarkus-addon` is added as a dependency that uses the Smallrye library to define a channel named `kogito-deadline-events`. In the `kogito-deadline-events` channel, an event that contains the notification data is published into the configured Apache Kafka topic.
...
To send an email notification, you need to add the following properties to the `application.properties` file.
