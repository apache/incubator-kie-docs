...
* *Domain Explorer*: Use this tab to explore your process instances in the {PRODUCT} services using specific data of the domain
...
When you select a process instance on the *Process Instances* page, it redirects you to the *Process Details* page, providing the process instance details including interactions with the process instance. You can access the *Process Details* page from a different {PRODUCT} Management Console tab, displaying the instance related data and enables you to perform the available actions.
...
You can use the domain explorer tab to view the data that is generated from the process instances in the {PRODUCT} services or domains, such as travels and visa applications domains.
...
.Attributes for VisaApplications domain data columns
...
Note that for testing purposes, the {PRODUCT} Management Console also provides a test user system, which is enabled using the `kogito.test.user-system.enabled=true` property.
...
configure Task Console to use those states and show them on the filters:

* `kogito.task.states.list`: Comma-separated list of values containing all the states you want to show on Task Console.
When not provided Task Console will show the default lifecycle states (`Ready`, `Reserved`, `Completed`, `Aborted` and `Skipped`);
* `kogito.task.active.states.list`: Comma-separated list of values containing the active task states. This states will
be used in *Task Inbox* the default filter. When not provided the default active states will be `Ready` and `Reserved`.

.Configuring custom task states on Task Console
[source,subs="+quotes"]
----
$ java  \
...
[NOTE]
====
For more information about task lifecycle and how to create custom task lifecycles read xref:con-task-lifecycle_kogito-developing-process-services[] and xref:proc-task-lifecycle-custom_{context}[]
====
...
Note that for testing purposes, the {PRODUCT} Task Console also provides a test user system, which is enabled using the `kogito.test.user-system.enabled=true` property.
