...
+
In the timeline if a node is related to a job, you can see a _clock icon_ near the node name. On clicking this icon the details of the job is shown. These nodes have additional options in the kebab(in combination with the options for error and active states) to perform _Cancel_, _Reschedule_ or _View Details_ of the job. xref:job-actions[For more information, see job actions]
+
.Dropdown with job actions(error node)
...
+
.Dropdown with job actions(active node)
image::kogito/bpmn/kogito-management-console-node-active-job-actions.png[Image of job actions options for ative node in timeline of Management Console]
+
...
+
For jobs in _Scheduled_, _Retry_ and _Error_ state , cancel action is available to cancel the jobs.
+
.Cancel a job
image::kogito/bpmn/kogito-management-console-jobs-panel-cancel.png[Image of jobs cancel option in Management Console]
+
.Successful cancellation of a job
image::kogito/bpmn/kogito-management-console-jobs-panel-cancel-details.png[Image of jobs cancel details in Management Console]
+
...
+
* *Jobs Management*: Use this page to view a list of jobs available and also filter the jobs by status. A kebab is available for each job to view details, reschedule a job or cancel a job. xref:job-actions[For more information, see job actions]
+
.Jobs management
image::kogito/bpmn/kogito-management-console-jobs-management.png[Image of jobs management page in Management Console]
+
The filter is a dropdown with a list of job status namely: _Cancel_, _Error_, _Executed_, _Retry_ and _Scheduled_.By default the _Scheduled_ status is selected. Select the required status and click on the *Apply filter* button to reload the list of jobs according to the filters. A *refresh button* is available to refresh the list. A *Reset to default* button is available to reset any applied filters.
+
...
+
You can also select multiple jobs using the checkboxes and perform bulk operation(s) at the top of the screen. A _Cancel selected_ option is available to cancel all the selected jobs.
+
