...
[role="_abstract"]
{PRODUCT} provides an addon named `process-svg-addon` that enables basic REST operations that you can use to visualize the process
...
NOTE: Currently, this addon is only available for Quarkus services but is planned add support to Spring Boot projects too
...
* *Process diagram*: Returns the process SVG diagram.
* *Process instance execution path Diagram*:  Returns Process SVG diagram displaying the executed path of a specific process instance


=== REST endpoints for the process SVG add-on

After you add the `process-svg-quarkus-addon` dependency to your {PRODUCT} project, the recommended configuration properties
...
Returns the process SVG diagram, highlighting the process instance executed path:
--
