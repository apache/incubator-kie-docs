...
NOTE: This REST endpoint is ready to be used in environments with security enabled at data-index service connected.
Internally, the add-on is generating a query on data-index service to retrieve the executed nodes. If the data-index
service has the security enabled, it needs a valid Bearer token passed to do that query.
The svg addon tries to retrieve the bearer token if it detects a known security Context enabled,
in other cases it reuses the one passed directly in the 'Authorization' header.

NOTE: Currently the process SVG add-on is ready to retrieve the token for systems with OpenID Connect using Keycloak
...
image::kogito/bpmn/kogito-management-console-domain-explorer-travels-domain-data.png[Image of filters for VisaApplications domain in Management Console]
...
image::kogito/bpmn/kogito-management-console-domain-explorer-travels-sorted-data.png[Image of Domain data expanded, displays the related process instances]
