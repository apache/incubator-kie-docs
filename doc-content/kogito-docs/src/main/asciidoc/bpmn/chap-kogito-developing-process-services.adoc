...
In {PRODUCT}, runtime tools Devuser interface allows developers to access different features of {PRODUCT} Management Console and Task Console together in an unified interface. The runtime tools Dev UI is intended for the local development environment, containing main features of both the consoles, which is also easy to set up.
