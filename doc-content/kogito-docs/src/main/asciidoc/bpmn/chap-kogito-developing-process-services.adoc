...
Alternatively, the same can be acomplished by using the Knative `KogitoSource`. To know more about this exclusive Knative Eventing component, https://github.com/knative-sandbox/eventing-kogito[please visit the project's website].
...
Optionally, you can have the https://quarkus.io/guides/deploying-to-kubernetes[Quarkus Kubernetes] dependency added as exemplified. In this case, the add-on will also generate Knative and Kubernetes resources to make it easier to deploy the {PRODUCT} Service on any cluster. The exemplification that follows assumes that you are using the `quarkus-kubernetes` extension.
...
NOTE: The environment variable `K_SINK` must exist in your environment if you plan to run it locally for test purposes. You can tweak the property to fallback to a constant value by adding the following value to it:
...
. After you configure and test your {PRODUCT} project locally, in your Kubernetes or OpenShift environment, add the following custom resources to deploy your {PRODUCT} project. These resources were generated automatically by the {PRODUCT} Knative Eventing Add-On and can be found at `target/kubernetes/kogito.yml` and `target/kubernetes/kubernetes.yml` files. They are required for consuming or producing events in a Knative Eventing environment.
+
...
* Knative `Trigger`: To make our service aware of the events of type `travellers` arriving in the Broker:
+
...
----
* Knative `SinkBinding`: The `SinkBinding` resource used to sink the {PRODUCT} service produced events to the Knative Broker. Alternatively, you can set the property `org.kie.kogito.addons.knative.eventing.generate-kogito-source=true`. This way the add-on will generate a Knative `KogitoSource` instead, as shown in the following example:
+
.Example `KogitoSource` custom resource for a {PRODUCT} service with the default `Broker` bound to it
...
After you apply these auto-generated resources to the Kubernetes or OpenShift namespace, if any BPMN or Serverless Workflow files in the {PRODUCT} project contain events to be consumed or produced, they will be dispatched to the default Broker.

The Knative Eventing `Trigger` resource filters all messages delivered to the default Broker and sends them to the {PRODUCT} service.

This Knative Eventing `SinkBinding` resource injects the `${K_SINK}` environment variable to the `Deployment` or Knative `Service` resource created by the add-on. Every message produced by the {PRODUCT} service is redirected to the default Knative Broker.

NOTE: If you opt to not use the auto-generated resources, you must create these `Trigger` and `SinkBinding` resources manually.
...
<!-- TODO: add the add-on section link -->
