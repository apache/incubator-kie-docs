...
{PRODUCT} Service task also allows the usage of a special parameter in your method signature in order to receive the process context in which the task is executed.
To get access to the context, simply add a new parameter of type `org.kie.kogito.internal.process.runtime.KogitoProcessContext` and {PRODUCT} will resolve the appropiate value
