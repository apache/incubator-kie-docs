...
Quarkus dev ui shows a list of extensions that you have on the project. To enable the runtime tools Quarkus extension from your Quarkus project, you must add the following dependency to the `pom.xml` file:
...
Also, add the following properties to the `application.properties` file. This configures users and impersonates them on the task page using the dropdown available at the top.
