...
In {PRODUCT}, runtime tools quarkus extension helps in developing your {PRODUCT} applications in quarkus. This extension will add a new user interface with unified features from the different {PRODUCT} Management Console and Task Console that will be accessible in the quarkus dev ui. This is intended for the local development environment, can be integrated with quarkus applications and allows you test the kogito-app without the need of a complex setup.

=== Setting up the Runtime tools quarkus extension

To enable the extension from your Quarkus project, you must add the following dependency to the `pom.xml` file:
...
Also, add the following properties to the `application.properties` file. This configures users that will be available in Task Inbox. The property quarkus.kogito-runtime-tools.users creates the list of users, while the quarkus.kogito-runtime-tools.users.{user}.groups assigns roles to them. These created users will be impersonated on the tasks page using the dropdown available at the top.
...
The Quarkus extension consists of different menus to navigate to different pages, such as Configuration, Kogito Runtime Tools, and Infinispan Client. Using the *{PRODUCT} Runtime Tools* menu, you can navigate to *Process Instances*, *Tasks*, *Jobs*, and *Forms*.
...
.Menus in Quarkus extension
image::kogito/bpmn/forms-section-kogito-runtime-tools.png[Image of menus in Quarkus extension]

The features in the Dev UI are similar to {PRODUCT} Management Console and Task Console. For more information about the {PRODUCT} Management Console and Task Console, see xref:con-management-console_kogito-developing-process-services[] and xref:con-task-console_kogito-developing-process-services[].

.Process Instances in {PRODUCT} runtime tools Dev UI
image::kogito/bpmn/kogito-runtime-tools-dev-ui.png[Image of process instances in Kogito runtime tools dev ui]

=== Forms

The {PRODUCT} quarkus extension enables you to customize the forms directly on the browser by using the dedicated code editor, which can compile HTML code and React code that are written in Typescript. You can use Patternfly components to create React forms. These customized forms will be used in Task Inbox.
...
Code editor has some toolbar options, which allows us to undo and redo changes. Clicking on play button would execute the code and changes are applied immediately, while save option allows us to save the changes made on the source or configuration of the forms.
