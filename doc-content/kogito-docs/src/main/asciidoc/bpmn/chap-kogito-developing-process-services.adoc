...
[role="_abstract"]
{PRODUCT} provides a `process-svg-addon` add-on that enables basic REST operations that you can use to visualize the process
diagram and execution path of its process instances. These REST operations are supplemental to any other specific REST
operations that you have configured in your application.

The addon requires to have access to processes svg files. They can be placed at file system where the service rely or
be available at classpath inside the structure 'META-INF/processSVG/'.

Currently, this is a manually process but it's meant to be automaticaly generated. The VSCode extension provide controls
to export the process to SVG format:

.Export process diagram at VSCode
image::kogito/bpmn/kogito-svg-addon-manually-export.png[Manually export to SVG at VSCode]

Once we have the SVGs files generated, they have to renamed to {ProcessId}.svg, to be used by the addon, and placed in a folder in
filesystem accessible by the service or in a 'META-INF/processSVG' folder at classpath.
The addon configuration property _kogito.svg.folder.path_ will point to the File system foder, otherwise it will be
searched at classPath.
...
.Project dependency to enable process SVG REST operations
[source,xml]
----
<dependency>
  <groupId>org.kie.kogito</groupId>
...
NOTE: Currently, this addon is only available for quarkus services but is planned add support to springboot projects too

The addon provides this set of configuration properties:

.Process SVG addon configuration properties in {PRODUCT}
[cols="30%,70%"]
|===
|Property |Description

a|`kogito.svg.folder.path`
|Determines in which folder the addon have to search initial process diagrams.'ProcessId.svg'. If this property is not set, the svg would
be searched in 'META-INF/processSVG/' location along the classpath.
...
|Change the color to fill completed nodes when showing the execution path.
...
|Change the color of completed nodes border, when showing the execution path.
...
|Change the color of active nodes border, when showing the execution path.
...
* *Process Diagram*: Get the process SVG diagram.
* *Process instance execution path Diagram*:  Get Process SVG diagram displaying the executed path of a specific process instance


=== REST endpoints for the process SVG add-on

After you add the `process-svg-quarkus-addon` dependency to your {PRODUCT} project, add the configuration properties
and run your {PRODUCT} services, you can use the following REST endpoints to get the process related SVGs.
...
Returns the SVG that shows the process diagram.
...
==== Process instance execution path Diagram*

Return process SVG diagram, showing the process instance executed path:
--
...
In order to visualize the process instance execution path at the management console it will be required to add:

* *{PRODUCT} process SVG add-on*: Enables the SVG diagram, visualizating the process instance execution path. If you do
not enable this add-on for your {PRODUCT} service, the Management Console won't be able to display the Diagram panel.
For more information about the process management add-on, see xref:con-bpmn-process-svg-addon_kogito-developing-process-services[].
...
+
If a node in the timeline is related to a job, you can select the clock icon to view the job details or select the kebab menu for additional job operations, such as *Job Reschedule* or *Job Cancel*, depending on the job status.
+
.Reschedule the job for a node in error state
image::kogito/bpmn/kogito-management-console-node-error-job-actions.png[Image of job actions options for error node in timeline of Management Console]
+
...
+
For jobs in *Scheduled* or *Error* state, you can also select the *Reschedule* option to reschedule the jobs or *Cancel* to cancel the jobs. You can modify the expiration time for one-time jobs, or the expiration time, repeat interval, and repeat limit for periodic jobs.
+
...
+
The *Process Diagram* panel displays the process diagram visualizing the execution path when the svg are provided.
+
.Process Diagram
image::kogito/bpmn/kogito-management-console-process-diagram.png[Image of process diagram panel in Management Console]
+
...
+
* *Jobs Management*: Use this page to view a list of available jobs, to filter jobs by status, and perform actions for individual jobs, such as view details, reschedule, or cancel.
+
