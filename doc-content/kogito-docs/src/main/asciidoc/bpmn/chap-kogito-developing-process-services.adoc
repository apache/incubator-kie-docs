...
+
If you plan to run the application locally, please take a look at the example https://github.com/kiegroup/kogito-examples/tree/stable/process-knative-quickstart-quarkus["Process Knative Quickstart Quarkus"]. You will understand how to configure the enviroment in your local machine using https://knative.dev/docs/eventing/samples/container-source/#create-a-knative-service[Knative Event Display].
+
--
...
When a user task deadline is reached and the *Task state type* is still *Not started* or *Not completed*, then a notification is triggered using the configured add-ons. To publish an event on Apache Kafka, {PRODUCT} provides `task-notification-quarkus-addon` and `task-notification-springboot-addon` addons, and to send an email notification, {PRODUCT} provides `mail-quarkus-addon` and `mail-springboot-addon`. The `mail-quarkus-addon` and `mail-springboot-addon` add-ons send the email notification using the information added on the *Notification* page.
...
Once the event is published into the configured Apache Kafka topic, you can configure a listener that publishes the email on event reception. To configure an event listener, add the `mail-quarkus-addon` in the `pom.xml` file of your {PRODUCT} project. The `mail-quarkus-addon` accesses the Apache Kafka records that are published on `kogito-deadline-events` and sends an email using the information in the Apache Kafka record.
...
* https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-services/src/main/java/org/kie/kogito/services/event/impl/ProcessInstanceEventBody.java[ProcessInstanceEvent]: This event is published when a process instance is created, modified, or completed.
* https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-services/src/main/java/org/kie/kogito/services/event/impl/UserTaskInstanceEventBody.java[UserTaskInstanceEvent]: This event is published when a change occurs on a user task.
* https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-services/src/main/java/org/kie/kogito/services/event/impl/VariableInstanceEventBody.java[VariableInstanceEvent]: This event is published when a variable is created, modified, or removed.
...
In Kogito, runtime tools  Devuser interface allows developers to access different features of Management Console and Task Console together in an unified interface. 

The runtime tools Dev UI is intended for the local development environment, containing  main features of both the consoles, which is also easy to set up..

To access the Dev UI from your Quarkus project, you can add the following dependency to the pom.xml file:
...
Also, add the following properties to the application.properties file:
...
To access the Dev UI, run your project in development mode and select the http://localhost:8080/q/dev URL:

.Quarkus Dev UI
image::kogito/bpmn/quarkus-dev-ui.png[Image of quarkus dev ui]

The Quarkus extension consists of different menus to navigate to different pages, such as Configuration, Kogito Runtime Tools, and Infinispan Client. Using the Kogito Runtime Tools menu, you can navigate to Process Instances, Tasks, Jobs, and Forms. 

* *Process Instances*: Use this section to view process and subprocess instances by status or business key.

* *Jobs*: Use this section to view a list of available jobs. You can also filter the jobs by status and perform different actions, such as view details, reschedule, or cancel.

* *Tasks*: Use this section to view the list of available user tasks that you can interact with by filtering based on the status.

* *Forms*: Use this section to generate and customize the forms directly in the browser.

The features in the Dev UI are similar to Kogito Management Console and Kogito Task Console. For more information about the Kogito Management Console and Kogito Task Console, see 

forms-section-kogito-runtime-tools
.Kogito runtime tools dev ui
image::kogito/bpmn/kogito-runtime-tools-dev-ui.png[Image of kogito runtime tools dev ui]

==== Processes

The Process Instances section displays the list of process instances and enables you to apply filters to the list. You can also filter processes based on business keys and you can abort, skip, or retry single or multiple processes.

When you select a process from the list, you are redirected to the process details page to view the detailed information of the process.

The features available on the Process Instances page are similar to the pages available in Management Console.  For more information about Management Console and related features, see <cross-reference link>.

==== Jobs

The Jobs section displays the list of existing jobs, which you can filter and perform related management actions such as viewing job details, rescheduling a job, or canceling a job.
...
The forms section on the Kogito Runtime Tools has the most exciting feature that facilitates developers to generate and customize the forms directly on the browser. To enable the customization, we have a dedicated and powerful code editor that can compile HTML forms and React forms written in typescript. Developers can use Patternfly components to create React forms at present.

.Forms section in kogito runtime tools
image::kogito/bpmn/forms-section-kogito-runtime-tools.png[Image of forms section kogito runtime tools]
...
.Forms table view
image::kogito/bpmn/forms-pages-table-view.png[Image of forms table view]

.Forms card view
image::kogito/bpmn/forms-pages-card-view.png[Image of forms card view]

When you select a form in the Forms page, you are redirected to form displayer. The form displayer contains a code editor, which compiles the source code and displays the results on the right-side panel.

Initially, the form content and the related configuration are added to the code editor. You can make changes to the code in the editor and click the play option from the toolbar to execute the code immediately on the browser. 


The Source tab loads the source code of the form in HTML or Typescript format based on the selected form. When the configurations of a form are loaded on the Connections tab, which is a JSON object containing the schema, a set of resources including CDN links, URLs pointing to JS or CSS files are added to the form. 

.Form displayer source code editor
image::kogito/bpmn/runtime-tools-dev-ui-code-editor.png[Image of form displayer source code editor]

.Form displayer cofigurations code editor
image::kogito/bpmn/runtime-tools-dev-ui-code-eitor-configurations.png[Image of form displayer configuration code editor]
