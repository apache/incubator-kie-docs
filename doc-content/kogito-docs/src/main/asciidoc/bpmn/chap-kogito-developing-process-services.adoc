...
[role="_abstract"]
{PRODUCT} management console has security enabled using keycloak and uses the bearer token to communicate with other kogito services.These tokens are issued by OpenID Connect and OAuth 2.0 compliant authorization servers such as https://www.keycloak.org/about.html[Keycloak].
...
[role="_abstract"]
{PRODUCT} task console has security enabled using keycloak and uses the bearer token to communicate with other kogito services.
