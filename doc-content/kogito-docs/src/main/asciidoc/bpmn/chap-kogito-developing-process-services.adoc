...
IMPORTANT: Make sure to login into the administration console of keycloak, navigate to the respective client configuration and add the correct urls corresponding to the different applications (or * for easier configuration but it could cause security issues) or the origin urls as a parameter to the *Web Origins*. This prevents issues related to `Access-Control-Allow-Origin`. 
