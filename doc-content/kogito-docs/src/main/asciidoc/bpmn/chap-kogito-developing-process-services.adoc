...
In {PRODUCT}, runtime tools quarkus extension allows developers to access different features of {PRODUCT} Management Console and Task Console together in an unified interface. This is intended for the local development environment, can be integrated with quarkus applications and allows you test the kogito-app without the need of a complex setup.

To access the extension from your Quarkus project, you can add the following dependency to the `pom.xml` file:
...
Also, add the following properties to the `application.properties` file. This configures users that will be available in Task Inbox.
