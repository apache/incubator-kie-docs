...
{PRODUCT} provides a process event add-on, enabling you to send processes, tasks, and variable events to an external event listener. In this case, the processes, tasks, and variable events are generated as a result of the execution of an operation, which modifies a process state; such events are known as runtime events.

In {PRODUCT}, every modifying operation is executed within an abstraction called a unit of work. Example of such operations includes creating a process instance, transitioning a task, and modifying a variable. You can publish a runtime event when a unit of work is completed.
...
The events that are published using the {PRODUCT} process event add-on follow https://cloudevents.io/[CloudEvent] specification, such as the data field contains a JSON of one of the following types:
...
Smallrye defines an abstraction named channel to enable multi-broker support. For every channel that is defined in your {PRODUCT} application, you can specify the connector to be used for that channel using the following form:
...
IMPORTANT: If defined property is not found, then the topic name is considered to be the same as Kafka name.
