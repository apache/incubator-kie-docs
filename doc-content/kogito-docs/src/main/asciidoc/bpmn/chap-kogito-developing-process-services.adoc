...
For example, the following REST operations use the endpoint `/orders` to interact with customer orders. You can use a REST client, curl utility, or the Swagger UI configured for the application (ex: \http://localhost:8080/q/swagger-ui or \http://localhost:8080/swagger-ui.html etc. depending on configuration) to send API requests to interact with the running application.

.Swagger UI to interact with all application endpoints (ex: \http://localhost:8080/q/swagger-ui or \http://localhost:8080/swagger-ui.html etc. )
