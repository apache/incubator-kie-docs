...
In {PRODUCT}, runtime tools Dev user interface allows developers to access different features of {PRODUCT} Management Console and Task Console together in an unified interface. The runtime tools Dev UI is intended for the local development environment, containing main features of both the consoles, which is also easy to set up.
...
The {PRODUCT} runtime tools Dev UI enables you to generate and customize the forms directly on the browser. To customize the forms, you can use the dedicated code editor, which can compile HTML and React forms that are written in Typescript. However, you use Patternfly components to create React forms in the {PRODUCT} Management Console.
