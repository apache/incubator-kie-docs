...
The remote integration approach uses REST APIs from the decision services where the process can can make use of any BPMN feature to send HTTP requests like a custom service task or the REST Work Item [*insert reference*] available in {PRODUCT}. In this approach the process communicates with the decision services through the network, in this way it is not necessary they are running in the same application.
