...
used in to guide the process flow upon the result of this decision. In {PRODUCT} decisions can be declared as distinct services using DMN, described in <<chap-kogito-using-dmn-models>> or DRL, described in <<chap-kogito-using-drl-rules>>.
...
The embedded integration approach basically makes usage of the `Business Rule Task` <<ref-bpmn-businessruletask_{context}>> available in
...
.Embedded process integration with decisions
...
To show it in more details there is available an example with https://github.com/kiegroup/kogito-examples/tree/main/process-decisions-quarkus[Quarkus] and https://github.com/kiegroup/kogito-examples/tree/main/process-decisions-springboot[Spring Boot].
...
The remote integration approach uses REST APIs from the decision services where the process can can make use of any BPMN feature to send HTTP requests like a https://github.com/kiegroup/kogito-examples/tree/main/process-rest-service-call-quarkus[Service Task] or the https://github.com/kiegroup/kogito-examples/tree/main/process-rest-workitem-quarkus[REST Work Item], available in {PRODUCT}. In this approach the process communicates with the decision services through the network, in this way it is not necessary they are running in the same application.
...
To show it in more details there is available an example with https://github.com/kiegroup/kogito-examples/tree/main/process-decisions-rest-quarkus[Quarkus] and https://github.com/kiegroup/kogito-examples/tree/main/process-decisions-rest-springboot[Spring Boot].
