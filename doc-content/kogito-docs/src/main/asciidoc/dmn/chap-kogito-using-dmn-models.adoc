...
Please note that for Json marshalling, Kogito is relying on https://quarkus.io/guides/rest-json#jackson[Quakus] and https://docs.spring.io/spring-boot/docs/1.3.0.M3/reference/html/howto-spring-mvc.html#howto-customize-the-jackson-objectmapper[SpringBoot] default mappers, which can be configured according to specific needs. 
