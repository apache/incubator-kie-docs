...
=== New operator in Data Index GraphQL API  

Starting this release, the Data Index GraphQL API now supports a `not` operator. This operator allows to negate any single filter available for a specific GraphQL type.

For example, UserTaskInstances type contains a attribute named `excludedUsers`, an array of Strings. So if I would like to create a query, making sure that a specific user is not in this list.
I can simply surround the usual filter for this attribute with a `not` one. Like the example below:
