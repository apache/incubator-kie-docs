...
* https://issues.redhat.com/secure/ReleaseNote.jspa?projectId=12322421&version=12353775[{PRODUCT} {COMMUNITY_VERSION_FINAL}]
...
==== {PRODUCT} Operator support for runtimes configuration using PostgreSQL

The {PRODUCT} Operator now supports runtimes configuration using PostgreSQL. For more information, see {URL_DEPLOYING_ON_OPENSHIFT}#con-kogito-operator-with-postgresql_kogito-deploying-on-openshift[_PostgreSQL persistence with {PRODUCT} services_].
...
==== Support for Keycloak in Management and Task Console for authentication

The authentication mechanism for the Management and Task Console is changed. You can now use Keycloak for the authentication.

For more information, see {URL_PROCESS_SERVICES}#con-management-console_kogito-developing-process-services[_{PRODUCT} Management Console_] and {URL_PROCESS_SERVICES}#con-task-console_kogito-developing-process-services[_{PRODUCT} Task Console_].

==== Ability to disable REST endpoint generation
You can now disable the REST endpoint generation globally or for a specific resource using the following properties:
