...
* {PRODUCT} Management console now provides a separate page to view the list of jobs available. The jobs can be filtered based on the job status.
+
.Jobs management
image::kogito/bpmn/kogito-management-console-jobs-management.png[Image of jobs management page in Management Console]

* Multiple jobs can be selected using the checkboxes and bulk operation(s) can be performed on them using the button(s) available on top of the screen. The operation available is _Cancel selected_.
+
...
* A job cancel operation is now available in the jobs panel of _process details_.
NOTE: the cancel operation is available for jobs in _Scheduled_, _Retry_ and _Error_ state.

.Cancel a job
image::kogito/bpmn/kogito-management-console-jobs-panel-cancel.png[Image of jobs cancel option in Management Console]

* The _timeline_ in _process details_ page now provides options to perform job related actions in addition to the options for active and error nodes. Such nodes also provide a timer icon which when clicked shows detais of the corresponding job.
+
...
For more information about the {PRODUCT} Management Console, see {URL_PROCESS_SERVICES}#con-task-console_kogito-developing-process-services[_{PROCESS_SERVICES}_].
