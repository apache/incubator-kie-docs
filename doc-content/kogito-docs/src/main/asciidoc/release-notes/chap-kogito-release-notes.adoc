...
==== Added new quickstarts at kogito-examples

Created three news quickstarts to show how to interact with the process using the {PRODUCT} Management Console and the
{PRODUCT} Task console:

* https://github.com/kiegroup/kogito-examples/tree/master/process-usertasks-quarkus-with-console[process-usertasks-quarkus-with-console]
Allows to explore a usertasks process using the consoles providing an easy way to create the needed services using
docker-compose (Kafka, Infinispan, Data Index, Management Console and Task Console) and see how is the interaction with
the process using the consoles
* https://github.com/kiegroup/kogito-examples/tree/master/process-usertasks-timer-quarkus-with-console[process-usertasks-timer-quarkus-with-console]
based in a process with usertasks and a job (timer), adds the {PRODUCT} Job Service to the docker-compose environment,
and allows to explore the execution cycle showing the console interaction withs jobs.
* https://github.com/kiegroup/kogito-examples/tree/master/process-usertasks-with-security-oidc-quarkus-with-console[process-usertasks-with-security-oidc-quarkus-with-console]
This example allows to show the interaction of the consoles enabling security with an open id connector, in this case Keycloak.
