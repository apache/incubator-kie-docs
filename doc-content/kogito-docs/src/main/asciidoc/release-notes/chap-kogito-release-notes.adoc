...
Starting from this release, {PRODUCT} runtimes allows registering custom UnitOfWorkEventListener.
That way, it is possible to react to events when a UnitOfWork starts, ends or abort.
To use this new feature, you simply need to create a new CDI bean that implements `org.kie.kogito.uow.events.UnitOfWorkEventListener`,
which then can be used to react on relevant events: `onBeforeStartEvent`, `onAfterEndEvent` and `onAfterAbortEvent`.
