...
==== Process SVG addon is ready to work directly with the process SVG exported by VSCode

Now, it is just needed to export the process to SVG at the VSCode and, the code generation will make it ready to be consumed by the process SVG addon.

The SVG generated by the export of a process contained in a {processFileName}.bpmn (or .bpmn2) is named to {processFileName}-svg.svg.
The process code generation now searches this export and, if this file exists, copies and renames it to `META-INF/processSVG/{processId}.svg
allowing the process SVG addon to consume it directly.
