...
The graphql endpoint url now requires the entire path, including */graphql* . For instance http://host-name:port-number/graphql. The url can be passed in as environment variable - `kogito.dataindex.http.url` to the quarkus app or image.
