...
** https://issues.redhat.com/secure/ReleaseNote.jspa?projectId=12322421&version=12353875[{PRODUCT} tooling 0.8.4]
...
==== Pluggable `UnitOfWork` event listeners

You can now register a custom `UnitOfWorkEventListener`, which assists in reacting to the events when a `UnitOfWork` starts, ends, or aborts. To use this feature, create a new CDI bean that implements `org.kie.kogito.uow.events.UnitOfWorkEventListener`, which is further used to react on the relevant events, such as `onBeforeStartEvent`, `onAfterEndEvent`, and `onAfterAbortEvent`.

==== Error handling in {PRODUCT} BPMN modeler

{PRODUCT} now effectively handles error events in a process. For example, when a service task is completed with an error end event, indicating that the process has encountered an unexpected situation, then the error can be caught with an event subprocess using an error start event, which handles the unexpected situation properly.
