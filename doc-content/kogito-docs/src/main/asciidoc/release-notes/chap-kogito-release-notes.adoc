...
{PRODUCT} now effectively handles the errors thrown by the tasks in the BPMN modeler. For example, if a service task ends with an error end event, then the error can be caught on an event subprocess using an error start event. 
