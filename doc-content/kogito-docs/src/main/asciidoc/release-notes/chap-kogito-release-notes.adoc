...
* Management Console include a new panel to display the process diagram visualizing the execution path at Process instance details
...
==== Support for {PRODUCT} Audit Console security with OpenID Connect

For Quarkus-based {PRODUCT} services, you can now use the https://quarkus.io/guides/security-openid-connect[Quarkus OpenID Connect adapter] with the {PRODUCT} Audit Console to enable the console to interact with the {PRODUCT} Trusty Service using bearer token authorization.

For more information about the {PRODUCT} Audit Console, see {URL_DECISION_SERVICES}#con-audit-console_kogito-dmn-models[_{PRODUCT} Audit Investigation Console_].
