...
* https://issues.redhat.com/secure/ReleaseNote.jspa?projectId=12322421&version=12353772[{PRODUCT} {COMMUNITY_VERSION_FINAL}]
** https://issues.redhat.com/secure/ReleaseNote.jspa?projectId=12322421&version=12355589[{PRODUCT} tooling 0.9.1]
...
When you export a process to SVG format in VSCode, the code generation now enables you to access the generated SVG using the process SVG addon.

The generated SVG is named to {processFileName}-svg.svg. Also, the process code generation searches the export and if the file exists, the file is copied and renamed to `META-INF/processSVG/{processId}.svg`, which allows the process SVG addon to access the file directly.
...
==== Support PostgreSQL persistence for Jobs Service

You can now configure the {PRODUCT} Jobs Service to use https://www.postgresql.org/[PostgreSQL] for persistence storage as an alternative to in-memory or Infinispan persistence. You need to have PostgreSQL installed and running before enabling the persistence for Jobs Service.

With this enhancement, the https://repository.jboss.org/org/kie/kogito/jobs-service/[`jobs-service`] artifact is deprecated and the following new artifacts are available:

* https://repository.jboss.org/org/kie/kogito/jobs-service-common/[`jobs-service-common`]: Use this artifact to run Jobs Service with in-memory persistence
* https://repository.jboss.org/org/kie/kogito/jobs-service-infinispan/[`jobs-service-infinispan`]: Use this artifact to run Jobs Service with Infinispan
* https://repository.jboss.org/org/kie/kogito/jobs-service-postgresql/[`jobs-service-postgresql`]: Use this artifact to run Jobs Service with PostgreSQL

With this enhancement, `kogito.jobs-service.persistence` property for Jobs Service is deprecated. There are different artifacts that are built to support different types of persistence for the Jobs Service.

For more information about PostgreSQL persistence for {PRODUCT} Jobs Service, see {URL_CONFIGURING_KOGITO}#proc-jobs-service-persistence-postgresql_kogito-configuring[_{CONFIGURING_KOGITO}_].

==== Support PostgreSQL persistence for Data Index

You can now configure the {PRODUCT} Data Index to use https://www.postgresql.org/[PostgreSQL] for persistence storage as an alternative to MongoDB or Infinispan persistence. You need to have PostgreSQL installed and running before enabling the persistence for Jobs Service.
...
==== Optionally indexing domain specific data in Data Index

Starting from this release, it is now possible to enable/disable the custom types ( domain specific ) indexing. This is achieved by changing the value of the property `kogito.data-index.domain-indexing` to true or false depending on the behaviour you would like to achieve. Domain indexing is enabled by default on Data Index using MongoDB and Infinispan persistence type.
By turning the domain indexing off, the Data Index service will not create the custom types in GraphQL query. GraphQL schema will only expose queries for `ProcessInstances`, `UserTaskInstances` and `Jobs` types.

NOTE: Data Index PostgreSQL persistence provider does not yet support the indexing of custom types. So setting `kogito.data-index.domain-indexing=true` has no effect.
