...
==== Support for user task management using the REST endpoints

You can now change the phase, update task results, manage comments and attachments of a task using the relevant REST endpoints. For more information about the user task REST endpoints in {PRODUCT}, see {URL_PROCESS_SERVICES}#con-task-lifecycle_kogito-developing-process-services[_Task lifecycle in {PRODUCT} processes_].
