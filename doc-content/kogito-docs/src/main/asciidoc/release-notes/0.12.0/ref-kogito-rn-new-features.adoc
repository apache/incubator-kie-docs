...
=== Added node management capabilites to process details page
The {PRODUCT} management console now adds support to cancel or retrigger a node which is in Active state. This is available in the timeline of the process details page.

* Provides a kebab on a node which is in Active state with process management capability enabled.
* Kebab provides two clickable buttons for retrigger and cancel
* Once the action is performed , it opens a modal box with either a success or failure message.
