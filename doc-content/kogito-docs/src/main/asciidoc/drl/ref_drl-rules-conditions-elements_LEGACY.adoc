...
*evacchi: content below this mark should be moved in a legacy/advanced section*

*evacchi: the following section on from is superseded by oopath notation, I would move the following content to a pattern section for legacy/advanced*
...
*evacchi: entrypoints are superseded by data sources, which are roughly equivalent, but declared explicitly, we may move this to a legacy section*
