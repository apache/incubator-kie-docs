...
a|Lists comma separated package names which contain REST endpoint Java classes. Only required for Spring Boot applications. Sub packages are automatically scanned. Wildcard is available. Packages generated by DMN namespaces commonly start from 'http' 
