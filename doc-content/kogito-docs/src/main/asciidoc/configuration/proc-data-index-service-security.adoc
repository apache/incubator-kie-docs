...
The {PRODUCT} Data Index Service uses two end points with different security https://quarkus.io/guides/security-openid-connect#quarkus-oidc_quarkus.oidc.application-type[`quarkus.oidc.application-type`] configurations:

* The `/graphql` endpoint is configured as a `service` application that enables Bearer token authentication.
* The `/graphiql` endpoint is configured as `web-app`, redirecting to the Keycloak login page if the user is not authenticated.

NOTE: The graphql UI endpoint path can be configured, throw kogito.data-index.vertx-graphql.ui.path property.

The default configuration provides a multi-tenant configuration. For more information about multi-tenancy with OpenID Connect, see https://quarkus.io/guides/security-openid-connect-multitenancy[Using OpenID Connect multi-tenancy] in Quarkus documentation.

For more information about the configuration properties file, see https://quarkus.io/guides/security-openid-connect#configuring-using-the-application-properties-file[Configuring the application.properties file] in Quarkus documentation.
--
