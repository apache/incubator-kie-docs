...
When using the {PRODUCT} Process Quarkus extension, an ephemeral Data Index service is automatically provisoned when running Quarkus in Development mode. When you are using one of the following Quarkus extensions, the Dev Service is setup for immediate use:
