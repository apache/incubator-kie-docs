...
The Data Index allows the following operations using the Gateway API:
...
+
--
Retrieves a process instance with the ID passed as a parameter and launches the update operation on related {PRODUCT} service with the new values passed in `variables`.
...
+
--
Triggers a new node instance with the node definition `nodeId`, which is included in the `nodeInstances` of a process instance. The ID of the process instance is passed as a parameter.

.Example mutation for triggering a node instance
[source]
----
mutation{
  NodeInstanceTrigger(
...
Retriggers a node instance using the ID, which is similar to the `nodeInstanceId`. The `nodeInstanceId` is related to the process instance ID that is passed as a parameter.

.Example mutation for retriggering a node instance
[source]
----
mutation{
...
+
--
Cancels a node instance using the ID, which is similar to the `nodeInstanceId`. The `nodeInstanceId` is related to the process instance ID that is passed as a parameter.

.Example mutation for cancelling a node instance
[source]
----
mutation{
  NodeInstanceCancel(
