...
[id="con-disable-dashboards-metrics-monitoring_{context}"]
==== Disabling Grafana dashboards generation in {PRODUCT}

[role="_abstract"]
You can disable the generation of some default dashboards in Prometheus metrics monitoring using the following properties:
...
. To disable a Grafana dashboard, add the comma-separated list of dashboard identifiers to the following properties:
+
--
.Properties for disabling the Grafana dashboard
[source]
----
kogito.grafana.disabled.operational.dashboards
kogito.grafana.disabled.domain.dashboards
...
[NOTE]
====
If you add spaces between the dashboard identifiers are eliminated as follows:
