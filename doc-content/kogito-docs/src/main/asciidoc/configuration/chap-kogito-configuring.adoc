...
[id="proc-data-index-service-queries_{context}"]
...
[id="proc-data-index-dev-service_{context}"]
=== {PRODUCT} Quarkus Dev Service for Data Index

When using the {PRODUCT} Process Quarkus extension, an ephemeral Data Index service is automatically provisoned when running Quarkus on Development mode. Whenver you're using either one of the following Quarkus extensions, Dev Service will be setup out-of-the-box:
...
This means that once you start your Quarkus project on dev mode, an in-memory instance of the Data Index service is automatically
started in the background. This feature is enabled by https://quarkus.io/guides/dev-services[Quarkus Dev Services], and leverages on https://www.testcontainers.org/[Testcontainers] to start an image of the Data Index service.
The {PRODUCT} Process Quarkus extension will also set up your Quarkus application to automatically replicate any
{PRODUCT} messaging events related to Process Instances and User Tasks into the provisioned Data Index instance.

Once the service is up and running, you can either query the GraphQL
interface directly using `http://localhost:8180/graphql` or via Quarkus Dev UI console: `http://localhost:8080/q/dev/`.

The Data Index GraphQL endpoint will then allow for querying `ProcessInstances` and `UserTaskInstances`, for more details about operations and attributes to query, see {URL_CONFIGURING_KOGITO}#proc-data-index-service-queries_kogito-configuring[_{PRODUCT} Data Index GraphQL queries_]

NOTE: {PRODUCT} Quarkus Dev Service for Data Index does not support indexing for custom types. The GraphQL schema only exposes queries for `ProcessInstances`, `UserTaskInstances`.

It is also possible to share the same Data Index instance across multiple Kogito services during development, this is enabled by default so only one instance is started. This behaviour can be tweaked if you would like to start multiple instances via `quarkus.kogito.devservices.shared` config property.

The Dev Service also allows for further configuration options such as:

- If you want to disable Data Index Dev Service you can use the `quarkus.kogito.devservices.enabled=false` config property.
- To change the port where the Data Index will be running you can use `quarkus.kogito.devservices.port=8180` config property.
- To change the image that is provisioned it can be tweaked via  `quarkus.kogito.devservices.imageName=quay.io/kiegroup/kogito-data-index-ephemeral` config property.
- If you would like to disable sharing the Data Index instance across multiple Quarkus applications, you use `quarkus.kogito.devservices.shared=false` config property.
