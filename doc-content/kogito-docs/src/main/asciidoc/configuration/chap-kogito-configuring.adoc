...
proc-postgresql-persistence-enabling[]
[id="proc-postgresql-persistence-enabling_{context}"]
=== Enabling PostgreSQL persistence for {PRODUCT} services

[role="_abstract"]
For Quarkus-based {PRODUCT} services, as an option for the runtime persistence, you can enable
https://www.postgresql.org/[PostgreSQL]. PostgreSQL is a powerful, open source object-relational database
system that uses and extends the SQL language, it is well established in the marked and provided as a
service (SAAS) by different cloud providers. For {PRODUCT} persistence with PostgreSQL, you must have it installed or
available in order to enable persistence.

.Prerequisites
* https://www.postgresql.org/[PostgreSQL] available, for information about PostgreSQL installation and
configuration, see the https://www.postgresql.org/docs/current/[PostgreSQL documentation].
...
----
--
. Add the following properties to the `src/main/resources/application.properties` file in your {PRODUCT} project to
configure PostgreSQL and to connect to the PostgreSQL server.
+
--
Replace the server host, port, credentials and other information according to your installation.

The `uri` parameter can be customized according to the https://www.postgresql.org/docs/current/libpq-connect
.html#LIBPQ-CONNSTRING/[connection string specification]

The `auto.ddl` indicates the application to run the scripts to create the necessary tables, the user configured to
the application should have the proper permissions to do it, this is convenient for testing and validation but it is
recomended to run the scripts in your infratructure in a more controlled way.

The `millis` is used to control the time the application waits for the reposonse on each executed query to the
database, the default value is shown below.
...
For an example {PRODUCT} service with PostgreSQL persistence, see the following example application in GitHub:

* https://github.com/kiegroup/kogito-examples/tree/stable/process-postgresql-persistence-quarkus[`process-postgresql
-persistence-quarkus`]: Example on Quarkus
* https://github.com/kiegroup/kogito-examples/tree/stable/process-postgresql-persistence-springboot[`process-postgresql
-persistence-springboot`]: Example on SpringBoot
