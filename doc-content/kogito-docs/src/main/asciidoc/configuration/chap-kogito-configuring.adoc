...
|Data-index protobuf generation
|`kogito.persistence.data-index.proto.generation` (default `true`)
a|Disable the generation of proto files used by data-index. Those proto files are only used for domain specific queries when the storage supports it (Infinispan and MongoDB at the moment) , in all other cases they are not necessary and can be skipped

Example: `kogito.persistence.data-index.proto.generation=false` to disable proto file generation

|Kogito-runtime protobuf marshaller generation
|`kogito.persistence.proto.marshaller` (default `true`)
a|Disable the generation of domain specific proto marshaller to serialize process variables. Disabling it, enables the support of many more custom/different types and it leverage Java serialization to persist them

NOTE: Process variable types must implement `java.io.Serializable` when proto marshaller generation is disabled.
