...
// tag::proc-events-mongodb-debezium[]
[id="proc-events-mongodb-debezium_{context}"]
=== Enabling Strong Consistency for Process Data and {PRODUCT} Events with MongoDB and Debezium

[role="_abstract"]
In order to avoid potential data inconsistencies, {PRODUCT} services should atomically update process data and publish events within a unit of work. However, Apache Kafka doesn't support distributed transactions.

A https://debezium.io/blog/2019/02/19/reliable-microservices-data-exchange-with-the-outbox-pattern/[solution] to this problem is to have an `outbox` table in the {PROJECT} service’s database. When storing process data in a unit of work, also insert the records representing the events to be sent into that `outbox` table in the same transaction. Then a separate Message Relay process publishes the events inserted into the `outbox` table to Kafka broker asynchronously.

{PROJECT} supports storing process data and events of the same unit of work in the same transaction with MongoDB.

https://debezium.io/[Debezium] is a good fit for the Message Relay, which comes with `Change Data Capture` connectors for several databases such as MongoDB. It supports capturing new entries in MongoDB collections and streaming them to Apache Kafka.

For this procedure, the {PROJECT} service is configured to store process data and events in the same transaction to MongoDB collections. A Debezium MongoDB connector is configured and started to stream {PROJECT} events from MongoDB collections to Kafka topics.

.Prerequisites
* https://kafka.apache.org/[Apache Kafka] is installed and includes any required topics. For information about Kafka installation and configuration, see the https://kafka.apache.org/documentation/[Apache Kafka documentation].
* https://www.mongodb.com/[MongoDB] version 4.4 or later is installed, started and with `oplog` enabled (e.g. setting up replica set). For information about MongoDB setting up for Debezium, see the https://debezium.io/documentation/reference/connectors/mongodb.html#setting-up-mongodb[MongoDB connector documentation].
...
.Procedure
. Add the following dependencies to the `pom.xml` file of your {PRODUCT} project to enable storing {PRODUCT} events to MongoDB:
+
--
.On Quarkus
[source, xml]
...
. Configure the MongoDB database and collections for storing {PRODUCT} events:
+
--
Add properties to the `src/main/resources/application.properties` file in your {PRODUCT} project to configure the MongoDB database and collections, for example:
[source]
----
...
. Follow xref:proc-mongodb-persistence-enabling_kogito-configuring[instructions] to use MongoDB as runtime persistence, then build your {PRODUCT} project and start the {PRODUCT} service:
+
--
Make sure to enable MongoDB transaction by adding the following property to the `src/main/resources/application.properties` file in your {PRODUCT} project, so that process data and {PRODUCT} events generated in the same unit of work will be stored to MongoDB in the same transaction, which avoids potential data inconsistencies:
[source]
----
...
Use the example configuration below to configure the Debezium MongoDB connector which will capture the {PRODUCT} events from MongoDB collections and stream the events to kafka topics:

Replace the server host, port, replica set, database and collections information according to your MongoDB, kafka installation and {PRODUCT} project configuration.
[source]
----
{
...
For example {PRODUCT} strong consistency with MongoDB and Debezium, see the following example applications in GitHub:

* https://github.com/kiegroup/kogito-examples/tree/main/process-outbox-mongodb-quarkus[`process-outbox-mongodb-quarkus`]: Example on Quarkus
* https://github.com/kiegroup/kogito-examples/tree/main/process-outbox-mongodb-springboot[`process-outbox-mongodb-springboot`]: Example on Spring Boot
// end::proc-events-mongodb-debezium[]
...
.Prerequisites
* https://infinispan.org/[Infinispan Server] 12.1.7 or later is installed and running. For information about Infinispan installation and configuration, see the https://infinispan.org/documentation/[Infinispan documentation].
