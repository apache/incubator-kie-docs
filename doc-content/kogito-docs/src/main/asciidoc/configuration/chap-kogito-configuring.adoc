...
== Monitoring in {PRODUCT} services

The monitoring modules are powered by https://micrometer.io/[Micrometer] and you can export the metrics to any monitoring system supported by Micrometer.
Two modules are provided out of the box for Prometheus and Elastic and can be configured accordingly. In addition to these modules, you can subscribe any micrometer registry using some API exposed by the {PRODUCT} monitoring library. 

[id="proc-custom-micrometer-metrics-monitoring_{context}"]
=== Monitoring with custom micrometer registry in {PRODUCT}

The complete list of supported monitoring system by Micrometer is available on the official https://micrometer.io/docs[Micrometer documentation]. As an example, in this section we demonstrate how you can export metrics to https://micrometer.io/docs/registry/atlas[Atlas]. 

.Procedure
. Depending on the framework you are using in your {PRODUCT} project, add following dependency to the `pom.xml` file:
+
...
----
These modules exposes some API that you can use to subscribe the Micrometer registries. In addition to that, these modules wire automatically some listeners to export some default metrics. In case you'd like to use only some of the default metrics (for example only metrics for rules and not for processes), it is possible to disable each of them individually with the following configuration properties: 
+
.Configuration properties for Monitoring core modules
[id="con-monitoring-core-configuration_{context}"]
[source]
------
...
------
As an alternative, if you are not interested in the default listeners and the default metrics, you can import the core common module directly: 
+
.Add dependency for Monitoring core add-on
[source,xml,subs="+quotes"]
----
<dependency>
  <groupId>org.kie.kogito</groupId>
...
----
. In order to write custom metrics, you can leverage on custom listeners documented {URL_CONFIGURING_KOGITO}#proc-event-listeners-registering_kogito-configuring[here], and then register the micrometer `Meter` object you construct to the global `CompositeMeterRegistry` using the method `getDefaultMeterRegistry` of the class `org.kie.kogito.monitoring.core.common.MonitoringRegistry`.

. Depending on the registry you'd like to use, add its dependency to the `pom.xml`. For example, the Atlas Micrometer registry is the following:
+
.Add dependency for Atlas Micrometer registry
[source,xml,subs="+quotes"]
----
<dependency>
...
. Subscribe your registry using the method `addRegistry` of the class `org.kie.kogito.monitoring.core.common.MonitoringRegistry`. For example, assuming that you are using Quarkus, create the following class in your project `AtlasProvider.java`:
+ 
.Example java class to subscribe a custom micrometer registry to Kogito core monitoring
[source,java]
----
...
----
To enable or disable the export of default metrics, see {URL_CONFIGURING_KOGITO}#con-monitoring-core-configuration_kogito-configuring[monitoring core configuration].
...
[id="proc-grafana-dashboards-metrics-monitoring_{context}"]
==== Grafana dashboards for default metrics in {PRODUCT}

[role="_abstract"]
If any of the Prometheus monitoring modules are imported as dependencies in the `pom.xml` file of your project, some grafana dashboards that use the default metrics are generated under the folder `target/classes/META-INF/resources/monitoring/dashboards/` every time you compile your {PRODUCT} application. 
Two types of dashboards are exported depending on the model used on the endpoints:

. *Operational dashboard*: this dashboard is generated for DMN and DRL endpoints and contains
...
. *Domain specific dashboard*: currently this dashboard is exported only for DMN endpoints. In particular, the domain specific dashboard contains a graph for each type of decision in the DMN model. At the moment, only the built-in types `number`, `string` and `boolean` are supported:
...
These dashboards are ready to be used and can also be customized by the user. At the current release, no dashboards are generated for BPMN resources.

[id="proc-elastic-metrics-monitoring_{context}"]
=== Enabling Elastic metrics monitoring in {PRODUCT}

[role="_abstract"]
Elasticsearch is a distributed, open source search and analytics engine that you can use with {PRODUCT} to collect and store metrics.

.Prerequisites
* Elastic is installed. For information about downloading and using Elastic, see the https://www.elastic.co/guide/index.html[Elastic documentation page].
...
. Edit your `application.properties` files to configure the Elastic addon using the following properties: 

+
.Elastic monitoring addon configuration properties
...
| The host to send metrics to. Default is `http://localhost:9200`.
...
|  Whether to create the index automatically if it doesn't exist. `true` by default.
