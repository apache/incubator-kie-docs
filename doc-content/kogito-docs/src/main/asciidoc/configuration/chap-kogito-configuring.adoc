...
a|Useful to globally disable the REST endpoints generation.
...
a|Useful to disable the REST endpoints generation for a specific resource.

Example: If we have a project with `.dmn` and `.bpmn` files and we want to disable only the decisions' REST endpoints `kogito.generate.decisions=false`
