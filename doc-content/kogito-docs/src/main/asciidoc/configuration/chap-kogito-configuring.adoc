...
.Prerequisites
* https://infinispan.org/[Infinispan Server] 12.1.4 or later is installed and running. For information about Infinispan installation and configuration, see the https://infinispan.org/documentation/[Infinispan documentation].
...
<dependency>
  <groupId>org.kie.kogito</groupId>
  <artifactId>jdbc-persistence-addon</artifactId>
  <version>__{PRODUCT_INIT_CAP}_VERSION__</version>
</dependency>
<dependency>
  <groupId>io.quarkus</groupId>
...
[role="_abstract"]
{PRODUCT} provides a Task Assigning Service that enables you to assign the user tasks created by the {PRODUCT} services to users as part of an optimal solution. This optimal solution is estimated by {PRODUCT_OP}, a lightweight and embeddable planning engine that optimizes planning problems.
...
When a user claims a task, the status of the task is changed to *Reserved* and the task is assigned to the claiming user. When a user claims a task, the user becomes the actual owner of the task and the task appears only on the user's Task Inbox. The following example shows the task information and available actions for `#cbf41` task after the execution of the claim operation:
...
. An optimized plan is estimated and all the tasks including unassigned tasks are assigned based on their configuration such as group condition and {PRODUCT_OP} optimization rules. The resulting optimized plan consists of the following statements:
+
* Tasks `#895ca` and `#cbf41` must be assigned to `user1`
* Tasks `#5aa2c` and `#ddc34` must be assigned to `user2`

. The Task Assigning Service performs the assignments. Note that the users are required to execute the `claim` operation. Therefore, the users focus on completing their assigned tasks, and the assignment is performed by {PRODUCT_OP}.

Overall, the Task Assigning Service analyzes the available tasks, estimates an optimized plan using {PRODUCT_OP}, and generates the necessary assignments in the process services. The users rely on the optimized plan and focus on executing their assigned tasks.

[NOTE]
====
In the previous example, only user tasks configuration is considered for the assignments estimation by {PRODUCT_OP}. However, the assignment estimation can also include the business information related to the user tasks.
====

The {PRODUCT} services that are running create many process instances and user tasks, and at the same time the users complete the assigned tasks. This user task creation and completion cycles repeatedly. In this scenario, the Task Assigning Service instructs the {PRODUCT_OP} to re-estimate the plan and generate new assignments to reflect a new optimized plan. {PRODUCT_OP} consists of the ability to react on the changing context. For more information, see the https://docs.optaplanner.org/latestFinal/optaplanner-docs/html_single/index.html[{PRODUCT_OP} documentation].
...
A simple strategy is to introduce the concept of a planning window, which is a defined subset of the tasks that are available and cannot be changed or reassigned when new plans are estimated. The planning window is only available to the users and the tasks in the planning window are known as published tasks.
...
The unassigned tasks do not belong to the planning window, therefore the unassigned tasks are assigned using {PRODUCT_OP}.
...
You can use the Task Assigning Service to assign the user tasks that are created by the {PRODUCT} service to users using an optimal solution. The optimal solution is estimated by {PRODUCT_OP}.
...
Considering that the Task Assigning Service is part of a Quarkus application, then all the configuration parameters are defined in the next topics. The configuration parameters can be established using the available Quarkus configuration sources.
...
[NOTE]
====
Generally, most of the parameters are pre-configured with appropriate values. However, the `application. properties` file is placed in the `$TASK_ASSIGNING_SERVICE_HOME/config` directory, which can be used to configure the Task Assigning Service parameters.
====
...
| Maximum number of tasks for each user that are assigned in {PRODUCT} runtimes when an optimized plan is estimated.
...
The estimation of an optimal plan for assigning user tasks requires consideration of business information as part of the definitions of the users, for example, using groups, skills related to a user, or using affinities in certain topics in the Task Assigning Service. For more information about using skills and affinities, see see xref:con-skills-affinities_kogito-configuring[].
...
The `PropertiesConnector` is a basic `UserServiceConnector` implementation that loads the user definitions and attributes from the properties, which encountered on the different configuration sources. The configuration sources can be used for the Task Assigning Service Quarkus application, for example, the `applicaion.properties` file.
...
The customization provided by users must include this constraint as a first-level hard constraint, otherwise, the business process semantics is not considered by the task assigning integration. For example, the tasks are assigned to the users who are not Potential Owners.
...
| Required Skills
| Hard constraint 1
| Determines that a task can only be assigned to a user, who has all the configured skills. For more information about skills, see xref:con-skills-affinities_kogito-configuring[]. In case, no user is found with the configured skills of the task, then the task is assigned to the Planning User. However, if the task does not consist of configured skills, the constraint contains no effect.
...
| High level priority
| Soft constraint 1
| Enforces the assignment of higher priority tasks first.

| Desired affinities
| Soft constraint 2
| Assigns the tasks according to the configured affinities. For more information about affinities, see xref:con-skills-affinities_kogito-configuring[]. If a task consists of configured affinities, the task is assigned to the user with most of the related affinities.
...
| Soft constraint 4
| Ensures that medium level priority tasks are assigned after higher priority tasks.

| Low level priority
| Soft constraint 5
| Ensures that low level priority tasks are assigned at last.

|===

[NOTE]
====
The `TaskAssigningSolution` is based on the `BendableLongScore` scoring function, therefore potential users provided a set of constraints are based on the same scoring function. However, the scoring function and the core model classes can be changed in a future release.
====
...
The following shows the implementation of constraint provider used by the default Solver configuration:
...
The skills and affinities in the Task Assigning Service implement the ability to declare business-related information. The business-related information is considered by the default constraints or the customized set of constraints. This process is a decision mechanism that you can use for the group-based assignment semantics defined in the business process to assign the user tasks. Also, this decision mechanism assists to generate the estimated attributes that are automatically defined to the task and user representation, which is used by {PRODUCT_OP}. The calculated attributes are generated by the `TaskAttributesProcessor` and the `UserAttributesProcesor`.
...
The task attributes processors generate task attributes in an internal model. The `TaskAttributesProcessors` are executed when the Task Assigning Service is activated.

.Task attributes processor
image::kogito/configuration/task-assigning-service/TaskAttributesProcessor.png[]

The `TaskAttributesProcessors` transforms user task information into a task attribute in the internal model, which is managed by {PRODUCT_OP}. The default constraints consider the task attributes for Task Assigning Service.
...
The user attributes processors generate user attributes in an internal model. The `UserAttributesProcessors` are executed when information for a user is queries and the Task Assigning Service is activated.

.User attributes processor
image::kogito/configuration/task-assigning-service/UserAttributesProcessor.png[]

The `UserAttributesProcessors` transforms external user information into a user attribute in the internal model, which is managed by {PRODUCT_OP}. The default constraints consider the user attributes for Task Assigning Service.
...
* `DefaultTaskAttributeProcessor`: Processes the user tasks with `skills` and `affinities` input values as a string of comma-separated values and creates Java set objects with tokenized string values for each attribute. The resulting set of values is assigned to the calculated attributes `SKILLS` and `AFFINITIES` respectively.
+
For example, the input values `english, finance` of `skills` is extracted as a set with the {"english", "finance"} values and assigned to the `SKILLS` attribute.
+
...
** `kogito.task-assigning.default-task-attributes-processor.priority` parameter to configure the execution order for the `DefaultTaskAttributeProcessor` processor. The default value of this parameter is `0`.
** `kogito.task-assigning.default-task-attributes-processor.skills` parameter to change the name of the input value of the user task, in which `SKILLS` attribute is calculated. The default value of this parameter is `skills`.
** `kogito.task-assigning.default-task-attributes-processor.affinities` parameter to change the name of the input value of the user, in which `AFFINITIES` attribute is calculated. The default value of this parameter is `affinities`.
...
** `kogito.task-assigning.default-user-attributes-processor.priority` parameter configures the execution order for the `DefaultUserAttributesProcessor` processor. The default value of this parameter is `0`.
** `kogito.task-assigning.default-user-attributes-processor.skills` parameter to change the name of the external user attribute, in which `SKILLS` attribute is calculated. The default value of this parameter is `skills`.
** `kogito.task-assigning.default-user-attributes-processor.affinities` parameter to change the name of the external user attribute, in which `AFFINITIES` attribute is calculated. The default value of this parameter is `affinities`.
...
The Task Assigning Service supports extension elements that extend the content of the Task Assigning Service. To optimize the Task Assigning Service, {PRODUCT} supports the following additional extensions:
...
To optimize the Task Assigning Service, you can use the additional Task Assigning Service extension in {PRODUCT}.
...
To create your custom task attributes processor, implement the `org.kie.kogito.taskassigning.model.processing.TaskAttributesProcessor` as follows:
