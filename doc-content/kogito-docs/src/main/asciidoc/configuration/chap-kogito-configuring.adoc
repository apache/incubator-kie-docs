...
// tag::proc-kafka-persistence-enabling[]
[id="proc-mongodb-persistence-enabling_{context}"]
...
--
. Add following properties to the `src/main/resources/application.properties` file in your {PRODUCT} project to configure Kafka persistence and to connect to the relevant Kafka server.
+
--
Replace the server host, port information according to your Kafka installation.
...
For example {PRODUCT} services with Kafka persistence, see the following example applications in GitHub:
...
// end::proc-kafka-persistence-enabling[]
