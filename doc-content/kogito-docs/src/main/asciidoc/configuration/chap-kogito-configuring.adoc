...
You can share the same Data Index instance across multiple Kogito services during development. Sharing Data Index instances is enabled by default, so only one instance is started. This behaviour can be adjusted to start multiple instances using the `quarkus.kogito.devservices.shared` config property.
