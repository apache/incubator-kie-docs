...
NOTE: https://quarkus.io/guides/flyway[Flyway] is enabled by default in Jobs Service, that according to the `quarkus.datasource.jdbc.url` property it creates/updates the required database schema to run the service, the configured user should have the proper grants to alter the database. In case you need to manage the database without Flyway automation, the property `-Dquarkus.flyway.migrate-at-start=false` should be included when starting the service, which disables Flyway, otherwise, the service may fail during the initialization.
