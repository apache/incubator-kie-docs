...
=== Enabling consistency in process data and {PRODUCT} events using MongoDB and Debezium

[role="_abstract"]
To avoid the data inconsistencies in process services, the {PRODUCT} services must update the process data and publish events within a unit of work. However, Apache Kafka does not support distributed transactions. Therefore, in order to resolve this problem, you can use an outbox table in the database of the {PRODUCT} service.

When you store the data of a process service in a unit of work, you can also add the records that represent the events to be sent into the outbox table related to the same transaction. After the transaction, a separate message relay process publishes the events that are added into the outbox table to Kafka broker asynchronously. For more information about the outbox table, see https://debezium.io/blog/2019/02/19/reliable-microservices-data-exchange-with-the-outbox-pattern/[Debezium blog].
...
{PRODUCT} enables you to store the data of a process and events of the same unit in a single transaction using MongoDB.

For this procedure, a {PRODUCT] service is configured to store the process data and events in a single transaction using MongoDB collection. Also, a Debezium MongoDB connector is configured and started to stream the events from MongoDB collection to Apache Kafka topics.
...
* https://www.mongodb.com/[MongoDB] version 4.4 or later is installed, started, and enabled using `oplog`. For information about MongoDB setting up for Debezium, see the https://debezium.io/documentation/reference/connectors/mongodb.html#setting-up-mongodb[MongoDB connector documentation].
...
. Configure the MongoDB database and collections to store the {PRODUCT} events:
+
--
...
+
--
NOTE: Ensure that you enable MongoDB transaction by adding the following property to the `src/main/resources/application.properties` file in your {PRODUCT} project:

.Dependency to enable MongoDB transaction
[source]
----
kogito.persistence.transaction.enabled=true
----
When you add the previous property to your {PRODUCT} project, the process data and events are generated in the same unit of work and is stored to MongoDB in the same transaction, which avoids potential inconsistencies.
--
...
You can also use the following example configuration to configure the Debezium MongoDB connector, which captures the events from MongoDB collection and stream the events to Kafka topics:
...
You can also use the following example application for consistency in a {PRODUCT} service using MongoDB and Debezium:
