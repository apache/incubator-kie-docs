...
* https://www.oracle.com/database/[*Oracle*]: Persists data in Oracle
...
// tag::proc-oracle-persistence-enabling[]
[id="proc-oracle-persistence-enabling_{context}"]
=== Oracle persistence for {PRODUCT} services

[role="_abstract"]
As an alternative to using Infinispan, MongoDB or PostgreSQL for {PRODUCT} runtime persistence, you can enable persistence for your {PRODUCT} services using https://www.oracle.com/database/[Oracle Database].
For {PRODUCT} persistence with Oracle, you must have Oracle Database installed and running to enable persistence.
...
.Prerequisites
* https://www.oracle.com/database/[Oracle Database] installed.
...
+
--
Replace the server host, port, and database information according to your Oracle Server installation. By default, the database is named `kogito`.
...
// end::proc-oracle-persistence-enabling[]
...
.Prerequisites
* Oracle Database (?? or later) is installed and running.

.Procedure
. Go to the https://repository.jboss.org/org/kie/kogito/data-index-service-oracle/[`data-index-service-oracle`] artifacts page, select the latest release of the Data Index Service, and download the `data-index-service-oracle-__VERSION__-runner.jar` file to a local directory.
