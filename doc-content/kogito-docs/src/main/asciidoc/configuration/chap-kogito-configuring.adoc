...
==== {PRODUCT} Data Index service Gateway API

Data-index incorporates a set of queries/mutations that allow to fire operations on runtime services endpoints using GraphQL notation.

.Data Index - {PRODUCT} services connection
image::kogito/configuration/data-index-gateway-api.png[Image of data-index external points connections]

NOTE: The related operation addon xref:con-bpmn-process-management-addon_kogito-developing-process-services[] needs to be enabled at {PRODUCT} runtime service
to be able to perform the requested operation.

The data-index allows to:

Abort Process Instance::
+
--
Aborts the process instance with the id passed as parameter.  Retrieves the process instance with the id passed as parameter and
launches the 'abort' operation on related {PRODUCT} service.
...
+
--
Retries the process instance with the id passed as parameter.  Retrieves the process instance with the id passed as parameter and
launches the 'retry' operation on related {PRODUCT} service.
...
Skip Process Instance::
+
--
Skips the process instance with the id passed as parameter.  Retrieves the process instance with the id passed as parameter and
launches the 'skip' operation on related {PRODUCT} service.
...
+
--
Updates the variables of the process instance with the id passed as parameter.  Retrieves the process instance with the id passed as parameter and
launches the 'update' operation on related {PRODUCT} service with the new values passed in 'variables'.
...
+
--
Triggers a new node instance with the node definition 'nodeId', that will be included in the nodeInstances of the process
instance with the `id` passed as parameter.
...
+
--
Retriggers the node instance with the id equals to 'nodeInstanceId' belonging to the process instance with the `id` passed as parameter.
...
+
--
Cancel the node instance with the id equals to 'nodeInstanceId' belonging to the process instance with the `id` passed as parameter.
...
Get the Process Instance Diagram::
+
--
Retrieve the Process Instance diagram that shows the execution path. When the field `diagram` of a process instance is query,
a call to the specific {PRODUCT} service is generated to get the requested process diagram.
...
NOTE: The  xref:con-bpmn-process-svg-addon_kogito-developing-process-services[] needs to be enabled too, at {PRODUCT} runtime service
to be able to perform the get process diagram operation.

Get the Process Instance Nodes::
+
--
Retrieve the Process Instance available nodes coming from the process definition.
When the field `nodeDefinitions` of a process instance is query, a call to the specific {PRODUCT} service is generated
to get the requested list of available node definition.
