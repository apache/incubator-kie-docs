...
+
--
Retrieves the User task schema of the user task instance with the `taskId`. The call allows passing `user` and `groups`, even though they are not mandatory.
If they are passed as parameters they will be used to run the call to runtimes.
When the `schema` field of a user task instance is queried, a call to a specific {PRODUCT} service is
generated to retrieve the requested task schema.

.Example query to task instance schema
[source]
----
...
+
--
Replaces all the related data of the user task instance with the `taskId`, by the information passed in `taskInfo` parameter.
This means that if one field comes empty or doesn't come into `taskInfo`, it will be set as empty.
The call allows passing `user` and `groups`, even though they are not mandatory. If they are passed as parameters they will
be used to run the call to runtimes.
...
Update a user task instance partially::
+
--
Updates the related data of the user task instance with the `taskId`, with the information passed in the `taskInfo` parameter.
This means that only the fields passed in `taskInfo` will be affected by the operation.
The call allows passing `user` and `groups`, even though they are not mandatory. If they are passed as parameters
they will be used to run the call to runtimes.

.Example mutation to update a task instance partially
[source]
----
mutation{
    TaskInstancePartialUpdate (
        taskId:"b356d136-e601-43ee-9c2d-ee47198b9303",
        user: "jdoe",
        groups: ["managers", "users", "IT"],
        taskInfo:{
...
Add a user task instance comment::
+
--
Adds a new comment to the user task instance with the `taskId`, with the information passed in the `comment` parameter.
The call allows passing `user` and `groups`, even though they are not mandatory. If they are passed as parameters they
will be used to run the call to runtimes.

.Example mutation to add a new comment to a task instance
[source]
----
mutation{
    TaskInstanceCommentCreate(
        taskId:"b356d136-e601-43ee-9c2d-ee47198b9303",
        user: "jdoe",
        groups: ["managers", "users", "IT"],
...
Add a user task instance attachment::
+
--
Adds a new attachment to the user task instance with the `taskId`, with the information passed in the `name` and `uri` parameter.
The call allows passing `user` and `groups`, even though they are not mandatory. If they are passed as parameters they will
be used to run the call to runtimes.

.Example mutation to add a new attachment to a task instance
[source]
----
mutation{
    TaskInstanceAttachmentCreate(
        taskId:"b356d136-e601-43ee-9c2d-ee47198b9303",
        user: "jdoe",
        groups: ["managers", "users", "IT"],
