...
* *Jobs cache*: Cache for each scheduled job instance. This cache contains all job instance information, including all detailed information related to a specific job such as how many times it was fired and next time it will be fired.

The indexing functionality in the Data Index Service is provided by choosing one of the following persistent providers: https://infinispan.org/[Infinispan], https://www.mongodb.com/[MongoDB] or https://www.postgresql.org/[PostgreSQL]. 
When using Infinispan, indexing is provided by https://lucene.apache.org/[Apache Lucene], and the communication between the Data Index Service and Infinispan is handled through a protobuf (https://developers.google.com/protocol-buffers/[protocol buffers]) schema and generated marshallers.
...
It is possible to enable/disable the custom types ( domain specific ) indexing. This is achieved by changing the value of the property `kogito.data-index.domain-indexing` to true or false depending on the behaviour you would like to achieve. Domain indexing is enabled by default on Data Index using MongoDB and Infinispan persistence type.
...
The {PRODUCT} Data Index Service supports GraphQL queries for domain specific types (domain cache) and for process instances, task instances and jobs.
...
NOTE: Data Index Service with PostgreSQL does not support indexing domain specific data. The GraphQL schema will only expose queries for `ProcessInstances`, `UserTaskInstances` and `Jobs`.
...
The {PRODUCT} Jobs Service also supports Infinispan or PostgreSQL persistence that you can enable when you run the Jobs Service so that job data is preserved across application restarts.
