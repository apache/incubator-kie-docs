...
.. In the upper-right corner of the DMN modeler, click the *Properties* icon and confirm that *Name* is `PersonDecisions`
...
.Example DMN data types
image::kogito/creating-running/kogito-dmn-example-person-data-types.png[Image of person DMN data types]
.. Click the input node `Person` and open *Properties*. Set *Data type* to `tPerson`.
.. Click the decision node `isAdult` and open *Properties*. Set *Data type* to `boolean`.
...
.. In the upper-right corner of the BPMN modeler, click the *Properties* icon and set *ID* to `persons`.
.. In *Properties*, under *Process Data*, add the following process variables:

* `person` with the type `org.acme.Person`. You can write the custom type by selecting `Custom ...`
...
** *Rule Language*: `DMN`
** *Namespace*: Copy&paste from *Namespace* property of `PersonDecisions.dmn` which you created in the previous step.
