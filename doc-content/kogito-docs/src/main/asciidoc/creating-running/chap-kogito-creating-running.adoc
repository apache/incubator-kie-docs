...
=== {PRODUCT} add-ons
You can find multiple examples demonstarting {PRODUCT} add-ons in the https://github.com/kiegroup/kogito-examples[`{PRODUCT_INIT}-examples`] repository in GitHub. A sample of these include:

* `dmn-event-driven-quarkus`: Demonstrates the capability of the {PRODUCT} Event-Driven Decisions add-on.
* `dmn-event-driven-springboot`: Demonstrates the capability of the {PRODUCT} Event-Driven Decisions add-on.
* `kogito-travel-agency`: Creates a software system for a startup travel agency.
* `process-mongodb-persistence-quarkus` is a quickstart project that processes deals for travellers.
* `process-quarkus-example`: A process service for ordering items, as a sequence of a script task (writing out some debug info) and a call activity invoking a sub-process, using a custom `Order` data element.
* `process-usertasks-with-security-oidc-quarkus-with-console`: A quickstart project shows very typical user task orchestration
* `process-mongodb-persistence-springboot`: A quickstart project that processes deals for travellers using MongoDb. 
* `process-springboot-example`: A simple process service for ordering items using Spring Boot.
* `process-mongodb-persistence-springboot`: A quickstart project that processes deals for travellers.
* `process-springboot-example`: A simple process service for ordering items,
* `process-usertasks-timer-quarkus-with-console`: A quickstart showcasing a basic implementation of a hiring process.
* `dmn-drools-quarkus-metrics`: Demonstrates enabling and consuming the runtime metrics monitoring feature.
* `dmn-tracing-quarkus`: A  DMN service to evaluate a loan approval and generate tracing events that might be consumed by the Trusty service.
* `onboarding-example`: An example illustrating how you can build a complete onboarding solution by combining multiple services (based on business processes and decisions).
...
// Add-Ons
include::../addons/chap-kogito-addons.adoc[leveloffset=+1]
