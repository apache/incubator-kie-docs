...
=== Customizing the {PRODUCT} Spring Boot project creation

On Spring Boot, it's possible to customize the final Maven project generated by the archetype. You can add any {PRODUCT} Spring Boot Starter or {PRODUCT} Add-On during the project creation.

For example, to generate a project based on {PRODUCT} process services you can run the following command with the property `starters`:
...
The final project will include only the dependencies needed to run a {PRODUCT} process services. You can combine multiple {PRODUCT} Spring Boot Starters using a comma separated list. For example `starters=processes,rules`. The available starters are:

////
@comment: Can we add a link to the section below? "Spring Boot Starters"?
////
- `decisions`
- `processes`
- `rules`
...
Similarly, you can add any available {PRODUCT} Add-On using the property `addons`. For example, to have Prometheus Monitoring and Infinispan Persistence you can run:
...
////
@comment: Maybe bringing this table to the docs see https://github.com/kiegroup/kogito-runtimes/pull/1463? In productized version this table will differ
I think we need to bring the whole Add-Ons docs from this PR to the docs. Maybe a top-level Kogito Add-Ons?
////

For a list of valid {PRODUCT} Add-Ons check the https://github.com/kiegroup/kogito-runtimes/tree/{COMMUNITY_VERSION_FINAL}/springboot/addons[following table].

NOTE: when passing the add-on to the property, the name doesn't require the suffix `kogito-addons-springboot`.

As shown in the example above, you can mix the `addons` and `starters` property to highly customize your project creation.

[id="proc-spring-boot-starters_{context}"]
==== {PRODUCT} Spring Boot Starters

https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#using-boot-starter[Spring Boot Starters] are a set of convenient dependency descriptors that you can include in your {PRODUCT} application. You get a one-stop-shop for all the Spring and {PRODUCT} technology that you need without having to hunt through sample code and copy paste loads of dependency descriptors.

If you created your project using the commands in the sections above, your `pom.xml` (dependency descriptor) should have all the necessary starters. You can include a new starter any time you want. Please see the following sections.

[id="proc-kogito-spring-boot-starter_{context}"]
===== {PRODUCT} Spring Boot Starter

The {PRODUCT} Spring Boot Starter is an all-in-one descriptor for projects that needs every Business Automation engine provided by {PRODUT}. It includes Decisions, Rules, Process, Predictions and the Serverless Workflow implementation.

If your project has all these assets, or you're just trying {PRODUCT} and want a quick way of getting started, this is the starter you need. For a more granular approach, consider the specific starters (or a combination of them).
...
WARNING: If you are planning to use Serverless Workflow with events, consider adding the `kogito-serverless-workflow-spring-boot-starter` as well to the project. This starter will include CloudEvents support required by Serverless Workflows.

[id="proc-kogito-decisions-spring-boot-starter_{context}"]
...
[id="proc-kogito-predictions-spring-boot-starter_{context}"]
...
[id="proc-kogito-processes-spring-boot-starter_{context}"]
...
[id="proc-kogito-rules-spring-boot-starter_{context}"]
...
[id="proc-kogito-sw-spring-boot-starter_{context}"]
...
NOTE: this starter will auto-include the https://github.com/kiegroup/kogito-runtimes/tree/main/springboot/addons/cloudevents[CloudEvents] and https://github.com/kiegroup/kogito-runtimes/tree/main/springboot/addons/events/kafka[Kafka] add-ons to the project as well for https://github.com/serverlessworkflow/specification/blob/0.6.x/specification.md#Event-Definition[eventing processing].
