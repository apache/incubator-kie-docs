...
NOTE: In the https://github.com/kiegroup/kogito-examples[`{PRODUCT_INIT}-examples`] repository in GitHub, the example applications in the default `stable` branch use the latest version of {PRODUCT}. When running the examples locally, ensure the environment matches the requirements listed in the `README` file in the relevant application folder; that may also require in the local environment making the necessary network ports available, as configured for Quarkus, Spring Boot and Docker compose where applicable.
