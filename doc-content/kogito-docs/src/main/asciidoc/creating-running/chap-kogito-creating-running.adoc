...
* *Infinispan*, *MongoDB* are persistence middleware technologies that {PRODUCT} supports.
...
PROTO file (`.proto`)::
A data library used for marshalling Java objects in protobuf (https://developers.google.com/protocol-buffers/[protocol buffers]) format. {PRODUCT} runtime persistence and communication with Infinispan/MongoDB are handled through a protobuf schema and generated marshallers.
+
[role="_additional-resources"]
.Additional resources
* {URL_CONFIGURING_KOGITO}#con-persistence_kogito-configuring[_Persistence in {PRODUCT}_]
