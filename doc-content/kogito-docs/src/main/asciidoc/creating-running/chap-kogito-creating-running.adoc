...
NOTE: The {PRODUCT} Serverless Workflow Spring Boot starter includes the https://github.com/kiegroup/kogito-runtimes/tree/main/springboot/addons/cloudevents[CloudEvents] and https://github.com/kiegroup/kogito-runtimes/tree/main/springboot/addons/events/kafka[Kafka] add-ons to the project for https://github.com/serverlessworkflow/specification/blob/0.6.x/specification.md#Event-Definition[eventing processing].
...
To configure messaging capabilities for your Kogito services, you can add the messaging add-on as a dependency in the pom.xml file of your Kogito project:
...
You can set up a channel property using the following form:

`mp.messaging.[incoming|outgoing].<channel name>.<property name>= <property value>`

NOTE: If defined property is not found, then the topic name is considered to be the same as Kafka name.

When Smallrye identifies a property starting with `mp.messaging.[incoming|outgoing].<channel name>`, it creates a channel. You can use the following approaches to define channels in your application:

* Define one incoming channel to receive all the incoming messages and one outgoing channel to store all the published messages.
* Define a channel for each message name so that every message type has a dedicated channel.
* Define an incoming channel, outgoing channel, and a channel for each message name. In this case, each channel can be used for its message. However, if a message name does not have a channel, the message can be stored in the incoming or outgoing channel.
...
In the {PRODUCT} messaging add-on, you can map the topics to messages using the message name and you can define the default topics to use for all the unmapped message names. To map a message name to a particular topic, define the related property in the following form:

`kogito.addon.cloudevents.kafka.[kogito_incoming_stream|kogito_outgoing_stream].<message name> = <topic name>.`

In case the property is not found, the default topic name is used. You can change the default incoming and outgoing topic name as follows:
...
==== Auto-generate Knative Resources

The Knative Eventing add-on is also capable to generate Knative Eventing resources by inspecting either your BPMN or Serverless Workflow files for event definitions. Please see the section [Knative Eventing in Kogito services] <!-- TODO: asciidoc link needed --> to understand how to create BPMN files with message trigger events. To know more about Serverless Workflows events, see the section [Orchestrating microservices with Serverless Workflow in Kogito] <!-- TODO: asciidoc link needed -->.

To activate this feature, add the following dependency to your project:
...
After having this dependency added to the project, the add-on will generate Knative Eventing resources to handle the events produced or consumed by your BPMN or Serverless Workflow resources on Knative Eventing platform.

For each consumed event, the add-on will generate a https://knative.dev/docs/eventing/broker/triggers/[Knative Eventing Trigger], for example:
...
This `Trigger` requires a subscriber, which is the current {PRODUCT} service to receive the event routed by the `Broker`. In this case, the add-on will use the {PRODUCT} Service target deployment configured by the Quarkus Kubernetes to be the subscriber. 

If a `Broker` is not specified, the add-on will generate a default, in-memory, one:
...
If you have a `Broker` already configured in the target namespace, you can specify it via application properties. For example: `org.kie.kogito.addons.knative.eventing.broker=myKafkaBroker`.

If your {PRODUCT} Service has any produced event definitions, the add-on will generate a https://knative.dev/development/eventing/custom-event-source/sinkbinding/[Knative Eventing `SinkBinding`], or a https://github.com/knative-sandbox/eventing-kogito#kogito-source[Knative Eventing `KogitoSource`]. By default, the `SinkBinding` is generated, but you can change this behavior by setting the property `org.kie.kogito.addons.knative.eventing.generate-kogito-source` to `true`.
...
Either for `SinkBinding` and `KogitoSource`, the subject will be the {PRODUCT} target deployment as defined in the Quarkus Kubernetes plugin.

You can configure the `sink` used by the generated resources using the application properties. By default, the generated default `Broker` is defined as the `sink`. 

The following table lists all the available properties for this add-on.
...
|org.kie.kogito.addons.knative.eventing.sink.namespace   |The Sink Namespace                                           |default
|org.kie.kogito.addons.knative.eventing.sink.name        |The Sink Name                                                |my-sink
|org.kie.kogito.addons.knative.eventing.sink.api-version |The Kubernetes API Versionn of the Sink                      |eventing.knative.dev/v1
|org.kie.kogito.addons.knative.eventing.sink.kind        |The Kubernetes Resource Kind of the Sink                     |Broker
|org.kie.kogito.addons.knative.eventing.broker           |Name of the predefined Broker                                |myKafkaBroker
|org.kie.kogito.addons.knative.eventing.generate-kogito-source |Wheter to generate KogitoSource instead of SinkBinding |`false`
|org.kie.kogito.addons.knative.eventing.auto-generate-broker   |Wheter to generate the default, in-memory Broker       |`true`
|===

See the following examples to see the add-on used in context:
