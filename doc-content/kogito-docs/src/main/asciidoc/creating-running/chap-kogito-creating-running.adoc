...
== Creating custom REST endpoints using the Kogito API (Incubation)
...
NOTE: The Kogito API is only available for Quarkus. The Kogito API is in the incubation stage and experimental. It is made available to early adopters with the goal of gathering feedback and improving the feature.

.Procedure
. Create a {PRODUCT} project using the Maven plugin or Quarkus CLI.
+
--
.Example project creation using the Quarkus Maven plugin
[source,subs="attributes+"]
----
mvn io.quarkus:quarkus-maven-plugin:create \
...
. Add a business asset to the `resource` folder such as a DMN file. To create the example DMN model, you can copy the following `PersonDecisions.dmn` file content:
+
--
.Example DMN file
[source,xml]
...
----
--
* In this REST endpoint you are injecting the `AppRoot` object.  You can use the `AppRoot` object to create identifiers for resources that are to be evaluated.
* The line ` @Inject DecisionService svc` injects the `DecisionService` service that you can use to evaluate a DMN. There are different types of services, and each service resolves and evaluates a specific type of identifier.
...
. Optionally, you can disable automated generation of the REST endpoint by setting `kogito.generate.rest.decisions` to `false` in the `application.properties` file.
...
----
--
The response is displayed, for example:
+
--
.Response
...
You can simplify and customize the example REST endpoint further. For example, you can avoid wrapping Map, and instead expect a `DataContext` in the method signature.
...
In addition, you can transform the Map data context into a class. This mapping is done automatically using field naming conventions, for example:
...
The `DataContext` interface marks your class as a bindable context. You are now able to convert another `DataContext` using the `as(Class<DataContext>)` method:
...
or use it directly as an argument
...
A `DataContext` is a class that implements the `DataContext` interface. 

It provides a method `<T extends DataContext> T as(Class<T>)` to convert one type of DataContext into another. The `DataContext` root type only provides the `as()` method, as the root type provides no extra behavior of its own.
