...
* `process-usertasks-quarkus-with-console` and `process-usertasks-springboot-with-console`: Process with User Tasks, showing whole lifecycle interaction using {PRODUCT} Management and Task consoles.
...
=== Add {PRODUCT} Consoles interaction

To get started quickly with {PRODUCT} Consoles interaction to run processes we have some examples described in the https://github.com/kiegroup/kogito-examples[`kogito-examples`] repository in GitHub
ended `with-console` that allow to experiment with the {PRODUCT} services and {PRODUCT} Consoles.
The consoles need to interact with the {PRODUCT} Data Index service with a minimal persistence configuration that is brought by a docker-compose configuration.

These examples provide **docker-compose** configuration to easily configure and start the needed services: Kafka, Infinispan, {PRODUCT} Data Index, {PRODUCT} Management Console and {PRODUCT} Task Console.

.Default services started by docker-compose 'with-console' examples
...
NOTE: In each example the docker-compose starts the needed services to run the project. It configures extra services like
Keycloak (process-usertasks-with-security-oidc-PLATFORM-with-console)or {PRODUCT} Jobs (process-usertasks-timer-quarkus-with-console)
...
. Compile the project to generate the persistence file descriptor that will be used by docker-compose
. From docker-compose folder execute ./startServices.sh to start the containers with the services needed in each case.
. Run the example, using one of the commands described in {URL_CREATING_RUNNING}#chap-kogito-creating-running[_{CREATING_RUNNING}_].
. After the {PRODUCT} service is running, use a REST client, curl utility, or the Swagger UI configured for the application to start a new Request
. Use the {PRODUCT} Management console to see the created process instance available by default at http://localhost:8280
. Use the {PRODUCT} Task console available by default at http://localhost:8380 to execute the user tasks. More detailed information can be found in each specific example.

NOTE: The configuration of the containers can be found in docker-compose/docker-compose.yml in case some ports needs to be changed.
