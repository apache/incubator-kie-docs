...
== Creating custom REST endpoint using the Kogito API (Incubation)

[role="_abstract"]
As a developer of business processes and decisions, you can use {PRODUCT} to develop custom REST endpoints.

.Prerequisites
* JDK 11 or later is installed.
* Apache Maven is installed.

NOTE: The Kogito API is currently only available for Quarkus and it is in in **incubation** stage. It means that it is experimental, but is made available to early adopters with the goal of gathering feedback and improving the feature.
...
. Add a business asset to the `resource` folder. In this example we will use a DMN file. To create this example DMN model quickly, you can copy the following `PersonDecisions.dmn` file content:
+
--
.Example DMN file
[source,xml]
...
----
--
This defines a DMN model with namespace `https://kiegroup.org/dmn/_52CEF9FD-9943-4A89-96D5-6F66810CA4C1` and name `PersonDecisions`


. Create a source file for your custom REST endpoint:
+
--
...
----
--
* In this REST endpoint you are injecting the `AppRoot` object. 
The `AppRoot` object allows you to create an identifier for 
a resource that should be evaluated.
* You are also injecting a `DecisionService`. A `DecisionService` allows you to evaluate a DMN. There are multiple types of Services (detailed below); each service is able to resolve and evaluate one specific type of identifier.
* The REST endpoint expects an arbitrary `Map<String, Object>` of values.
* In the body of the REST handler, we convert the `Map` into a `DataContext`. Each service expects a `DataContext` to evaluate together with the identifier.
* The `evaluate(id, ctx)` method returns a `DataContext`. This can be returned directly by the REST handler: the system knows how to handle the response. 
*

. You can also disable the automated generation of the REST endpoint by putting in your `application.properties`
...
. Let us test the REST endpoint by invoking with a 79-years old Person named "Paul":
+
--
...
The REST endpoint above can be further simplified and customized. For instance, it is possible to avoid wrapping the `Map`, and instead expect a context in the method signature:
...
Further, it is possible to transform the Map data context into a class. The mapping will be made automatically using field naming conventions. For instance:
...
The `DataContext` interface will mark your class as a bindable context. You are now able to convert another `DataContext` using the `as(Class<DataContext>)` method:
...
or even use it directly as an argument
...
It is also possible to use custom data contexts in the return type:
...
A `DataContext` is simply a class that implements the `DataContext` interface. 

It usually provide a method `<T extends DataContext> T as(Class<T>)` to convert one type of DataContext into another. The `DataContext` root type only provides the `as()` method, as the root type provides no extra behavior of its own.

For instance, in the previous code we converted an "opaque" `DataContext` into a `MapDataContext` and a `Person` object.
...
`MapDataContext` is the only type of built-in data context. It behaves like a `Map<String, Object>`, and provides, `get()`, `set()` methods to access to data. Additionally the `get()` method allows to pull typed data, by performing an internal conversion (if applicable).
...
=== Types of Services ===
There are currently four types of services:
...
Each of these services can be `@Inject`ed and provide an `evaluate(Id, DataContext)` method.
For further information on how to invoke each of these service, refer to the examples:
