...
* `process-decision-quarkus`: A process service integrated with decisions services, using an embedded approach through Business Rule Tasks for communication. The decisions are declared with DMN and DRL.
* `process-decision-rest-quarkus`: Similar to the `process-decision-quarkus` process service but using an REST APIs for a remote communication.
