...
For example, to use Kafka as an event broker, you can add the following dependency in the `pom.xml` file of your {PRODUCT} project:
...
* Define incoming channel, outgoing channel, and a channel for each message name. In this case, each channel can be used for its message. However, if a message name does not have a channel, then the message can be stored in the incoming or outgoing channel.

In the {PRODUCT} messaging add-on, you can map the topics to messages using the message name and you can define the default topics to be used for all the unmapped message names. To map a message name to a particular topic, define the related property in the following form:
