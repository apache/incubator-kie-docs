...
To configure messaging capabilities for your Kogito services, you should add the messaging add-on as a dependency in the pom.xml file of your Kogito project:
...
For example, to use Kafka as an event broker in Quarkus, you should add the following dependency in the `pom.xml` file of your {PRODUCT} project:
...
Smallrye uses an abstraction named channel, which real implementation depends on underlying the event broker. For example, when using Kakfa, every channel is mapped to a topic. 

Channels are configured through properties. The pattern for these properties is `mp.messaging.[incoming|outgoing].<channel name>.<property_name>`.
...
* Define a channel for certain message names and let the non mapped message names to use the default incoming or outgoing channel. 

The name for default incoming channel is `kogito_incoming_stream` and for default outgoing channel is `kogito_outgoing_stream`. You can change the default incoming and outgoing topic name as follows:
...
NOTE: If topic property is not found, then the topic name is considered to be the same as Kafka name.

==== Providing a key for a kafka record

In order to specify the key to your kafka record, you can use a custom https://github.com/kiegroup/kogito-runtimes/blob/main/quarkus/addons/messaging/common/src/main/java/org/kie/kogito/addon/quarkus/messaging/common/message/MessageDecorator.java[MessageDecorator] implementation. 

You can find an example https://github.com/kiegroup/kogito-examples/blob/main/kogito-quarkus-examples/process-kafka-avro-multi-quarkus/src/main/java/org/acme/travel/StringKeyDecorator.java[here]

==== Customizing message format

By default, Kogito uses Cloud event json format for messaging. You can change the default format by providing your our own https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-events-api/src/main/java/org/kie/kogito/event/EventMarshaller.java[event marshaler] implementation. 
You can find an example https://github.com/kiegroup/kogito-examples/blob/main/kogito-quarkus-examples/process-kafka-avro-multi-quarkus/src/main/java/org/acme/travel/AvroEventMarshaller.java[here]
