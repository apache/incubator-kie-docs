...
Smallrye uses an abstraction named channel, which real implementation depends on the underlying event broker. For example, when using Kakfa, every channel is mapped to a topic. 
...
The name for default incoming channel is `kogito_incoming_stream` and for default outgoing channel is `kogito_outgoing_stream`. 
...
NOTE: If the topic property is not found, then the topic name is considered to be the same as Kafka name.
...
By default, Kogito uses Cloud event json format for messaging. You can change the default format by providing your own https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-events-api/src/main/java/org/kie/kogito/event/EventMarshaller.java[event marshaler] implementation. 
