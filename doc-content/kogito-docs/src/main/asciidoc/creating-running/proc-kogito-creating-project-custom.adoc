...
* `org.kie.kogito:kogito-maven-plugin:generateModel`: Generates the code.
* `kogito.codegen.sources.directory`: Defines the target directory where you want the code to be generated, such as `src/main/java` in this example. In the automatic {PRODUCT} project build, the default directory is `target/generated-sources/kogito`.
