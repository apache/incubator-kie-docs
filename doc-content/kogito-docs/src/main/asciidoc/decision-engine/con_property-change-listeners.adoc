...
By default, the {DECISION_ENGINE} does not re-evaluate all fact patterns for fact types each time a rule is triggered, but instead reacts only to modified properties that are constrained or bound inside a given pattern. For example, if a rule calls `modify()` on a fact as part of the rule actions, this modification does not trigger automatically the re-evaluation of all patterns referring to the modified type, but only of the patterns constrained on the actually changed properties of that fact. This property reactivity behavior prevents unwanted recursions and results in more efficient rule evaluation. This behavior also mitigates the need of using the `no-loop` rule attribute to avoid infinite recursion.
...
You can update the `drools.propertySpecific` system property in the `application.conf` file of your {PRODUCT} distribution:
...
//@comment I am not entirely sure how this works with OOPath -- evacchi
