...
This in turn will allow more control over what is executed in sequence and improve overall manageability of process execution. http://mswiderski.blogspot.com/2015/04/asynchronous-continuation-in-jbpm-63.html[Here] you can read an article describing this in detail.
* Signal scopes
+
Version 6.3 comes with improved support for signaling process instances.
Based on concepts of signals defined in BPMN2 jBPM provides additional characteristic to them - the scope.
Scope defines how to propagate the signal:
+
** process instance scope - signals only elements within the same process instance, other process instances won't be affected
...
* Milestones
+
You can define milestones as part of the case definition (or even dynamically) and keep track of which milestones were reached for specific case instances.
...
[source]
----
{server}/business-central/rest/runtime/{deploymentId}/process/{processDefId}/image
----
+
...
[source]
----
{server}/business-central/rest/runtime/{deploymentId}/process/{processDefId}/image/{procInstId}
----
+
...
Though it still provides the same set of capabilities:
...
This delay can be given as time expressions that will be calculated from current timestamp.
...
In case number of retry delays is smaller than the number of retries it will use the last available value
...
In 6.3 this execution server has been enhanced and now support for rules and process (including user tasks and asynchronous jobs). It provides a lightweight mechanism for executing your business assets.
...
This feature will be added to the task list as well in future versions.
