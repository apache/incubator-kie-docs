...
jBPM focuses on executable business processes, which are business processes that contain enough detail so they can actually be executed on a BPM {PROCESS_ENGINE}.
...
On top of the {PROCESS_ENGINE}, a lot of features and tools are offered to  support business processes throughout their entire life cycle:
...
* Case management capabilities added to the {PROCESS_ENGINE} to support more adaptive and flexible use cases
...
* All combined in one web-based {CENTRAL} application, supporting the complete BPM life cycle:
+
...
* Remote API to {PROCESS_ENGINE} as a service (REST, JMS, Remote Java API)
...
jBPM is not just an isolated {PROCESS_ENGINE}.
