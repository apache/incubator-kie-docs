...
Dashbuilder Runtime WAR can be installed in latest Widlfly release, just copy the WAR in `standalone/deployments` and start Widlfly.

To login create an user with `admin` role and access context root for the Widlfly location (e.g. http://localhost:8080). By default Dashbuilder Runtime is installed on root context, this can be modified by changin `dashbuilder-runtime.war/WEB-INF/jboss-web.xml`.

After login users may upload dashboard files which were exported from {CENTRAL}

image::BAM/DashbuilderRuntimeWelcomePage.png[]

== Exporting dashboards

In {CENTRAL} it is possible to author datasets, pages and organize the pages navigation to create business dashboards. Once dashboards are created they can be exported in a ZIP file using Data Transfer utility, which can be accessed using *Admin -> Data Transfer*

image::BAM/DataTransferPage.png[]

It is possible to export all pages and datasets from {CENTRAL} by using "Export All" button or select what assets to export using Gradual Export.
...
* *SINGLE*: This is the default mode. When a dashboard is uploaded then it will be used in Runtime. It is also possible to refer on dashboards under the directory configured usying system property `dashbuilder.import.base.dir`, default value is `/tmp/dashbuilder/models`. 
If a file is in model directory, then it is possible to load id using `RUNTIME_URL?import=FILE_NAME`. For example, the file sales_dashboard.zip, places in models directory, can be loaded using `RUNTIME_URL?import=sales_dashboard`(notice that the extension is not used).
...
Dashboards running in Dashbuilder Runtime can be embedded in third part applications using an iframe. To embedded dashboards the query parameter *standalone* should be added to the URL along with the dashboard page name using the query param *perspective*: 
...
Third part applications can interact with Dashbuilder Runtime using its REST API. All endpoints use Basic authentication:
...
....
Server template is a generic way to configure Kie Server for multiple datasets that uses the same template. It is also possible make the same setting by dataset:
....
