...
. With the task node selected, click image:getting-started/diagram_properties.png[Properties icon] to open the *Properties* panel on the right side of the designer.
...
For information about how to change role assignments during case run time, see xref:case-management-modifying-roles-during-runtime-proc-{context}[] or <<case-management-modifying-roles-during-runtime-API-proc-{context}>>.
