...
<2> Notifying the WorkItemManager instance when your work item has been completed is crucial. For
...
[TIP]
====
Remember, if the `WorkItemManager` is not notified about the completion, the  {PROCESS_ENGINE} will never be notified that your service node has completed.
====
