...
image::jbpmImages/DomainSpecificProcesses/NotificationPalette.png[]
...
image::jbpmImages/DomainSpecificProcesses/JavaNode.png[]
...
[TIP]
====
Remember, if the `WorkItemManager` is not notified about the completion, the  {ENGINE} will never be notified that your service node has completed.
====
