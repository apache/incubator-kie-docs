...
In turn, RuntimeEngine encapsulates two the most important elements of {ENGINE}:
...
* good fit for low to medium load on {ENGINE} due to synchronized access
...
* completely isolated {ENGINE} and task service operations for every request
...
----
  ...
  public interface RuntimeEnvironment {
    ...
    /**
     * Delivers custom class loader that shall be used by the {ENGINE} and task service instances
     * @return
     */
...
public interface RuntimeEnvironmentBuilderFactory {
     ...
     * 	<li>DefaultRuntimeEnvironment</li>
     * </ul>
     * but it does not have persistence for {ENGINE} configured so it will only store process instances in memory
     * @return new instance of <code>RuntimeEnvironmentBuilder</code> that is already preconfigured with defaults
     *
     * @see DefaultRuntimeEnvironment
     */
...
Besides KieSession Runtime Manager provides access to TaskService too as integrated component of a RuntimeEngine that will always be configured and ready for communication between {ENGINE} and task service.

Since the default builder was used, it will already come with predefined set of elements that consists of:

* Persistence unit name will be set to org.jbpm.persistence.jpa (for both {ENGINE} and task service)
...
[NOTE]
====
Whenever there is a need to interact with the {ENGINE}/task service from within handler or listener, recommended approach is to use RuntimeManager and retrieve RuntimeEngine (and then KieSession and/or TaskService) from it as that will ensure proper state managed according to strategy
====
