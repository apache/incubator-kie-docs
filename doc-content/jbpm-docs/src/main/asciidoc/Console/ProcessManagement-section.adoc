...
Finally, to complement the process logs you can open the *Diagram* tab that shows the completed activities by default in
grey, the current activities highlighted in blue and the active (related job pending of execution or in error) asynchronous
nodes in red. All this default colors can be configured in *Settings* -> *Process Administration* section
