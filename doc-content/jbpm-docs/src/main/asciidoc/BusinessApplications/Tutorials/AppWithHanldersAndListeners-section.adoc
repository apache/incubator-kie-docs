...
* develop process event listener that will receive events from process engine
...
=== Create custom service task in jBPM Console
...
* annotate the class with `@Component` annotation with name that matches the work ite
defined in the jBPM Console
...
Follow the same procedure as described in
 <<BusinessApplications/Tutorials/AppWithProcesses-section.adoc#_execute_business_process_from_jbpm_console_ui,Execute business process from jBPM Console UI>>

== Summary

Congratulations! you have enhanced your business application to take advantage custom service tasks
...
integrate your application with outside world.
