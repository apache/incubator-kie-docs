...
There is always a default deployment descriptor available, even if it was not explicitly configured, when running in business-process (kie-workbench) the default values are as follows:
...
{CENTRAL} enables configuration of access restrictions to repositories (see <<_wb.userandgroupmgmt,Security management>> chapter for how this can be done).
