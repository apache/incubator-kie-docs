...
If you are however executing in a simple Java environment, you can use the `DataSourceFactory` class from kie-test-util module of drools - see the following code fragment (where we are using the H2 in-memory database in combination with Narayana and Tomcat DBCP in this case). Alternatively, you can use JBPMHelper to do this for you (see below for tests only).
