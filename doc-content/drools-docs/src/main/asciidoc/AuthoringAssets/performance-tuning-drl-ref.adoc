...
+
If you wish `drools-metric` to enable trace logging, configure a logger for `org.drools.metric.util.MetricLogUtils` as shown in the following example:
...
+
Alternatively, you may have `drools-metric` expose the data via https://asciidoctor.org[Micrometer]. 
In order to do that, you need to enable the Micrometer registry of your choice as shown in the following example:
...
* `evalCount` is the number of constraint evaluations against inserted facts during the node execution. When used with Micrometer, a counter with this data is called `org.drools.metric.evaluation.count`.
* `elapsedMicro` is the elapsed time of the node execution in microseconds. When used with Micrometer, look for a timer called `org.drools.metric.elapsed.time`.
--
