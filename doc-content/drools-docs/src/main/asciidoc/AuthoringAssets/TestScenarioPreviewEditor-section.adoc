...
An integral part of the editor is the right panel, which can be used for configuring the data object mappings, for accessing the cheat sheet that contains a set of notes used for quick reference, and for displaying the tests results using the *Test Report* tab.
...
WARNING: In this version of the product, the editor can only use the default KIE session.
