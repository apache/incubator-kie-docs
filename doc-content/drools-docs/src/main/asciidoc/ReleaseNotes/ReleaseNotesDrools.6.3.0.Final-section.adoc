...
Having extended that class, the domain objects can notify the {DECISION_ENGINE} when one of its field has been updated by invoking the inherited method `notifyModification` as in the following example:
