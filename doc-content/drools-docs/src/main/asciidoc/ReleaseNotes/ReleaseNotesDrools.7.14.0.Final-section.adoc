...
You can now use the `kie-maven-plugin` build component to generate DMN executable model classes and compile them in an {PRODUCT} project (`kjar`). This support enables DMN decision table logic in DMN projects to be evaluated more efficiently.
