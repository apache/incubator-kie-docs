...
You can configure the threshold of the alpha node range index using the `drools.alphaNodeRangeIndexThreshold` system property. The `drools.alphaNodeRangeIndexThreshold` system property is used to evaluate the rule constraint. The default value of the threshold is `9`, indicating that the alpha node range index is enabled when a precedent node contains more than nine nodes with inequality constraints. For example, you can have more than nine rules similar to `Person(age > 10)`, `Person(age > 20)`, ..., `Person(age > 90)` .
+
The default value of the threshold is based on the related advantages and overhead. However, if you configure a smaller value for the threshold, then it can improve the performance depending on your rules. For example, you can configure the `drools.alphaNodeRangeIndexThreshold` value to `6`, enabling the alpha node range index when you have only six alpha nodes for a precedent node. You can set a suitable value for the threshold based on your standards.
