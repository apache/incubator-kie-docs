...
When enabled, this property will allow compilation of DMN Decision Table decision logic into Drools Executable Model rules. You can use this property to leverage better performances while evaluating DMN Decision Table(s); if the compilation was not performed during KJAR compile-time, there will be some initial overhead as the engine will make the compilation during the first evaluation.
+
--
Default value: false
