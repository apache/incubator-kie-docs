...
When enabled, this property allow during runtime the compilation of DMN Decision Table decision logic into Drools Executable Model rules.
You can use this property to leverage better performances when evaluating DMN Decision Table(s).
This property is helpful when this compilation was not originally performed during KJAR compile-time; on other hand, as this property enable runtime compilation, it will result in an initial overhead as the engine will make on-the-fly compilation during the first evaluation.
+
--
Default value: false
