...
When you use a REST endpoint of {KIE_SERVER}, the best practice is to define a domain object Plain Old Java Object (POJO) Java class, annotated with standard {KIE_SERVER} marshalling annotations. For example, the following code is using a domain object `Person` class that is annotated properly:
