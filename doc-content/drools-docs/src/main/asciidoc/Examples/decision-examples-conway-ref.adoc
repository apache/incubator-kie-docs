...
ifdef::DM,PAM[]
`~/{PRODUCT_FILE}-sources/src/droolsjbpm-integration-$VERSION/droolsjbpm-integration-examples` of the *Red Hat Process Automation Manager {PRODUCT_VERSION_LONG} Source Distribution* from the https://access.redhat.com/jbossnetwork/restricted/listSoftware.html[Red Hat Customer Portal].
endif::[]
