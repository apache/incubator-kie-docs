...
<2> Create a `KieServices Factory` and a `ks` element to produce the logs because this audit log is not available at the `KieContainer` level.
<3> Modify the `execute` method to use both `KieServices` and `KieContainer`.
<4> Modify the `execute` method to pass in `KieServices` in addition to the `KieContainer`.
<5> Create the listeners.
<6> Build the log that can be passed into the debug view or *Audit View* or your IDE after executing of the rules.
