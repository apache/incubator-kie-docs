...
.EntryPoint
image::droolsImages/UserGuide/EntryPoint.png[align="center"]
...
When a fact is inserted into the session, some of its properties might be immediately evaluated (eager evaluation) and some might be deferred for later evaluation (lazy evaluation). The exact behaviour depends on the {ENGINE} algorithm being used. 
...
The {ENGINE} must be notified of modified facts, so that they can be reprocessed.
...
Alternatively, after changing a fact object's field values through calls of setter methods you must invoke `update` immediately, event before changing another fact, or you will cause problems with the indexing within the {ENGINE}.
...
.RuleRuntime
image::droolsImages/UserGuide/RuleRuntime.png[align="center"]
...
.QueryResults
image::droolsImages/UserGuide/QueryResults.png[align="center"]


.QueryResultsRow
image::droolsImages/UserGuide/QueryResultsRow.png[align="center"]
...
.StatefulRuleSession
image::droolsImages/UserGuide/StatefulRuleSession.png[align="center"]


==== Agenda Filters

.AgendaFilters
image::droolsImages/UserGuide/AgendaFilter.png[align="center"]
...
.Two Phase Execution
image::droolsImages/HybridReasoning/Two_Phase.png[align="center"]
...
.Agenda
image::droolsImages/UserGuide/Agenda.png[align="center"]
...
(The basics to this are covered in chapter "Quick Start".) As firing a rule may have side effects on the working memory, the {ENGINE} needs to know in what order the rules should fire (for instance, firing ruleA may cause ruleB to be removed from the agenda).
...
.AgendaGroup
image::droolsImages/UserGuide/AgendaGroup.png[align="center"]
...
You can switch the group which has focus either from within the {ENGINE}, or via the API.
...
.ActivationGroup
image::droolsImages/UserGuide/ActivationGroup.png[align="center"]
...
.RuleFlowGroup
image::droolsImages/UserGuide/RuleFlowGroup.png[align="center"]
...
image::droolsImages/UserGuide/WorkingMemoryEventManager.png[align="center"]
...
.StatelessKieSession
image::droolsImages/UserGuide/StatelessKieSession.png[align="center"]
...
As a general guideline, Passive Mode is most suitable for {ENGINE} applications which need to explicitly control when the engine shall evaluate and fire the rules, or for CEP applications making use of the Pseudo Clock. Active Mode is most effective for {ENGINE} applications which delegate control of when rules are evaluated and fired to the engine, or for typical CEP application making use of the Real Time Clock.
