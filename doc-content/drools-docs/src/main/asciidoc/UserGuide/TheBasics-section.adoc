...
To make the {DECISION_ENGINE} aware of data, so it can be processed against the rules, we have to _insert_ the data, much like with a database.
When the Applicant instance is inserted into the {DECISION_ENGINE} it is evaluated against the constraints of the rules, in this case just two constraints for one rule.
...
At this point the {DECISION_ENGINE} has done all of its matching, but no rules have fired yet.
...
With the fires now in the {DECISION_ENGINE}, once `fireAllRules()` is called, the alarm is raised and the respective sprinklers are turned on.
...
* Rules execute by matching against any data as long it is inserted into the {DECISION_ENGINE}.
