...
[NOTE]
====
Constraint binding takes count only the first atomic expression following it. So for example in the following pattern:
...
only binds the age of the person to the variable `$a`

For clearer and more efficient rule definitions it is suggested that you separate constraint bindings and constraint expressions. Although mixed bindings and expressions are supported, they can complicate patterns and affect evaluation efficiency.
...
In the former example, in case you want to bound to the variable `$a` the double of the person's age, you need to make it an atomic expression by wrapping it in parentheses like in:
