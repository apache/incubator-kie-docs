...
You can customize this by providing an alternative date format mask as the system property named ``drools.dateformat``. This system property can contain also time format mask part (e.g.``drools.dateformat="dd-MMM-yyyy HH:mm"``).
Other possibility to customize date format is to change language locale by ``drools.defaultlanguage`` and ``drools.defaultcountry`` properties (e.g. locale of Thailand set as ``drools.defaultlanguage=th`` and ``drools.defaultcountry=TH``).
