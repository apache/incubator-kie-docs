...
Just to mention one example, if a rule reasons over the average price of a given stock over the last 60 minutes, how the {DECISION_ENGINE} knows what stock price changes happened over the last 60 minutes in order to calculate the average? The obvious response is: by comparing the timestamp of the events with the "current time". How the {DECISION_ENGINE} knows what **time is now**? Again, obviously, by querying the Session Clock.

The session clock implements a strategy pattern, allowing different types of clocks to be plugged and used by the {DECISION_ENGINE}.
This is very important because the {DECISION_ENGINE} may be running in an elements of different scenarios that may require different clock implementations.
...
* *Regular execution:* usually, when running rules in production, the application will require a real time clock that allows the {DECISION_ENGINE} to react immediately to the time progression.
