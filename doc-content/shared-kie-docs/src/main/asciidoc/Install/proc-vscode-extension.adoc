...
IMPORTANT: The editors in the VSCode are partially compatible with the editors in {CENTRAL}, and several {CENTRAL} features are not supported in the VSCode.
...
If the DMN, BPMN, or test scenario modelers open only the XML source of a DMN, BPMN, or test scenario file and display an error message, review the reported errors and the model file to ensure that all elements are correctly defined.
