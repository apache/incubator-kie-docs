...
The {PRODUCT} provides standalone editors that are distributed in a self-contained library providing an all-in-one JavaScript file for each editor. The JavaScript file uses a comprehensive API to set and control the editor.
...
.. Go to the https://www.npmjs.com/package/@kogito-tooling/kie-editors-standalone[KIE standalone editors repository] in NPM and download the NPM package.
