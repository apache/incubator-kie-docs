...
|`IGNORE`
|While using JSON unmarshalling, when a class in a payload is not found, the behavior can be changed by the property. If the property is set to `IGNORE`, the payload is converted to a HashMap. If the property is set to `WARN`, the payload is converted to a HashMap and a warning is logged. If the property is set to `EXCEPTION`, {KIE_SERVER} throws an Exception.
