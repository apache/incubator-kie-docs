...
ifdef::PAM[]
download the *{PRODUCT} {PRODUCT_VERSION} - Maven Repository* from the {PRODUCT_DOWNLOAD_LINK}[IBM Business Automation Manager Open Editions {ENTERPRISE_VERSION} downloads] page  and navigate to `~/jboss-{PRODUCT_FILE_BA}.GA-maven-repository/maven-repository/org/jbpm/jbpm-services-ejb-*`.
endif::[]
ifdef::JBPM[]
