...
ifdef::PAM[]
download the *Red Hat Process Automation Manager {PRODUCT_VERSION_LONG} Maven Repository* from the https://access.redhat.com/jbossnetwork/restricted/listSoftware.html[Red Hat Customer Portal] and navigate to `~/jboss-{PRODUCT_FILE_BA}.GA-maven-repository/maven-repository/org/jbpm/jbpm-services-ejb-*`.
endif::[]
ifdef::JBPM[]
