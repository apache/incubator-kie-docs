...
+
If you are deploying {PRODUCT} on {OPENSHIFT}, create a custom {KIE_SERVER} image and add this JAR file to the image. For more information about creating a custom {KIE_SERVER} image with an additional JAR file, see {URL_DEPLOYING_OPENSHIFT_OPERATOR}#customimage-jar-proc_openshift-operator[_{DEPLOYING_OPENSHIFT_OPERATOR}_].
+
endif::[]
