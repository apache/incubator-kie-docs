...
You can configure your {KIE_SERVER} deployment on {OPENSHIFT} to use Prometheus to collect and store metrics related to your business asset activity in {PRODUCT}. By default, the `PROMETHEUS_SERVER_EXT_DISABLED` OpenShift template parameter is set to `false` to enable the Prometheus extension for {KIE_SERVER} deployments on OpenShift. For the list of available metrics that {KIE_SERVER} exposes with Prometheus,
ifdef::DM,PAM[]
...
.Procedure
//Deemed unneeded for now. (Stetson, 23 May 2019)
//. In the OpenShift template that you are using for your OpenShift deployment (for example, `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-immutable-kieserver.yaml`), set the `PROMETHEUS_SERVER_EXT_DISABLED` template parameter to `false` to enable the Prometheus extension.
