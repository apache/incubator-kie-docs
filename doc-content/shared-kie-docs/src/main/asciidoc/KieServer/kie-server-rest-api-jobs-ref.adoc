...
NOTE: For a list of supported job commands in {PRODUCT}, download and extract the *{PRODUCT} {PRODUCT_VERSION} - Source Distribution* from the https://www.ibm.com/support/pages/node/6596913[IBM Business Automation Manager Open Editions {ENTERPRISE_VERSION} downloads] page  and navigate to `~/jbpm-$VERSION/jbpm-services/jbpm-executor/src/main/java/org/jbpm/executor/commands`.
