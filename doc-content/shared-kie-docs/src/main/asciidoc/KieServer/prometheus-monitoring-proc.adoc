...
. In the `pom.xml` file of your Maven project, add the following dependencies:
+
...
----
. On Spring Boot (if applicable), also configure the following key in the `application.properties` system property:
+
[source,xml]
----
ifdef::PAM,JBPM[]
...
----
. In the `prometheus.yaml` file of your Prometheus distribution, add the following settings to configure Prometheus to scrape metrics from {KIE_SERVER}:
+
--
[source,yaml]
----
...
After you start the configured {KIE_SERVER} instance, Prometheus begins collecting metrics and {KIE_SERVER} publishes the metrics to the REST API endpoint `\http://HOST:PORT/kie-server/services/rest/metrics`.
--
. In a REST client or curl utility, send a REST API request with the following components to verify that Prometheus is publishing metrics to {KIE_SERVER}:
+
--
For REST client:
...
* *URL*: Enter the {KIE_SERVER} REST API base URL and metrics endpoint, such as `\http://localhost:8080/kie-server/services/rest/metrics`.
