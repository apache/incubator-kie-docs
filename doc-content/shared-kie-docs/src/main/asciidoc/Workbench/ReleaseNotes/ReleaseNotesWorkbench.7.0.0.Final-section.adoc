...
* The security-related operations it provided (`add-role-repo`, `remove-role-repo`, `add-role-org-unit`, `remove-role-org-unit`, `add-role-project`, `remove-role-project`) have been replaced by more comprehensive see xref:con-business-central-security-management_{context}[Security management] feature.
