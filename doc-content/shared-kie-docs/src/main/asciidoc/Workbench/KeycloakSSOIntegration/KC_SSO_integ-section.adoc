...
Both jBPM and Drools workbenches provide different remote service endpoints that can be consumed by third party clients using the <<knowledge-store-rest-api-con_kie-apis,Knowledge Store REST API>>.

In order to authenticate those services thorough Keycloak the _BasicAuthSecurityFilter_ must be disabled, apply those modifications for the _WEB-INF/web.xml_ file (app deployment descriptor) from jBPM's WAR file:
...
The jBPM workbench provides an administration area which provides user, group and role management features (see xref:con-business-central-security-management_{context}[Security management] section).
