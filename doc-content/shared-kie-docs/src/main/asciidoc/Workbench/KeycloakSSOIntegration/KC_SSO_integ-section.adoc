...
<filter>
  ...
  <init-param>
    ...
    <param-value>{CENTRAL} Realm</param-value>
  </init-param>
</filter>
...
{CENTRAL} provides an administration area which provides user, group and role management features (see <<_wb.userandgroupmgmt,Security management>>).
