...
This section provides an overview of the Workbench SSH keystore and includes a guide for platform users. It explains
how to use the Workbench SSH keystore to register and use it's SSH public keys.
...
Each SSH public key consists of a pair of files on the storage folder:
...
doesn not exist or it contains a wrong value, the default SSH keystore will be loaded.
...
The SSH keystore provides an intuitive to enables users to manage their SSH public keys on the system. It is accessible from
...
After you click the ``SSH Keys`` menu option the SSH Keys Editor will open. It displays a table showing the user SSH public keys and
...
3. Copy the contents of your SSH Public key onto the clipboard. Use ``cat`` command to display your key content. If you
used a different key name, replace ``id_rsa`` with your key name and copy it
[source,shell]
cat ~/.ssh/id_rsa.pub

4. In the SSH keystore UI press the ``Add SSH Key`` button to open the *New SSH public key* form. Specify a name,
...
[NOTE]
====
* ``Name`` field is cannot be empty, this field defines a meaningful name for the user to identify the key on the SSH public keys table.
