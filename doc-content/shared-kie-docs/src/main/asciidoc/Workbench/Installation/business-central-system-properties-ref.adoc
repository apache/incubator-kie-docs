...
ifdef::PAM,DM[]
//This is all about the standalone.jar file, right? So it isn't installed is it? Would it be better to say 'To run standalone {CENTRAL}'. Also, this is confusing because we have the standalone.xml files. So these proper
