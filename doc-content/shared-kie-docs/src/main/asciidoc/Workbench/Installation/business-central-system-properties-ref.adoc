...
* `org.appformer.m2repo.url`: {CENTRAL} uses the default location of the Maven repository when looking for dependencies. It points to the Maven repository inside the {CENTRAL}. For example, `\http://localhost:8080/business-central/maven2`. Set this property before starting up the {CENTRAL}. Default value: File path to the inner `m2` repository.
