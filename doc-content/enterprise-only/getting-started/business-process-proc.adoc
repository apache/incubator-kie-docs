...
= Recreating the MortgageApprovalProcess business process
...
{PRODUCT} contains a predefined selection of node types to simplify business process creation. You will use some of the predefined nodes during the business process creation. The predefined node panel is located on the left side of the diagram editor.
...
+
The *Package* specifies the location that the asset will be created and saved in. In this case, the asset will be created in the *mortgage_app* project in the *myspace* default space.
...
return incdownpayment;
----
. Above the canvas, click *Save*, and *Save*, to confirm your changes.
