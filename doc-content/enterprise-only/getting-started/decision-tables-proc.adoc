...
The goal of this chapter is to introduce you to the *MortgageDecisionTable* decision table. For this tutorial, you will not create and set the decision table conditions. Instead, review the values and the conditions that are already defined in the *Mortgage_Process* sample project's *MortgageDecisionTable* Guided Decision Tables asset. For information about creating decision tables, see {URL_GUIDED_DECISION_TABLES}[_{GUIDED_DECISION_TABLES}_].

.Prerequisites
The business rules have been defined. For more information, see <<_defining_business_rules>>.

.Procedure
//. Log in to {CENTRAL} and click *Menu* -> *Design* -> *Projects*, and *{PROJECT}*.
//. Click *Add Asset* -> *Guided Decision Table*, and enter:
