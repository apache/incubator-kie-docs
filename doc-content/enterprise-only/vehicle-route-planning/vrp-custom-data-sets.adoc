[id='vrp-custom-data-sets_{context}']
...
If the application is running through the `runLocally.sh` script, the data set directory is set to `$HOME/{data-dir-name}/dataset`.

Otherwise, the property is from `application.properties` and defaults to `optaweb-vehicle-routing-backend/local/dataset`.
====
