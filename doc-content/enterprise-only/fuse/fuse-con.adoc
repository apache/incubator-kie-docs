...
You can install {FUSE_LONG} on the Apache Karaf container platform and then install and configure Red Hat Process Automation Manager in that container.

You can also install {FUSE_LONG} on a separate instance of {EAP_LONG} and integrate it with Red Hat Process Automation Manager. The `kie-camel` module provides integration between {FUSE_LONG} and Red Hat Process Automation Manager.
...
[IMPORTANT]
====
For the version of {FUSE_LONG} that Red Hat Process Automation Manager {ENTERPRISE_VERSION} supports, see
ifdef::PAM[]
