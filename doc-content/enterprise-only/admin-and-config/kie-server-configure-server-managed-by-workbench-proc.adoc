...
NOTE: In production environments, use two distinct users, each with one role. In this simple situation, we use only one user named `controllerUser` that has both the `rest-all` and the `kie-server` roles.
