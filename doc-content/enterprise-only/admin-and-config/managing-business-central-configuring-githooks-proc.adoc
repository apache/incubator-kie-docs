...
Each line of the  `*.properties` should have the following format `<exit code>=<message to show>. Where `<exit code>` is
the git hook exit code and `<message to show>` is the text of the message to display on the notification.

For example:

[source]
----
0=Succes! Nothing wrong happened and the git hook worked as expected :)
1=Ups! Something went wrong, check the logs and tell your admin.
...
...
[NOTE]
====
It is not necessary to define all possible exit codes on the `*.properties` file. It will only appear notifications for
the existing file entries.
====
...
* `0`: Succes. A git hook that exits with code `0` will display a success notification on UI.
...
The notification service will choose the right one based on the user language, if there are no available translations
