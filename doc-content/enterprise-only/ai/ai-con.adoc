...
Online shopping is another example of pragmatic AI.  A company uses machine learning to identify customers that have similar purchasing patterns. When a customer shops for a particular product, the shopping web site displays a message telling the customer about similar products that other customers have bought when performing the same search. The company can further enhance the messages with digital decisioning to prevent the suggester from suggesting a product that the customer recently bought.

As this example shows, you can combine machine learning to extract information from data analysis, digital decisioning to model the human knowledge and experience, and mathematical optimization to solve complex problems.
