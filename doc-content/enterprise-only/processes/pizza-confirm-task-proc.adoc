...
image::enterpriseImages/processes/create-task3.png[Creating an outgoing connection from the Process Order task to a user task]

. Convert the new task to a user task.
+
.Convert into a User task
image::enterpriseImages/processes/user_task3.png[Converting in to a user task]
...
image::enterpriseImages/processes/pizza-vals.png[Input the Task Name and Actor in to the Diagram properties window]
...
image::enterpriseImages/processes/val-data-io3.png[Input values in to the Confirmation Data input and output assignments window]
