...
= Running the cloud balancing Hello World application

You can run the sample Hello World application to demonstrate the solver.
...
. Builds the `Solver` based on a solver configuration (in this case an XML file, `cloudBalancingSolverConfig.xml`, from the classpath).
+
