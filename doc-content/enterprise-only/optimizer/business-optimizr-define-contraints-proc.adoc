[id='business-optimizr-define-contraints-proc_{CONTEXT}']
= Define the constraints and calculate the score

When solving a problem, a _score_ represents the quality of a specific solution.
...
to perform incremental score calculation. This class uses {PLANNER_SHORT}'s ConstraintStream API which is inspired by Java 8 Streams and SQL.
