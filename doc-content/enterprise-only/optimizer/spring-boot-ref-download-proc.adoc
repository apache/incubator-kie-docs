...
. Extract the `{PRODUCT_FILE}-reference-implementation.zip` file. This archive contains the `rhpam-7.11.0-openshift-drools-hacep-distribution.zip` file.
. Extract the `rhpam-7.11.0-openshift-drools-hacep-distribution.zip` file. This archive contains the `openshift-drools-hacep-distribution` directory.
. Navigate to the `openshift-drools-hacep-distribution/sources/springboot` directory.
//Anna I'm lost at this point.I don't see anything in CR1 that resembles this.
...
. Navigate to the `optaplanner-spring-boot-school-timetabling-{MAVEN_ARTIFACT_VERSION}` directory.
