...
.Procedure
. In the guided rules designer, click the plus icon (image:rules/5686.png[]) on the right side of the `THEN` section.
+
The *Add a new action* window with the available action elements opens.
+
.Add a new action to the rule
image::enterpriseImages/rules/5695.png[Add a new action to the rule]
+
...
+
.Add a field
image::enterpriseImages/rules/5696.png[Add a field]
+
After you select a field, the window closes automatically.
. Click the edit icon (image:rules/6191.png[]) to define the field value. The field value can be a literal value or a formula.
