...
. Extract the downloaded `{PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.zip` file to a temporary directory.
. In the WebLogic Administration Console, navigate to *Deployments* to view all existing applications.
. Click *Install*.
. Navigate to the temporary directory where you downloaded and extracted the `{PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.zip` file, and go to `{PRODUCT_FILE}-kie-server-ee7/kie-server.war`.
