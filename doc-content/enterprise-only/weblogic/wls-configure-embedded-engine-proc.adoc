[id='wls-configure-embedded-engine-proc']
ifdef::PAM[]
...
ifdef::PAM[]
If you intend to use an embedded engine with your {PRODUCT} application, you must add Maven dependencies to your project by adding the {PRODUCT} bill of materials (BOM) files to the project’s `pom.xml` file.
endif::[]
...
+
ifdef::DM[]
. If you use a {DECISION_ENGINE} with persistence support in your project, you must declare the following hibernate dependencies in the `dependencyManagement` section of your `pom.xml` file by copying the `version.org.hibernate-4ee7` property from the  {PRODUCT} BOM file:
+
