...
You can change the number of replicas of all components when configuring the deployment. If you want to modify the number of independent {KIE_SERVER} pods or to use a different database server, you must modify the template. For instructions about modifying the template, see <<environment-managed-modify-proc_{context}>>.
