...
[NOTE]
====
`kieconfigs-8.0.1` also holds the routes and services relating to these three components.
====

kieconfigs-8.0.1-dashbuilder::
...
kieconfigs-8.0.1-pim::
This contains the process instance migration (PIM) DeploymentConfig and related PIM configuration. If you are using MySQL or any other database with PIM you must use the `kieconfigs-8.0.1-dbs-pim` configMap and edit the `mysql.yaml` file.
