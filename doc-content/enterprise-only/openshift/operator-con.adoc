[id='operator-con_{context}']
= Deployment and management of an {PRODUCT} environment using OpenShift operators

To deploy an {PRODUCT} environment, the OpenShift operator uses a YAML source that describes the environment. {PRODUCT} provides an installer that you can use to form the YAML source and deploy the environment.
