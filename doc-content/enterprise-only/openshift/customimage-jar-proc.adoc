...
For example, you can create a custom class JAR to provide custom Prometheus metrics in the {KIE_SERVER}. For instructions about creating the custom class, see {URL_MANAGING_SETTINGS}#prometheus-monitoring-custom-proc_execution-server[Extending Prometheus metrics monitoring in KIE Server with custom metrics] in _{MANAGING_KIE_SERVER}_. 
