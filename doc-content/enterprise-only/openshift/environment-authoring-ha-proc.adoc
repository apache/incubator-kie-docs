...
If you want to modify the environment defined by the template file, see <<environment-authoring-ha-modify-proc_{context}>>.
endif::PAM[]
...
. If an AMQ scaledown controller image is not available in the `openshift` namespace and you installed the scaledown controller image stream (see <<imagestreams-file-install-proc_{context}>>), set the following parameter:
