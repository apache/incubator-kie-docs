...
.. To assign a role permission to perform a case operation, add it to the list of authorized roles for that operation in the `case-authorization.properties` file. For example, to allow anyone with the `manager` role to perform a `CLOSE_CASE` operation, add it to the list of roles, separated by a comma:
+
...
+
Where `OPERATION` refers to one of the operations listed in the file that begins with `#`. These operations have restrictions, which can be ignored and can be performed by anyone involved in the case.
