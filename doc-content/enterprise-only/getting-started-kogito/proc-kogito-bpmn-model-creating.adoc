...
* The *IBM Business Automation Bundle* VS Code extension is installed and enabled in your VS Code IDE. For information about enabling the VS Code extension, see {URL_GETTING_STARTED_KOGITO_MICROSERVICES}#proc-VS Code-extension_getting-started-kogito-microservices[_Installing the {PRODUCT} VS Code extension bundle_].
