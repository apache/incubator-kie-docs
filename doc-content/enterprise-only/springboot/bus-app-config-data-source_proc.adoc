...
----
.. Optional: To create a schema that uses the `bytea` column type instead of the `oid` column type, set the value of the `org.kie.persistence.postgresql.useBytea` property to `true`:
+
[source]
----
