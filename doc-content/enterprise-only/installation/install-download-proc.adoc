...
ifeval::["{context}" == "install-on-eap"]
* If you want to install {PRODUCT} on {EAP} {EAP_VERSION} using the deployableZIP files, download the following files:
ifdef::PAM[]
...
ifeval::["{context}" == "install-on-jws"]
* To install {KIE_SERVER} on {JWS} using the deployableZIP files, download the following files: 
