...
ifdef::PAM[]
** *{PRODUCT} {PRODUCT_VERSION_LONG} {KIE_SERVER} for All Supported EE8 Containers* (`{PRODUCT_INIT}-{PRODUCT_VERSION}-KS8.zip`)
** *{PRODUCT} {PRODUCT_VERSION_LONG} {CENTRAL} Deployable for EAP 7*
(`{PRODUCT_INIT}-{PRODUCT_VERSION}-BC7.jar`)
** *{PRODUCT} {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_INIT}-{PRODUCT_VERSION}-AO.zip`)
endif::PAM[]
...
ifdef::DM[]
* To install {KIE_SERVER} on {JWS} using the deployable ZIP file, download the *{PRODUCT} {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_INIT}-{PRODUCT_VERSION}-AO.zip`) file.
+
...
ifdef::DM[]
* To install {KIE_SERVER} on {TOMCAT} using the deployable ZIP file, download the *{PRODUCT} {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_INIT}-{PRODUCT_VERSION}-AO.zip`) file.
endif::DM[]
endif::[]


* To run {CENTRAL} without needing to deploy it to an application server, download *{PRODUCT} {PRODUCT_VERSION_LONG} {CENTRAL} Standalone* (`{PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.jar`).
