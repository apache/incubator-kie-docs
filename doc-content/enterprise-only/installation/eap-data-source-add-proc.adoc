...
[NOTE]
====
The PostgreSQL DDL scripts create the PostgreSQL schema with auto-incrementing integer value (OID) columns for entity attributes annotated with `@LOB`. To use other binary column types such as BYTEA instead of OID, you must create the PostgreSQL schema with the `postgresql-bytea-jbpm-schema.sql` script and set the {PRODUCT} `org.kie.persistence.postgresql.useBytea=true` flag. {PRODUCT} does not provide a migration tool to change from an OID-based to a BYTEA-based schema.
====
