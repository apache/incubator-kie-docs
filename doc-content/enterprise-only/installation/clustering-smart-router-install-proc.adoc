...
. Extract the downloaded `{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip` file to a temporary directory. The `{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-smart-router.jar` file is in the extracted `{PRODUCT_FILE}-add-ons` directory.
