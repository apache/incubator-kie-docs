...
. Add a {KIE_SERVER} user with the `kie-server` role to the `_TOMCAT_HOME_/tomcat/conf/tomcat-users.xml` file. In the following example, replace `<USER_NAME>` and `<PASSWORD>` with the user name and password of your choice.
+
[source]
----
