...
[NOTE]
====
The {PRODUCT} JAR file installer does _not_ support the {EAP} distribution installed by yum or RPM Package Manager. If you want to install {PRODUCT} in this type of {EAP} installation, download the *{PRODUCT} {PRODUCT_VERSION} - {CENTRAL} for EAP 7* file and follow the steps in <<assembly_installing-on-eap-deployable_install-on-eap>>.
====
