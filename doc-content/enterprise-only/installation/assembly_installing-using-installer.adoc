...
[NOTE]
====
The {PRODUCT} JAR file installer does _not_ support the {EAP} distribution installed by yum or RPM Package Manager. If you want to install {PRODUCT} in this type of {EAP} installation, download the *{PRODUCT} {PRODUCT_VERSION} Deployable for {EAP} {EAP_VERSION}* file and follow the steps in <<assembly_installing-on-eap-deployable_install-on-eap>>.
====
