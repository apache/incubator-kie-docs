...
[NOTE]
====
Use this method only for non-production environments. For production environments, store passwords in a keystore as described in xref:securing-passwords-jws-proc_{context}[].
====
