...
You can update or upgrade {PRODUCT} versions 7.2.x through 7.3.1 directly. For example, you can use the 7.3.0 upgrade tool to upgrade {PRODUCT} 7.2.x to version 7.3.0. Likewise, you can directly upgrade {PRODUCT} 7.2.0 to version 7.3.1. However, you must have {PRODUCT} 7.3.1 to upgrade to version 7.4.0.
...
Do not attempt to update `{PRODUCT_INIT}-kie-server-ee7` to `{PRODUCT_INIT}-kie-server-ee8`. It is not supported but the update tool will not prevent you from doing it. If you are updating {PRODUCT} 7.3.0 to 7.3.1, update `{PRODUCT_INIT}-kie-server-ee7` only. If you are updating {PRODUCT} 7.3.1 to 7.4.x, update `{PRODUCT_INIT}-kie-server-ee8` only.
...
--
+
If you are upgrading to {PRODUCT} 7.4.x from 7.2.0 or higher, first apply the 7.3.1 patch update to your current version of {PRODUCT} and then follow this procedure again to upgrade to 7.4.x.
+
