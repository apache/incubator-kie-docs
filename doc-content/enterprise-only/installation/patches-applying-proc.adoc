...
+
This update tool automates the update of certain components of {PRODUCT}, such as {CENTRAL}, {KIE_SERVER}, and the headless {PRODUCT_SHORT} controller. Use this update tool first to apply updates and then install any other updates or new release artifacts that are relevant to your {PRODUCT} distribution.
+
...
* `controller-ee7`: Updates the headless {PRODUCT_SHORT} controller (`controller.war`)
ifdef::DM[]
* `controller-jws`: Updates the headless {PRODUCT_SHORT} controller on Red Hat JBoss Web Server (`controller.war`)
endif::DM[]
...
Example update to headless {PRODUCT_SHORT} controller, if used:
[source]
----
