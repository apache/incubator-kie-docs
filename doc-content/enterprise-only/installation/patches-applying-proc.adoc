...
Do not attempt to update `kie-server-ee7` to `kie-server-ee8`. It is not supported but the update tool will not prevent you from doing it. If you are updating {PRODUCT} 7.3.0 to 7.3.1, update `kie-server-ee7` only. If you are updating {PRODUCT} 7.3.1 to 7.4.x, update `kie-server-ee8` only.
