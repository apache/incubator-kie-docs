...
--
+
If you are upgrading to a new minor release of {PRODUCT}, such as an upgrade from version {PRODUCT_VERSION}.x to 7.5, first apply the latest patch update to your current version of {PRODUCT} and then follow this procedure again to upgrade to the new minor release.
+
NOTE: If you are upgrading from {PRODUCT} 7.4.1 to {PRODUCT} 7.5.0, you must use the {CENTRAL} migration tool on your `.niogit` directory to prepare project data and system configurations for migration to {PRODUCT} 7.5. For instructions on using the migration tool, see
ifeval::["{context}" == "patching-upgrading"]
xref:migration-tool-niogit-7.4-proc_patching-upgrading[].
endif::[]
ifeval::["{context}" == "execution-server"]
{URL_PATCHING_UPGRADING}#migration-tool-niogit-7.4-proc_patching-upgrading[_{PATCHING_UPGRADING}_].
endif::[]
+
