...
. Add users and roles to the `_JWS_HOME_/tomcat/conf/tomcat-users.xml` file. In the following example, `<ROLE_NAME>` is a role supported by {PRODUCT} and `<USER_NAME>` and `<USER_PWD>` is a user and password combination of your choice:
