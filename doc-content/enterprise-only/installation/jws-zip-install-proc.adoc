...
ifdef::DM[]
* The *{PRODUCT} {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip`) file has been downloaded, as described in <<install-download-proc_{context}>>.
endif::DM[]
ifeval::["{context}" == "install-on-jws"]
