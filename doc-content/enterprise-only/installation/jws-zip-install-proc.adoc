...
{JWS} {JWS_VERSION_LONG} or higher server instance.
endif::[]
...
ifeval::["{context}" == "install-on-tomcat"]
Ensure the names of the {PRODUCT} deployments you are copying do not conflict with your existing deployments in the {TOMCAT} instance.
endif::[]
====
...
. Move the `jboss-jacc-api-<VERSION>.jar`, `slf4j-api-<VERSION>.jar`, and `slf4j-jdk14-<VERSION>.jar` files from the `ENGINE_TEMP_DIR/lib` directory to the `_TOMCAT_HOME_/tomcat/lib` directory, where `<VERSION>` is the version artifact file name, in the `lib` directory.
. Unzip the `{PRODUCT_FILE}-maven-repository.zip` file. 
. Copy the following libraries from the unzipped Maven repository to the `_TOMCAT_HOME_/tomcat/lib` folder:
endif::[]
ifdef::DM[]
