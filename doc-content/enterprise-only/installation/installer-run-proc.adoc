...
+
ifeval::["{context}" == "install-on-jws"]
You cannot install {CENTRAL} on Red Hat JBoss 5.0 Web Server. You can only install it on {EAP}. However, you can install {KIE_SERVER} and the {PRODUCT_SHORT} controller on Red Hat JBoss 5.0 Web Server. The controller is used to manage {KIE_SERVER}. Install the controller if you plan to manage multiple {KIE_SERVER} instances.
endif::[]
ifeval::["{context}" == "install-on-eap"]
[NOTE]
====
