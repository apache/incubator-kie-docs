...
+
ifeval::["{context}" == "install-on-jws"]
You cannot install {CENTRAL} on Red Hat JBoss Web Server. You can only install it on {EAP}. However, you can install {KIE_SERVER} and the {HEADLESS_CONTROLLER} on Red Hat JBoss Web Server. The {HEADLESS_CONTROLLER} is used to manage {KIE_SERVER}. Install the {HEADLESS_CONTROLLER} if you plan to manage multiple {KIE_SERVER} instances.
ifeval::["{context}" == "install-on-eap"]
[NOTE]
====
