...
+
. If you selected *Customize {KIE_SERVER} properties*, on the *{KIE_SERVER} Properties Configuration* page change any of the following:
+
* Change *{KIE_SERVER} ID* to change the name of the {KIE_SERVER} property.
ifeval::["{context}" == "install-on-eap"]
* Change *Controller URL* to change the URL of the {HEADLESS_CONTROLLER}.
endif::[]
