...
* The {PRODUCT_PAM} {PRODUCT_VERSION_LONG} Maven Repository (`{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-maven-repository.zip`) and the {PRODUCT} {PRODUCT_VERSION}.x Add-Ons (`{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-add-ons.zip`) files have been downloaded, as described in  <<install-download-proc_{context}>>.
