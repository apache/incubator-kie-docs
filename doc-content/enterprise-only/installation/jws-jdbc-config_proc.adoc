...
* The {PRODUCT} {PRODUCT_VERSION_LONG} Maven Repository (`rhpam-7.4.0-maven-repository.zip`) and the {PRODUCT} {PRODUCT_VERSION_LONG} Add-Ons (`{PRODUCT_INIT}-{PRODUCT_VERSION}-add-ons.zip`) files have been downloaded, as described in  <<install-download-proc_{context}>>.
