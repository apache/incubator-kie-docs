...
* The {PRODUCT} {PRODUCT_VERSION} Maven Repository (`{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-maven-repository.zip`) and the {PRODUCT} {PRODUCT_VERSION} Add-ons (`{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip`) files have been downloaded, as described in  <<install-download-proc_{context}>>.
