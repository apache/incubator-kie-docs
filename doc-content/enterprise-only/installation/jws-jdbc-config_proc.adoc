...
* The {PRODUCT} {PRODUCT_VERSION}.x Maven Repository (`{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-maven-repository.zip`) and the {PRODUCT} {PRODUCT_VERSION}.x Add-Ons (`{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip`) files have been downloaded, as described in  <<install-download-proc_{context}>>.
...
.. Extract `{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-{PRODUCT_INIT_TOP}-{PRODUCT_VERSION}-AO.zip` in a temporary directory, for example `TEMP_DIR`.
