...
If a {KIE_SERVER} is not configured in your {PRODUCT_PAM} environment, or if you require additional {KIE_SERVERS} in your {PRODUCT_PAM} environment, you must configure a {KIE_SERVER} to connect to {CENTRAL}.
...
--
+
. To verify that {KIE_SERVER} starts successfully, send a GET request to `http://_SERVER:PORT_/kie-server/services/rest/server/` when {KIE_SERVER} is running. For more information about running {PRODUCT_PAM} on {KIE_SERVER}, see {URL_INSTALLING_AND_CONFIGURING}#eap-ba-dm-run-proc_install-on-eap[_Running {PRODUCT_PAM}_].
+
