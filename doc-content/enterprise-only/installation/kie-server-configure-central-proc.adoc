...
--
+
. Send a GET request to `http://_SERVER:PORT_/kie-server/services/rest/server/` to verify that {KIE_SERVER} started successfully. After authentication, you receive an XML response similar to the following example:
+
[source,xml]
----
