...
[NOTE]
====
If you installed the {HEADLESS_CONTROLLER} on a separate {EAP} instance from the {EAP} instance where you installed the {KIE_SERVER}, you can start {CENTRAL} with the `standalone.sh` script without specifying the `standalone-full.xml` file. In this case, ensure that you make the configuration changes listed in this section to the `standalone.xml` file.
====
