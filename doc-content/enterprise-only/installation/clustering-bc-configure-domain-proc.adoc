...
* {DATAGRID} {DATAGRID_VERSION_LONG} is installed as described in xref:clustering-datagrid-proc_{context}[].
* {EAP} is installed on each node of the cluster as described in xref:clustering-download-eap-bc-proc[].
