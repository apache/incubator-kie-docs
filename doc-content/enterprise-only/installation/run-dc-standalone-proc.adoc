...
.Prerequisites
* The *Red Hat Process Automation Manager {PRODUCT_VERSION_LONG} {CENTRAL} Standalone* (`{PRODUCT_FILE}-{URL_COMPONENT_CENTRAL}-standalone.jar`) and the *Red Hat Process Automation Manager {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_FILE}-add-ons.zip`) files have been downloaded from the {PRODUCT_DOWNLOAD_LINK}[Software Downloads] page for Red Hat Process Automation Manager {PRODUCT_VERSION}, as described in <<install-download-proc_{context}>>.
