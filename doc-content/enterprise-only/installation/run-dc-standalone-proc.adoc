...
.Prerequisites
* The *{PRODUCT} {PRODUCT_VERSION_LONG} {CENTRAL} Standalone* (`{PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.jar`) and the *{PRODUCT} {PRODUCT_VERSION_LONG} Add Ons* (`{PRODUCT_INIT}-{PRODUCT_VERSION}-AO.zip`) files have been downloaded from the {PRODUCT_DOWNLOAD_LINK}[IBM Support] page.

.Procedure

. Extract the downloaded `{PRODUCT_FILE}-{PRODUCT_INIT}-{PRODUCT_VERSION}-AO.zip` to a temporary directory. This archive includes the `{PRODUCT_FILE}-standalone-sample-configuration.zip` file.
. Extract the `{PRODUCT_FILE}-standalone-sample-configuration.zip` file to the directory that contains the `{PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.jar` file. The `{PRODUCT_FILE}-standalone-sample-configuration.zip` file contains the following sample configuration files:
+
...
+
You can run the `{PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.jar` files with the sample data provided in the configuration files or you can customize the data for your requirements.
...
----
java -jar {PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.jar --cli-script=application-script.cli
----
...
[source,subs="attributes+"]
----
java -jar {PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.jar --cli-script=application-script.cli -D<PROPERTY>=<VALUE> -D<PROPERTY>=<VALUE>
----
+
...
----
java -jar {PRODUCT_INIT}-{PRODUCT_VERSION}-KS7.jar \
