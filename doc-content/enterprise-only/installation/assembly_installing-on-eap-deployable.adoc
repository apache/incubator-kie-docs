...
= Installing {PRODUCT_PAM} from ZIP files
You can use the {PRODUCT_PAM} ZIP files (one for {CENTRAL} and one for {KIE_SERVER}) to install {PRODUCT_PAM} without using the installer.
