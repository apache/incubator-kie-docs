...
[source,subs="attributes+"]
----
Specify the home directory of one of the following servers:  Red Hat JBoss EAP 7.2 or {JWS} {JWS_VERSION_LONG}
----
endif::[]
ifdef::DM[]
[source,subs="attributes+"]
...
----
. Enter `y` to create XML files that contains the installation data, or `n` to complete the installation. If you enter `y`, you are prompted to specify a path for the XML files.
...
Use the The `auto.xml` file to easily repeat a {PRODUCT} installation on the same type of server with the same configuration as the original installation on multiple systems. Note that you must change the `installpath` parameter in the `auto.xml` file to specify the path of the new server that you want to install {PRODUCT} on. Enter the following command to perform an installation with the XML file:
+
[source,subs="attributes+"]
----
ifdef::PAM[]
