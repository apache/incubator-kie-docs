...
{PRODUCT} versions are designated with a numerical `Major.Minor.Patch` format, such as `8.0.1`. In this example, the major release is `8.x.x`, the minor release is `8.0.x`, and the patch release is `8.0.1`. Major releases often require data migration, while minor release upgrades and patch updates are typically managed with update tools provided with the {PRODUCT} release artifacts.
