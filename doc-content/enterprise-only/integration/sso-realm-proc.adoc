...
. Download *Red Hat Single Sign-on {RH-SSO_VERSION_LONG} Server* (`rh-sso-{RH-SSO_VERSION_LONG}.zip`) and apply the latest patches.
...
* Update the `_RHSSO_HOME_/standalone/configuration/standalone-full.xml` file and set a port offset to 100. For example:
+
[source]
----
