...
.Procedure
. Open the RH-SSO Admin Console in a web browser and log in using the admin credentials that you created while installing RH-SSO.
+
* To open the RH-SSO Admin Console in a standalone environment, enter `\http://localhost:8180/auth/admin` in a web browser.
* To open the RH-SSO Admin Console in an OpenShift deployment, enter (what??).
+
