...
* Ad-hoc – Supports optional modeling paths that can be recommended to end users or automatically triggered by rules. Must be set to `true` to show the case management properties.
...
New support for showing the label name for sequence flows coming to and from gateways, nodes, tasks, and end events. Users can now click any sequence flow line to view the label name.