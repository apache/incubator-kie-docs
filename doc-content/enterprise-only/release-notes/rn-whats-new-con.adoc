...
The test scenarios designer in {CENTRAL} is now available. You can use it to complete the following tasks:
...
=== Case modeler
The Case designer and new process designer are merged into one editor to enable users to switch between views.
...
extension for OpenShift enabled (the `KIE_WORKBENCH_CONTROLLER_OPENSHIFT_ENABLED` environment variable). These features enable
