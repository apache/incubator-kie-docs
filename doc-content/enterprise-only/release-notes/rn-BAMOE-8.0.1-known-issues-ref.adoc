...
Issue: When a process has a variable of the type `java.util.Date`, the generated form, if the `showTime` attribute is `true`, the process does not allow inserting the time part. Then after submitting the `Date` variable shows all zeros in the time part of the datatype.
