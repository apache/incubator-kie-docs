...
Issue: DataObject from canvas is missing in assignments when case file var is present. This applies to both top level nodes as well as nodes placed in sub-processes.

Steps to reproduce:

. Create a case project.
. Create a case definition.
. Add case File variable to the process.
. Create *DataObject* on canvas.
. Create node with assignments on canvas or in sub-process.
. Activate node with assignments, open assignments and click on *Source/Target*.

Actual result: DataObject is missing in listed items.

Expected result: Both case file variable as well as DataObject from Canvas is present in the listed items.
...
. Create multiple instance node or data object on canvas.
. In multiple instance node, set MI Collection input/output and try to change Data Input/Output type.
. In data object, try to change the data type.
