...
. Open *Case Management* in the *Properties* panel and add a new case roles as `owner:1`.
...
. Create a case project.
. Create a case definition.
. Add the case file variable to the process.
. Create the *DataObject* on canvas.
. Create a node with assignments on the canvas or in the sub-process.
. Activate the node that contains the assignments, open assignments and click *Source/Target*.

Actual result: The *DataObject* is missing from the listed items.

Expected result: Both the case file variable and the DataObject from canvas are present in the listed items.
...
. In the data object, try to change the data type.
...
Issue: When you are using an engine embedded in a {KIE_SERVER} packaged as a Spring Boot application, the bean defined as `userGroupCallback` is not injected into the engine, and therefore when you try to call some of the rest endpoints, fetching some tasks based on the user or groups assigned to them (such as `potOwner`, `stakeHolders`, `businessAdmin`, etc) will not work as expected because the `UserGroupCallback` implementation used in the engine is different than the one defined at the Spring boot application level. Note that this is only applicable to cases and not to processes.
...
Issue: Kafka dependencies for the community are not aligned with the dependencies supported by RHOAR for {PRODUCT} {PRODUCT_VERSION}. The current version of the Kafka community version is 2.8.0 and it must be aligned with the version used by {AMQ_STREAMS} 2.1.0 which is 3.1.0 for the community.
...
Issue: The current Spring Boot version (2.6.6) in the Maven repository is not certified. You receive a mismatch for the narayana starter in productized binaries.
...
Issue: The Kafka-clients dependency version for {KOGITO} Spring Boot is by default managed by the `org.springframework.boot:spring-boot-dependencies` BOM. Depending on which Spring Boot version you use, you might end up with an unsupported or vulnerable version of Kafka-clients. You must override the default dependency in your `kogito-spring-boot-bom` to make sure you have the expected Kafka-clients version.
