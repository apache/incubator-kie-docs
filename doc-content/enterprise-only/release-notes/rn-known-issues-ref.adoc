...
== {CENTRAL}
ifdef::DM[]
.All line separators in RHS in spreadsheet are escaped https://issues.jboss.org/browse/RHDM-903[RHDM-903]
...
. Add more that one line to a row in the in *ACTION* column of a spreadsheet.
. Build the spreadsheet with {PRODUCT}.

Expected result: The spreadsheet builds successfully.

Actual result: The build fails will errors similar to `line 1:15 no viable alternative at character '\'`.
