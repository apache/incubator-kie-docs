...
Issue: Task operations such as *Start*, *Complete*, or *Claim* are noticeably slower when the KIE server API or `UserTaskService` is used. This issue appeared when a join was added to the `getTaskInstanceById` query. The performance of these operations will be restored to the previous state in a future patch release.
