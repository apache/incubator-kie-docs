...
ifdef::PAM[]
== {KIE_SERVER}
.Query-related operations on Process Server running on Red Hat JBoss Web Server or SpringBoot with DB2 return an error [https://issues.jboss.org/browse/RHPAM-2292[RHPAM-2292]]

Issue: Query-related operations on Process Server running on Red Hat JBoss Web Server or SpringBoot with DB2 return an error.

Steps to reproduce:
{KIE_SERVER} registers a new default query.

Expected result: Correct information is returned from {KIE_SERVER}.

Actual result: An error code is returned.

Workaround: For {JWS}, set `defaultAutoCommit=false` in the `XADataSource`. For SpringBoot, set `spring.datasource.defaultAutoCommit=false` in application properties.
endif::[]
