...
Issue: In a high-availability deployment of {CENTRAL}, Sometimes the service is not deployed to a {KIE_SERVER} when you use the {CENTRAL} user interface in a high-availability deployment.

Workaround: There is no workaround in an on-premise deployment using the {EAP}. Do not use an on-premises high-availability deployment of {CENTRAL} to deploy services to {KIE_SERVERS}.

In a deployment on {OPENSHIFT}, the issue is resolved starting in {PRODUCT} version 7.7.1. When you deploy a high-availability authoring environment on {OPENSHIFT} in {PRODUCT} version 7.7.0, you must enable the `OpenShiftStartupStrategy` setting. For instructions about enabling this setting, see
ifdef::PAM[]
