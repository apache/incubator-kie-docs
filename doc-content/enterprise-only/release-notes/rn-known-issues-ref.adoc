...
== {CENTRAL}
ifdef::DM[]
.Guided rules expose parenthesis outside of an expression [https://issues.jboss.org/browse/RHDM-1046[RHDM-1046]]

Issue: In a guided rule, if you use parenthesis in an expression in a *Data* cell, the expression is not displayed correctly.

like (red), (blue) in "Data" cell, it will be not presented correctly in "Source" Car( color in ("red)", "(blue") )

Steps to reproduce:

. Create a guided rule.
. Add an expression that contains parenthesis to the *Data* cell, for example `(red)`.
. Click *Source*.

Expected result: The source displays `(red)`.

Actual result: The source displays `("red")`.
