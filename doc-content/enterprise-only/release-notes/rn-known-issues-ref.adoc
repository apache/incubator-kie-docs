...
Issue: If you use the installer to install both {CENTRAL} and {KIE_SERVER}, on the Advanced Configuration page the default value for `kie-server` is incorrect.
...
Expected result: The value of the `kie-server` URL is `\http://localhost:8080/controller/rest/controller`.

Actual result: The value of the `kie-server` URL is `\http://localhost:8080/{URL_COMPONENT_CENTRAL}/rest/controller`
...
.WIP: A container is removed from the UI even though it was not possible to stop it [https://issues.jboss.org/browse/RHPAM-1698[RHPAM-1698]]
...
. In a terminal, change directory to the `<JWS-HOME>/jws-5.0/tomcat/lib` directory where you installed {PRODUCT}.
...
Issue: When the value of `LocalDateTime` is converted to a flat value in the Multiple Subform, the time zone is wrong. 
