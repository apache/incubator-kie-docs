...
. Define column that check `Car.color is contained in comma separated list`
. Select `Model` tab and put value `(red), (blue)` into corresponding cell


Expected result: `Car(color in ("(red)", "(blue)"))` rule is generated

Actual result: `Car( color in ("red)", "(blue") )` rule is generated
