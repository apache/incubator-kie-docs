...
NOTE: For any custom service tasks, ensure the handler is registered in the deployment descriptor in the *Work Item Handlers* section, where the name is the same as the `nodeType` used for creating a dynamic service task. For more information about registering the deployment descriptor, see {URL_DEVELOPING_PROCESS_SERVICES}#registering_custom_tasks_using_the_deployment_descriptor_inside_business_central[_{DEVELOPING_PROCESS_SERVICES}_].
