...
|===
+
The `caseStageId` is the name of the stage in the case definition where the dynamic task is to be created. This can be any dynamic or service task payload. See {URL_DEVELOPING_PROCESS_SERVICES}#case-management-dynamic-service-task-API-proc[_{DEVELOPING_PROCESS_SERVICES}_].
