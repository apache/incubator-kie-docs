...
* _Wait for completion_ (by default true): If this property is true, this sub-process node will only continue if the child process that was started has terminated its execution (completed or aborted); otherwise it will  continue immediately after starting the sub-process (so it will not wait for its completion).
...
* __CollectionOutput__: The name of a variable that represents a collection of elements that will gather all output of the multi instance sub process
