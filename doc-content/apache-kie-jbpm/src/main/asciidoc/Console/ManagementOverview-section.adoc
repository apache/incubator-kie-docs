...
That panels can be expanded or collapsed using the standard {CENTRAL} docks controls.
...
the possibility of define a custom range of dates

image::Console/console-datefilter.png[align="left", title="Date field filter opened"]

== Advanced filter dock

Provides the ability to create more complex filters, allowing to select any item (process instance, task,..) field available
...
Once the column is selected, depending on its type, a new dropdown list is opened with the kind of restrictions available
...
or set the number of items to show on a page.
...
** Bulk actions in some of the management screens like Process Instances, Execution errors the possibility of bulk action
